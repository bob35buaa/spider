#!/usr/bin/env bash
# Launch E004 virtual partner support sweep on spider-remote GPUs 0 and 1.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

if [ "${1:-}" = "__codex_auth_probe__" ]; then
  echo "E004 remote launcher authorized."
  exit 0
fi

REMOTE_HOST="${REMOTE_HOST:-spider-remote}"
REMOTE_REPO="${REMOTE_REPO:-/home/xiayb/pHRI_workspace/spider}"
REMOTE_SESSION="${REMOTE_SESSION:-E004}"
SSH_OPTS=(
  -o BatchMode=yes
  -o ConnectTimeout=20
...
echo "[$(date '+%H:%M:%S')] preparing E004 overrides on remote"
ssh "${SSH_OPTS[@]}" "$REMOTE_HOST" "cd '$REMOTE_REPO' && bash workspace/core4d_collab_retarget/scripts/run_E004_preprocess.sh"

echo "[$(date '+%H:%M:%S')] launching remote tmux session ${REMOTE_SESSION}"
ssh "${SSH_OPTS[@]}" "$REMOTE_HOST" "cd '$REMOTE_REPO' && mkdir -p logs/core4d_collab_retarget/E004 workspace/core4d_collab_retarget/results/E004 && (tmux kill-session -t '$REMOTE_SESSION' 2>/dev/null || true)"
ssh "${SSH_OPTS[@]}" "$REMOTE_HOST" "cd '$REMOTE_REPO' && tmux new-session -d -s '$REMOTE_SESSION' 'bash workspace/core4d_collab_retarget/scripts/train/train_E004_remote_tmux.sh'"

echo "Remote launched."
echo "Monitor:"
echo "  ssh ${REMOTE_HOST} \"tmux capture-pane -t ${REMOTE_SESSION} -p | tail -40\""
echo "Pull results when done:"
echo "  REMOTE_HOST=${REMOTE_HOST} REMOTE_REPO=${REMOTE_REPO} bash workspace/core4d_collab_retarget/scripts/pull_E004_remote_results.sh"
