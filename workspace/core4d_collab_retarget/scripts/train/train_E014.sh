#!/usr/bin/env bash
# E014: COLA-B kinematic support + soft weld sweep.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

MODE="${1:-local_wave}"
GPU="${2:-0}"
VARIANTS_FILE="${VARIANTS_FILE:-workspace/core4d_collab_retarget/scripts/E014/variants.tsv}"
RESULTS="${RESULTS:-workspace/core4d_collab_retarget/results/E014}"
LOGS="${LOGS:-logs/core4d_collab_retarget/E014}"
...
snapshot_task() {
  ...
  cp "$src"/scene_e014_jointB_*.xml "$dst/" 2>/dev/null || true
...
snapshot_variants() {
  ...
  if [ "${#variants[@]}" -gt 0 ]; then
    echo "[$(date '+%H:%M:%S')] === E014 scene snapshot variants=${variants[*]} ==="
...
run_one() {
  ...
  row="$(variant_row "$variant")"
  if [ -z "$row" ]; then
    echo "Unknown E014 variant: $variant" >&2
    return 2
  fi
  task="$(field_from_row "$row" 2)"
...
run_variants() {
  ...
  if [ "${#variants[@]}" -eq 0 ]; then
    echo "No E014 variants selected" >&2
    exit 2
  fi
  local status=0
  ...
  for variant in "${variants[@]}"; do
    if ! run_one "$variant" "$gpu"; then
      echo "[$(date '+%H:%M:%S')] WARN ${variant} failed; continuing E014 queue" | tee -a "$LOGS/failed_${MODE}.log" >&2
      status=1
    fi
  done
  if [ "${SKIP_EVAL:-0}" != "1" ]; then
    .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E014.py "${variants[@]}" | tee "$LOGS/eval_${MODE}.log"
  fi
  return "$status"
}
...
case "$MODE" in
  __codex_auth_probe__)
    echo "E014 train entrypoint authorized."
    ;;
  smoke)
  ...
  one|single)
    ...
    run_one "$VARIANT" "$GPU"
    .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E014.py "$VARIANT" | tee "$LOGS/eval_${VARIANT}.log"
    ;;
  eval)
    shift || true
    .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E014.py "$@" | tee "$LOGS/eval_E014.log"
    ;;
  *)
    echo "Usage:"
    echo "  $0 smoke 0"
    ...
    echo "  $0 one 0 E014_box025_p2_jointB_t02"
    echo "  $0 eval [variant ...]"
    exit 2
    ;;
esac
