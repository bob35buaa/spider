#!/usr/bin/env bash
# E004: true-freejoint virtual partner support sweep.
set -euo pipefail
...
LOGS="${LOGS:-logs/core4d_collab_retarget/E004}"
...
all_variants() {
  awk -F '\t' 'NF && $1 !~ /^#/ && $5 != "probe" {print $1}' "$VARIANTS_FILE"
}
...
variants_for_wave() {
  local wave=$1
  awk -F '\t' -v want_wave="$wave" 'NF && $1 !~ /^#/ && $7 == want_wave && $5 != "probe" {print $1}' "$VARIANTS_FILE"
}
...
snapshot_variants() {
  ...
  if [ "${#variants[@]}" -gt 0 ]; then
    echo "[$(date '+%H:%M:%S')] === E004 scene snapshot variants=${variants[*]} ==="
...
run_one() {
  ...
  row="$(variant_row "$variant")"
  if [ -z "$row" ]; then
    echo "Unknown E004 variant: $variant" >&2
    return 2
  fi
  task="$(field_from_row "$row" 2)"
...
run_variants() {
  ...
  if [ "${#variants[@]}" -eq 0 ]; then
    echo "No E004 variants selected" >&2
    exit 2
  fi
  ...
  if [ "${SKIP_EVAL:-0}" != "1" ]; then
    .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E004.py "${variants[@]}" | tee "$LOGS/eval_${MODE}.log"
  fi
}

case "$MODE" in
  __codex_auth_probe__)
    echo "E004 train entrypoint authorized."
    ;;
  smoke)
  ...
  one|single)
    ...
    run_one "$VARIANT" "$GPU"
    .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E004.py "$VARIANT" | tee "$LOGS/eval_${VARIANT}.log"
    ;;
  local_wave)
  ...
  eval)
    shift || true
    .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E004.py "$@" | tee "$LOGS/eval_E004.log"
    ;;
  *)
    echo "Usage:"
    echo "  $0 smoke 0"
    ...
    echo "  $0 one 0 E004_box025_p2_s20"
    echo "  $0 eval [variant ...]"
    exit 2
    ;;
esac
