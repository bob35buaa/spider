#!/usr/bin/env bash
# E030: lower-body geometry / surface-control.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

MODE="${1:-local}"
GPU="${2:-0}"
VARIANTS_FILE="${VARIANTS_FILE:-workspace/core4d_collab_retarget/results/E030/manifest.tsv}"
RESULTS="${RESULTS:-workspace/core4d_collab_retarget/results/E030}"
LOGS="${LOGS:-logs/core4d_collab_retarget/E030}"
...
snapshot_variants() {
  ...
  echo "[$(date '+%H:%M:%S')] === E030 scene snapshot tasks=${tasks[*]} ==="
  bash workspace/core4d_collab_retarget/scripts/convert/snapshot_scenes.sh E030 "${tasks[@]}"
}
...
run_one() {
  ...
  row="$(variant_row "$variant")"
  if [ -z "$row" ]; then
    echo "Unknown E030 variant: $variant" >&2
    return 2
  fi
  task="$(field_from_row "$row" 3)"
...
run_variants() {
  ...
  local -a variants=("$@")
  if [ "${#variants[@]}" -eq 0 ]; then
    echo "No E030 variants selected" >&2
    exit 2
  fi
  local status=0
  ...
  for variant in "${variants[@]}"; do
    if ! run_one "$variant" "$gpu" "${RUN_EXTRA_ARGS[@]}"; then
      echo "[$(date '+%H:%M:%S')] WARN ${variant} failed; continuing E030 queue" | tee -a "$LOGS/failed_${MODE}.log" >&2
      status=1
    fi
  done
  if [ "${SKIP_EVAL:-0}" != "1" ]; then
    .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E030.py "${variants[@]}" | tee "$LOGS/eval_${MODE}.log"
  fi
  return "$status"
}
...
case "$MODE" in
  __codex_auth_probe__)
    echo "E030 train entrypoint authorized."
    ;;
  smoke)
  ...
  one|single)
    ...
    run_one "$VARIANT" "$GPU"
    if [ "${SKIP_EVAL:-0}" != "1" ]; then
      .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E030.py "$VARIANT" | tee "$LOGS/eval_${VARIANT}.log"
    fi
    ;;
  eval)
    shift || true
    .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E030.py "$@" | tee "$LOGS/eval_E030.log"
    ;;
  *)
    echo "Usage:"
    echo "  $0 smoke 0"
    ...
    echo "  $0 one 0 E030_box025_p1_tinygeom_surface_gate"
    echo "  $0 eval [variant ...]"
    exit 2
    ;;
esac
