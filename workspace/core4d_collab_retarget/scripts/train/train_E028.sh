#!/usr/bin/env bash
# E028: D003 Box021 13-case local serial support-proxy dynamic retarget.
set -euo pipefail
...
snapshot_variants() {
  ...
  echo "[$(date '+%H:%M:%S')] === ${EXP_ID} scene snapshot tasks=${tasks[*]} ==="
  bash workspace/core4d_collab_retarget/scripts/convert/snapshot_scenes.sh "$EXP_ID" "${tasks[@]}"
}
...
extract_keyframes() {
  ...
  for f in 16 32 50 75 90 100 115 130 160 180 204 250; do
...
case "$MODE" in
  ...
  *)
    ...
    echo "  $0 smoke 0"
    echo "  $0 local 0"
    echo "  $0 one 0 E028_d003_box021_20231020_020_p1_canonical_t02"
    echo "  $0 eval [variant ...]"
    exit 2
    ;;
esac
