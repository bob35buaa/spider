#!/usr/bin/env bash
# E007: COLA-style support-body proxy sweep.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

MODE="${1:-local_wave}"
GPU="${2:-0}"
VARIANTS_FILE="${VARIANTS_FILE:-workspace/core4d_collab_retarget/scripts/E007/variants.tsv}"
RESULTS="${RESULTS:-workspace/core4d_collab_retarget/results/E007}"
LOGS="${LOGS:-logs/core4d_collab_retarget/E007}"
...
snapshot_variants() {
  ...
  if [ "${#variants[@]}" -gt 0 ]; then
    echo "[$(date '+%H:%M:%S')] === E007 scene snapshot variants=${variants[*]} ==="
...
run_one() {
  ...
  row="$(variant_row "$variant")"
  if [ -z "$row" ]; then
    echo "Unknown E007 variant: $variant" >&2
    return 2
  fi
  task="$(field_from_row "$row" 2)"
...
run_variants() {
  ...
  if [ "${#variants[@]}" -eq 0 ]; then
    echo "No E007 variants selected" >&2
    exit 2
  fi
  ...
  if [ "${SKIP_EVAL:-0}" != "1" ]; then
    .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E007.py "${variants[@]}" | tee "$LOGS/eval_${MODE}.log"
  fi
}

case "$MODE" in
  __codex_auth_probe__)
    echo "E007 train entrypoint authorized."
    ;;
  smoke)
  ...
  one|single)
    ...
    run_one "$VARIANT" "$GPU"
    .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E007.py "$VARIANT" | tee "$LOGS/eval_${VARIANT}.log"
    ;;
  local_wave)
  ...
  eval)
    shift || true
    .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E007.py "$@" | tee "$LOGS/eval_E007.log"
    ;;
  *)
    echo "Usage:"
    echo "  $0 smoke 0"
    ...
    echo "  $0 one 0 E007_box025_p2_yneg_k20_simdt"
    echo "  $0 eval [variant ...]"
    exit 2
    ;;
esac
