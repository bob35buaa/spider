#!/usr/bin/env bash
# E005: corrected partner-force timing and support-site geometry sweep.
set -euo pipefail
...
LOGS="${LOGS:-logs/core4d_collab_retarget/E005}"
...
snapshot_variants() {
  ...
  if [ "${#variants[@]}" -gt 0 ]; then
    echo "[$(date '+%H:%M:%S')] === E005 scene snapshot variants=${variants[*]} ==="
...
run_one() {
  ...
  row="$(variant_row "$variant")"
  if [ -z "$row" ]; then
    echo "Unknown E005 variant: $variant" >&2
    return 2
  fi
  task="$(field_from_row "$row" 2)"
...
run_variants() {
  ...
  if [ "${#variants[@]}" -eq 0 ]; then
    echo "No E005 variants selected" >&2
    exit 2
  fi
  ...
  if [ "${SKIP_EVAL:-0}" != "1" ]; then
    .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E005.py "${variants[@]}" | tee "$LOGS/eval_${MODE}.log"
  fi
}

case "$MODE" in
  __codex_auth_probe__)
    echo "E005 train entrypoint authorized."
    ;;
  smoke)
  ...
  one|single)
    ...
    run_one "$VARIANT" "$GPU"
    .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E005.py "$VARIANT" | tee "$LOGS/eval_${VARIANT}.log"
    ;;
  local_wave)
  ...
  eval)
    shift || true
    .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E005.py "$@" | tee "$LOGS/eval_E005.log"
    ;;
  *)
    echo "Usage:"
    echo "  $0 smoke 0"
    ...
    echo "  $0 one 0 E005_box025_p2_yneg_s20"
    echo "  $0 eval [variant ...]"
    exit 2
    ;;
esac
