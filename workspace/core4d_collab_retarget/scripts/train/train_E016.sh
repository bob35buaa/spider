#!/usr/bin/env bash
# E016: E014-B multi-case generalization.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

MODE="${1:-quick}"
GPU="${2:-0}"
VARIANTS_FILE="${VARIANTS_FILE:-workspace/core4d_collab_retarget/results/E016/manifest.tsv}"
RESULTS="${RESULTS:-workspace/core4d_collab_retarget/results/E016}"
LOGS="${LOGS:-logs/core4d_collab_retarget/E016}"
...
snapshot_task() {
  ...
  cp "$src"/scene_e016_jointB_*.xml "$dst/" 2>/dev/null || true
...
run_one() {
  ...
  row="$(variant_row "$variant")"
  if [ -z "$row" ]; then
    echo "Unknown E016 variant: $variant" >&2
    return 2
  fi
  task="$(field_from_row "$row" 3)"
...
run_variants() {
  ...
  if [ "${#variants[@]}" -eq 0 ]; then
    echo "No E016 variants selected" >&2
    exit 2
  fi
  local status=0
  ...
  for variant in "${variants[@]}"; do
    if ! run_one "$variant" "$gpu" "${RUN_EXTRA_ARGS[@]}"; then
      echo "[$(date '+%H:%M:%S')] WARN ${variant} failed; continuing E016 queue" | tee -a "$LOGS/failed_${MODE}.log" >&2
      status=1
    fi
  done
  if [ "${SKIP_EVAL:-0}" != "1" ]; then
    .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E016.py "${variants[@]}" | tee "$LOGS/eval_${MODE}.log"
  fi
  return "$status"
}
...
case "$MODE" in
  __codex_auth_probe__)
    echo "E016 train entrypoint authorized."
    ;;
  smoke)
  ...
  quick)
    mapfile -t variants < <(all_variants)
    RUN_EXTRA_ARGS=(
      num_samples="${E016_QUICK_NUM_SAMPLES:-128}"
      max_num_iterations="${E016_QUICK_MAX_ITERS:-4}"
      save_video=false
      viewer=none
    )
  ...
  one|single)
    ...
    run_one "$VARIANT" "$GPU"
    .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E016.py "$VARIANT" | tee "$LOGS/eval_${VARIANT}.log"
    ;;
  eval)
    shift || true
    .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E016.py "$@" | tee "$LOGS/eval_E016.log"
    ;;
  *)
    echo "Unknown E016 mode: $MODE" >&2
    exit 2
    ;;
esac
