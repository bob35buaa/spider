#!/usr/bin/env bash
# E018: canonical support-proxy anchor GT gate.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

MODE="${1:-local_gt}"
GPU="${2:-0}"
VARIANTS_FILE="${VARIANTS_FILE:-workspace/core4d_collab_retarget/results/E018/manifest.tsv}"
RESULTS="${RESULTS:-workspace/core4d_collab_retarget/results/E018}"
LOGS="${LOGS:-logs/core4d_collab_retarget/E018}"
...
snapshot_variants() {
  ...
  echo "[$(date '+%H:%M:%S')] === E018 scene snapshot tasks=${tasks[*]} ==="
  bash workspace/core4d_collab_retarget/scripts/convert/snapshot_scenes.sh E018 "${tasks[@]}"
}
...
run_one() {
  ...
  row="$(variant_row "$variant")"
  if [ -z "$row" ]; then
    echo "Unknown E018 variant: $variant" >&2
    return 2
  fi
  task="$(field_from_row "$row" 3)"
...
run_variants() {
  ...
  local -a variants=("$@")
  if [ "${#variants[@]}" -eq 0 ]; then
    echo "No E018 variants selected" >&2
    exit 2
  fi
  local status=0
  ...
  for variant in "${variants[@]}"; do
    if ! run_one "$variant" "$gpu" "${RUN_EXTRA_ARGS[@]}"; then
      echo "[$(date '+%H:%M:%S')] WARN ${variant} failed; continuing E018 queue" | tee -a "$LOGS/failed_${MODE}.log" >&2
      status=1
    fi
  done
  if [ "${SKIP_EVAL:-0}" != "1" ]; then
    .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E018.py "${variants[@]}" | tee "$LOGS/eval_${MODE}.log"
  fi
  return "$status"
}
...
case "$MODE" in
  __codex_auth_probe__)
    echo "E018 train entrypoint authorized."
    ;;
  smoke)
    ...
    ;;
  local_gt|full|local|local_wave)
  ...
  one|single)
    ...
    run_one "$VARIANT" "$GPU"
    .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E018.py "$VARIANT" | tee "$LOGS/eval_${VARIANT}.log"
    ;;
  eval)
    shift || true
    .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E018.py "$@" | tee "$LOGS/eval_E018.log"
    ;;
  *)
    echo "Usage:"
    echo "  $0 smoke 0"
    echo "  $0 local_gt 0"
    echo "  $0 one 0 E018_box023_p2_canonical_t02"
    echo "  $0 eval [variant ...]"
    exit 2
    ;;
esac
