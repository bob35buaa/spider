#!/usr/bin/env bash
# E029: bucket001 stability / posture-valid contact.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

MODE="${1:-local}"
GPU="${2:-0}"
VARIANTS_FILE="${VARIANTS_FILE:-workspace/core4d_collab_retarget/results/E029/manifest.tsv}"
RESULTS="${RESULTS:-workspace/core4d_collab_retarget/results/E029}"
...
snapshot_variants() {
  ...
  echo "[$(date '+%H:%M:%S')] === E029 scene snapshot tasks=${tasks[*]} ==="
  bash workspace/core4d_collab_retarget/scripts/convert/snapshot_scenes.sh E029 "${tasks[@]}"
}
...
run_variants() {
  ...
  if [ "${SKIP_EVAL:-0}" != "1" ]; then
    .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E029.py "${variants[@]}" | tee "$LOGS/eval_${MODE}.log"
  fi
  return "$status"
}
...
case "$MODE" in
  ...
  one|single)
    ...
    run_one "$VARIANT" "$GPU"
    if [ "${SKIP_EVAL:-0}" != "1" ]; then
      .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E029.py "$VARIANT" | tee "$LOGS/eval_${VARIANT}.log"
    fi
    ;;
  eval)
    shift || true
    .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E029.py "$@" | tee "$LOGS/eval_E029.log"
    ;;
  *)
    echo "Usage:"
    echo "  $0 smoke 0"
    ...
    echo "  $0 one 0 E029_bucket001_p1_upright_barrier_t055"
    echo "  $0 eval [variant ...]"
    exit 2
    ;;
esac
