...
MODE="${1:-full}"
GPU="${2:-0}"
VARIANTS_FILE="${VARIANTS_FILE:-workspace/core4d_collab_retarget/scripts/E013/variants.tsv}"
RESULTS="${RESULTS:-workspace/core4d_collab_retarget/results/E013}"
LOGS="${LOGS:-logs/core4d_collab_retarget/E013}"
...
snapshot_variants() {
  ...
  if [ "${#variants[@]}" -gt 0 ]; then
    echo "[$(date '+%H:%M:%S')] === E013 scene snapshot variants=${variants[*]} ==="
...
extract_keyframes() {
  ...
  for f in 32 50 75 100 115 130 160 180 204; do
...
run_one() {
  ...
  shift 2
  local extra_args=("$@")
  local row task override out_dir npz_out variant_log
  row="$(variant_row "$variant")"
  if [ -z "$row" ]; then
    echo "Unknown E013 variant: $variant" >&2
    return 2
  fi
  task="$(field_from_row "$row" 2)"
...
run_variants() {
  ...
  if [ "${#variants[@]}" -eq 0 ]; then
    echo "No E013 variants selected" >&2
    exit 2
  fi
  ...
  if [ "${SKIP_EVAL:-0}" != "1" ]; then
    .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E013.py "${variants[@]}" | tee "$LOGS/eval_${MODE}.log"
  fi
}

case "$MODE" in
  __codex_auth_probe__)
    echo "E013 train entrypoint authorized."
    ;;
  smoke)
  ...
  one|single)
    ...
    run_one "$VARIANT" "$GPU"
    .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E013.py "$VARIANT" | tee "$LOGS/eval_${VARIANT}.log"
    ;;
  eval)
    shift || true
    .venv/bin/python workspace/core4d_collab_retarget/scripts/eval/eval_E013.py "$@" | tee "$LOGS/eval_E013.log"
    ;;
  *)
    echo "Usage:"
    echo "  $0 smoke 0"
    echo "  $0 full 0"
    echo "  $0 one 0 E013_box025_p2_obj_oracle"
    echo "  $0 eval [variant ...]"
    exit 2
    ;;
esac
