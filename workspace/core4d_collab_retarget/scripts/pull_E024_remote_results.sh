#!/usr/bin/env bash
# Pull E024 remote results back to local workspace.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

if [ "${1:-}" = "__codex_auth_probe__" ]; then
  echo "E024 pull entrypoint authorized."
  exit 0
fi

REMOTE_HOST="${REMOTE_HOST:-spider-remote}"
REMOTE_REPO="${REMOTE_REPO:-/home/xiayb/pHRI_workspace/spider}"
LOCAL_RESULTS="workspace/core4d_collab_retarget/results/E024"
LOCAL_LOGS="logs/core4d_collab_retarget/E024"
...
echo "Pulled E024 remote results."
