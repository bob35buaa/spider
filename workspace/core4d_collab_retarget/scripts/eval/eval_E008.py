#!/usr/bin/env python3
"""E008 evaluation wrapper for COLA-style support-body proxy sweep."""
...
RESULTS = REPO / "workspace/core4d_collab_retarget/results/E008"
VARIANTS_FILE = REPO / "workspace/core4d_collab_retarget/scripts/E008/variants.tsv"
...
def _add_support_proxy_metrics(
    ...
    summary["E008_point_local"] = point_local.tolist()
    summary["E008_support_proxy_metrics_present"] = bool(
    ...
    if qpos is not None and qpos.shape[1] >= 7 and qpos_ref.shape[1] >= 7:
        ...
        summary["E008_object_xy_disp_m"] = obj_xy
        summary["E008_ref_object_xy_disp_m"] = ref_xy
        summary["E008_ref_support_xy_disp_m"] = ref_support_xy
        summary["E008_object_xy_disp_ratio"] = float(obj_xy / max(ref_xy, 1e-8))
        summary["E008_object_xy_disp_ratio_vs_ref_support"] = float(
            obj_xy / max(ref_support_xy, 1e-8)
        )
        summary["E008_object_rot_deg"] = _quat_angle_deg(obj_quat[0], obj_quat[-1])
        summary["E008_ref_object_rot_deg"] = _quat_angle_deg(
        ...
        summary["E008_case_window_object_xy_disp_m"] = cw_obj_xy
        summary["E008_case_window_ref_object_xy_disp_m"] = cw_ref_xy
        summary["E008_case_window_object_xy_disp_ratio"] = float(
        ...
        summary["E008_case_window_end_height_diff_mean_m"] = float(
            cw_end_height.mean()
        )
        summary["E008_case_window_end_height_diff_max_m"] = float(cw_end_height.max())

    if not summary["E008_support_proxy_metrics_present"]:
        return
    ...
    if proxy_pos is not None and qpos is not None and qpos_ref.shape[1] >= 7:
        ...
        summary["E008_proxy_xy_disp_m"] = proxy_xy
        summary["E008_proxy_xy_disp_ratio_vs_ref_obj"] = float(
            proxy_xy / max(ref_xy, 1e-8)
        )
        summary["E008_proxy_xy_disp_ratio_vs_ref_support"] = float(
            proxy_xy / max(ref_support_xy, 1e-8)
        )
        summary["E008_proxy_final_gap_to_ref_support_m"] = final_gap
        summary["E008_proxy_support_tracking_ok"] = bool(
            summary["E008_proxy_xy_disp_ratio_vs_ref_support"] >= 0.95
            and final_gap <= 0.08
        )
        summary["E008_proxy_timebase_ok"] = summary["E008_proxy_support_tracking_ok"]
    ...
    for name, arr in [
        ...
        summary[f"E008_case_window_{name}_mean"] = float(cw.mean())
        summary[f"E008_case_window_{name}_max"] = float(cw.max())
...
def evaluate_variant(
    ...
    if variant not in variants:
        raise ValueError(f"Unknown E008 variant: {variant}")
    ...
    summary["E008_wave"] = meta["wave"]
    summary["E008_queue"] = meta["queue"]
    summary["E008_support_proxy_gravity_scale"] = float(
        meta["support_proxy_gravity_scale"]
    )
    summary["E008_support_proxy_connector_kp"] = float(
        meta["support_proxy_connector_kp"]
    )
    summary["E008_support_proxy_connector_kd"] = float(
        meta["support_proxy_connector_kd"]
    )
    summary["E008_support_proxy_xy_velocity_scale"] = float(
        meta["support_proxy_xy_velocity_scale"]
    )
    summary["E008_support_proxy_max_xy_speed"] = float(
        meta["support_proxy_max_xy_speed"]
    )
    summary["E008_support_proxy_height_tau"] = float(meta["support_proxy_height_tau"])
    summary["E008_support_proxy_ref_dt"] = float(meta["support_proxy_ref_dt"])
    summary["E008_support_proxy_force_clamp"] = float(
        meta["support_proxy_force_clamp"]
    )
    summary["E008_support_proxy_torque_clamp"] = float(
        meta["support_proxy_torque_clamp"]
    )
    summary["E008_hold_contact_rew_scale"] = float(meta["hold_contact_rew_scale"])
    summary["E008_freejoint_parity_ok"] = bool(
    ...
    summary["E008_e081_baseline_variant"] = baseline["variant"]
    summary["E008_vs_E081_obj_mean_delta_m"] = float(
        summary["case_window_obj_err_mean_m"] - baseline["obj_mean"]
    )
    summary["E008_vs_E081_obj_max_delta_m"] = float(
        summary["case_window_obj_err_max_m"] - baseline["obj_max"]
    )
    summary["E008_vs_E081_hand_contact_delta_pp"] = float(
        summary["case_window_sim_contact_frames_pct"] - baseline["hand_pct"]
    )
    summary["E008_vs_E081_floor_contact_delta_pp"] = float(
        summary["case_window_sim_object_floor_contact_frames_pct"]
        - baseline["floor_pct"]
    )
    summary["E008_vs_E081_leg_intf_delta_pp"] = float(
        summary["case_window_sim_leg_box_interference_frames_pct"]
        - baseline["leg_intf_pct"]
    )
    summary["E008_vs_E081_bottom_mean_delta_m"] = float(
    ...
    majority_checks = {
        ...
        "xy_transport": summary.get("E008_object_xy_disp_ratio", 0.0) >= 0.75,
    }
    summary["E008_e081_majority_score"] = int(sum(majority_checks.values()))
    summary["E008_e081_majority_checks"] = json.dumps(
        majority_checks, sort_keys=True
    )
    summary["E008_beats_or_matches_E081_majority"] = bool(
        summary["role"] == "main" and summary["E008_e081_majority_score"] >= 4
    )

    effort_ok = True
    if summary["E008_support_proxy_metrics_present"]:
        effort_ok = bool(
            summary["E008_case_window_force_norm_n_mean"] < 80.0
            and summary["E008_case_window_force_norm_n_max"] < 160.0
            and summary["E008_case_window_torque_norm_nm_max"] < 25.0
        )
    summary["E008_effort_reasonable_proxy"] = effort_ok
    summary["E008_reaches_E081_transport_proxy"] = bool(
        summary["role"] == "main"
        and summary.get("E008_proxy_support_tracking_ok", False)
        ...
        and summary.get("E008_object_xy_disp_ratio", 0.0) >= 0.75
        and summary.get("E008_object_rot_deg", 180.0) <= 15.0
        and effort_ok
        and summary["E008_freejoint_parity_ok"]
    )
    summary["E008_guard_stable_proxy"] = bool(
        summary["role"] == "guard"
...
def main() -> None:
    ...
    if not summaries:
        raise SystemExit("No E008 variant results found.")
    ...
    aggregate = {
        ...
        "num_freejoint_parity_ok": sum(
            bool(r["E008_freejoint_parity_ok"]) for r in summaries
        ),
        "num_support_proxy_metrics_present": sum(
            bool(r["E008_support_proxy_metrics_present"]) for r in summaries
        ),
        "num_main_proxy_timebase_ok": sum(
            bool(r.get("E008_proxy_timebase_ok", False)) for r in main_rows
        ),
        "num_main_proxy_support_tracking_ok": sum(
            bool(r.get("E008_proxy_support_tracking_ok", False)) for r in main_rows
        ),
        "num_main_reaches_E081_transport_proxy": sum(
            bool(r["E008_reaches_E081_transport_proxy"]) for r in main_rows
        ),
        "num_main_beats_or_matches_E081_majority": sum(
            bool(r["E008_beats_or_matches_E081_majority"]) for r in main_rows
        ),
        "num_guard_stable_proxy": sum(
            bool(r["E008_guard_stable_proxy"]) for r in guard_rows
        ),
