#!/usr/bin/env python3
"""Index E028 online rollout videos and create contact sheets."""
...
def main() -> None:
    ...
    lines = [
        "# E028 Online Video Evaluation",
        "",
        "Videos are direct `run_mjwp.py` online rollout outputs. No offline qpos replay is used.",
        "",
        "| Variant | Queue | Face | Anchor Review | Video | Sheet | ffprobe | Epos m | Erot deg | Contact 5cm % | Deep pen % | Leg % | Fall | Diagnosis |",
        "|---------|-------|------|---------------|-------|-------|---------|--------|----------|--------------|------------|-------|------|-----------|",
    ]
    for variant in selected:
        ...
        if not video.is_file():
            lines.append(
                f"| `{variant}` | {manifest[variant]['queue']} | {manifest[variant]['anchor_face']} | "
                f"{manifest[variant]['anchor_face_review']} | missing | missing |  |  |  |  |  |  |  | missing_video |"
            )
            continue
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    ...
                    row.get("E028_robot_fall_detected", ""),
                    row.get("E028_diagnostic_class", ""),
                ]
            )
            + " |"
        )
