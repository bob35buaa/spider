#!/usr/bin/env python3
"""Index E018b online rollout videos and create contact sheets.

This script deliberately does not replay qpos offline. It only uses the mp4
files emitted directly by examples/run_mjwp.py during optimization.
"""
...
def main() -> None:
    ...
    lines = [
        "# E018b Online Video Evaluation",
        "",
        "Videos are direct `run_mjwp.py` online rollout outputs. No offline qpos replay is used.",
        "",
        "| Variant | Queue | Face | Video | Sheet | ffprobe | Epos m | Erot deg | Contact 5cm % | Deep pen % | Leg % | Fall | Pelvis min | Diagnosis |",
        "|---------|-------|------|-------|-------|---------|--------|----------|--------------|------------|-------|------|------------|-----------|",
    ]
    for variant in selected:
        ...
        if not video.is_file():
            lines.append(f"| `{variant}` | {manifest[variant]['queue']} | {manifest[variant]['anchor_face']} | missing | missing |  |  |  |  |  |  |  |  | missing_video |")
            continue
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    ...
                    row.get("E018b_robot_fall_detected", ""),
                    fmt(row, "full_pelvis_z_min_m"),
                    row.get("E018b_diagnostic_class", ""),
                ]
            )
            + " |"
        )
    ONLINE_DIR.mkdir(parents=True, exist_ok=True)
    (ONLINE_DIR / "online_video_eval.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {ONLINE_DIR / 'online_video_eval.md'}")
