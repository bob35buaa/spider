#!/usr/bin/env python3
"""E028b evaluation for D003 Box021 contact-centroid anchor refit."""
...
def _half(row: dict[str, str]) -> np.ndarray:
    return np.array(
        [
...
def _surface_anchor_ok(row: dict[str, str], point: np.ndarray) -> bool:
    ...
    if abs(point[axis] - sign * half[axis]) > 1e-5:
        return False
    for idx in range(3):
        if idx == axis:
            continue
        if abs(point[idx]) > 0.9001 * half[idx] + 1e-6:
            return False
    return bool(np.all(np.isfinite(point)) and np.linalg.norm(point) > 0.05)


def _scene_anchor_ok(row: dict[str, str]) -> bool:
    scene = BASE / row["derived_task"] / f"{row['scene_name']}.xml"
    if not scene.is_file():
        return False
    text = scene.read_text(encoding="utf-8")
    return bool(
        "object_target" not in text
...
def evaluate_variant(variant: str, manifest: dict[str, dict[str, str]]) -> dict[str, Any]:
    ...
    freejoint_parity_ok = bool(
        ...
        and str(summary["config_scene_name"]).startswith("scene_e028b_jointB_")
    ...
    surface_anchor_pass = _surface_anchor_ok(row, point)
    scene_anchor_pass = _scene_anchor_ok(row)
    ...
    old_dist = _as_float(row.get("old_anchor_to_selected_face_centroid_m"))
    new_dist = _as_float(row.get("anchor_to_selected_face_centroid_m"))
    distance_ratio = _as_float(row.get("anchor_distance_ratio_vs_e028"))

    summary["E028b_queue"] = row["queue"]
    summary["E028b_role"] = row["role"]
    summary["E028b_wave"] = row["wave"]
    summary["E028b_source_task"] = row["source_task"]
    summary["E028b_source_e028_variant"] = row["source_e028_variant"]
    summary["E028b_derived_task"] = row["derived_task"]
    summary["E028b_scene_name"] = row["scene_name"]
    summary["E028b_point_local"] = point.tolist()
    summary["E028b_anchor_policy"] = row["anchor_policy"]
    summary["E028b_anchor_face"] = row["anchor_face"]
    summary["E028b_anchor_face_source"] = row["anchor_face_source"]
    summary["E028b_anchor_face_review"] = row["anchor_face_review"].lower() == "true"
    summary["E028b_anchor_face_review_reason"] = row["anchor_face_review_reason"]
    summary["E028b_anchor_top_face"] = row["anchor_top_face"]
    summary["E028b_anchor_side_face_frac"] = _as_float(row["anchor_side_face_frac"])
    summary["E028b_anchor_z_face_frac"] = _as_float(row["anchor_z_face_frac"])
    summary["E028b_anchor_side_margin"] = _as_float(row["anchor_side_margin"])
    summary["E028b_contact_points_used"] = int(row["contact_points_used"])
    summary["E028b_selected_face_points_used"] = int(row["selected_face_points_used"])
    summary["E028b_old_anchor_to_selected_face_centroid_m"] = old_dist
    summary["E028b_anchor_to_selected_face_centroid_m"] = new_dist
    summary["E028b_anchor_distance_ratio_vs_e028"] = distance_ratio
    summary["E028b_anchor_distance_improved"] = bool(np.isfinite(distance_ratio) and distance_ratio < 1.0)
    summary["E028b_anchor_refit_fallback"] = row["anchor_refit_fallback"].lower() == "true"
    summary["E028b_freejoint_parity_ok"] = freejoint_parity_ok
    summary["E028b_no_direct_wrench"] = no_direct_wrench
    summary["E028b_anchor_not_com_oracle"] = scene_anchor_pass
    summary["E028b_surface_anchor_pass"] = surface_anchor_pass
    summary["E028b_config_ok"] = config_ok
    summary["E028b_robot_fall_detected"] = robot_fall
    summary["E028b_robot_upright_ok"] = not robot_fall
    summary["E028b_artifact_ok"] = artifact_ok
    summary["E028b_floor_leg_ok"] = floor_leg_ok
    summary["E028b_object_transport_pass"] = bool(
        summary.get("paper_dynaretarget_object_success", False)
        and summary.get("paper_transport_success", False)
    )
    summary["E028b_generalization_pass"] = bool(
        config_ok
        and surface_anchor_pass
        and summary["E028b_anchor_distance_improved"]
        and summary["E028b_object_transport_pass"]
        and not robot_fall
        and artifact_ok
        and floor_leg_ok
        and not summary["E028b_anchor_face_review"]
    )

    if not config_ok:
        diag = "config_failed"
    elif not surface_anchor_pass:
        diag = "surface_anchor_failed"
    elif not summary["E028b_anchor_distance_improved"]:
        diag = "anchor_refit_not_improved"
    elif summary["E028b_anchor_face_review"]:
        diag = "anchor_face_or_edge_review"
    ...
    summary["E028b_diagnostic_class"] = diag
...
def main() -> None:
    ...
    for variant in selected:
        ...
        try:
            ...
            continue
    if not summaries:
        raise SystemExit("No E028b variant results found.")
    ...
    aggregate = {
        ...
        "num_config_ok": sum(bool(r["E028b_config_ok"]) for r in summaries),
        "num_surface_anchor_pass": sum(bool(r["E028b_surface_anchor_pass"]) for r in summaries),
        "num_anchor_distance_improved": sum(bool(r["E028b_anchor_distance_improved"]) for r in summaries),
        "num_anchor_face_review": sum(bool(r["E028b_anchor_face_review"]) for r in summaries),
        ...
        "num_object_transport_pass": sum(bool(r["E028b_object_transport_pass"]) for r in summaries),
        "num_contact_preservation_ok": sum(
            _as_float(r.get("paper_omniretarget_contact_preservation_5cm_pct"), 0.0) >= 50.0
            for r in summaries
        ),
        "num_robot_upright_ok": sum(bool(r["E028b_robot_upright_ok"]) for r in summaries),
        "num_robot_fall_detected": sum(bool(r["E028b_robot_fall_detected"]) for r in summaries),
        "num_artifact_ok": sum(bool(r["E028b_artifact_ok"]) for r in summaries),
        "num_floor_leg_ok": sum(bool(r["E028b_floor_leg_ok"]) for r in summaries),
        "num_generalization_pass": sum(bool(r["E028b_generalization_pass"]) for r in summaries),
        "mean_anchor_distance_ratio_vs_e028": float(
            np.mean([_as_float(r["E028b_anchor_distance_ratio_vs_e028"]) for r in summaries])
        ),
        ...
        "diagnostic_classes": {
            name: sum(str(r["E028b_diagnostic_class"]) == name for r in summaries)
            for name in sorted({str(r["E028b_diagnostic_class"]) for r in summaries})
        },
