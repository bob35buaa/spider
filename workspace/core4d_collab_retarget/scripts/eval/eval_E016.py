#!/usr/bin/env python3
"""E016 evaluation: E014-B paper-aligned multi-case generalization."""
...
def evaluate_variant(variant: str, variants: dict[str, dict[str, str]]) -> dict[str, object]:
    ...
    summary["E016_queue"] = meta["queue"]
    summary["E016_wave"] = meta["wave"]
    summary["E016_scene_name"] = meta["scene_name"]
    summary["E016_point_local"] = _point_local(meta).tolist()
    summary["E016_support_point_method"] = meta["support_point_method"]
    summary["E016_freejoint_parity_ok"] = bool(
        not summary["config_contact_guidance"]
        and str(summary["config_scene_name"]).startswith("scene_e016_jointB_")
    ...
    summary["E016_anchor_not_com_oracle"] = _scene_anchor_not_oracle(meta)
    summary["E016_no_direct_wrench"] = bool(
        float(cfg.partner_force_scale) == 0.0
        and float(cfg.partner_force_spring_kp) == 0.0
        and float(cfg.support_proxy_connector_kp) == 0.0
    )
    summary["E016_config_ok"] = bool(
        summary["E016_freejoint_parity_ok"]
        and summary["E016_anchor_not_com_oracle"]
        and summary["E016_no_direct_wrench"]
    )
    ...
    summary["E016_artifact_ok"] = artifact_ok
    summary["E016_floor_leg_ok"] = floor_leg_ok
    summary["E016_generalization_pass"] = bool(
        summary["E016_config_ok"]
    ...
    summary["E016_diagnostic_class"] = diag
...
def main() -> None:
    ...
    for variant in selected:
        ...
        try:
            ...
            continue
    if not summaries:
        raise SystemExit("No E016 variant results found.")
    ...
    aggregate = {
        ...
        "num_config_ok": sum(bool(r["E016_config_ok"]) for r in summaries),
        ...
        "num_artifact_ok": sum(bool(r["E016_artifact_ok"]) for r in summaries),
        "num_generalization_pass": sum(bool(r["E016_generalization_pass"]) for r in summaries),
        ...
        "diagnostic_classes": {
            name: sum(r["E016_diagnostic_class"] == name for r in summaries)
            for name in sorted({str(r["E016_diagnostic_class"]) for r in summaries})
        },
