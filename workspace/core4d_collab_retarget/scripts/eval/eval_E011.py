#!/usr/bin/env python3
"""E011 evaluation wrapper for soft object tether diagnostics."""
...
RESULTS = REPO / "workspace/core4d_collab_retarget/results/E011"
VARIANTS_FILE = REPO / "workspace/core4d_collab_retarget/scripts/E011/variants.tsv"
...
def _add_support_proxy_metrics(
    ...
    summary["E011_point_local"] = point_local.tolist()
    summary["E011_support_proxy_metrics_present"] = bool(
    ...
    if qpos is not None and qpos.shape[1] >= 7 and qpos_ref.shape[1] >= 7:
        ...
        summary["E011_object_xy_disp_m"] = obj_xy
        summary["E011_ref_object_xy_disp_m"] = ref_xy
        summary["E011_ref_support_xy_disp_m"] = ref_support_xy
        summary["E011_object_xy_disp_ratio"] = float(obj_xy / max(ref_xy, 1e-8))
        summary["E011_object_xy_disp_ratio_vs_ref_support"] = float(
            obj_xy / max(ref_support_xy, 1e-8)
        )
        summary["E011_object_rot_deg"] = _quat_angle_deg(obj_quat[0], obj_quat[-1])
        summary["E011_ref_object_rot_deg"] = _quat_angle_deg(
        ...
        summary["E011_case_window_object_xy_disp_m"] = cw_obj_xy
        summary["E011_case_window_ref_object_xy_disp_m"] = cw_ref_xy
        summary["E011_case_window_object_xy_disp_ratio"] = float(
        ...
        summary["E011_case_window_end_height_diff_mean_m"] = float(
            cw_end_height.mean()
        )
        summary["E011_case_window_end_height_diff_max_m"] = float(cw_end_height.max())

    if not summary["E011_support_proxy_metrics_present"]:
        return
    ...
    if proxy_pos is not None and qpos is not None and qpos_ref.shape[1] >= 7:
        ...
        summary["E011_proxy_xy_disp_m"] = proxy_xy
        summary["E011_proxy_xy_disp_ratio_vs_ref_obj"] = float(
            proxy_xy / max(ref_xy, 1e-8)
        )
        summary["E011_proxy_xy_disp_ratio_vs_ref_support"] = float(
            proxy_xy / max(ref_support_xy, 1e-8)
        )
        summary["E011_proxy_final_gap_to_ref_support_m"] = final_gap
        summary["E011_proxy_support_tracking_ok"] = bool(
            summary["E011_proxy_xy_disp_ratio_vs_ref_support"] >= 0.95
            and final_gap <= 0.08
        )
        summary["E011_proxy_timebase_ok"] = summary["E011_proxy_support_tracking_ok"]
    ...
    for name, arr in [
        ...
        summary[f"E011_case_window_{name}_mean"] = float(cw.mean())
        summary[f"E011_case_window_{name}_max"] = float(cw.max())


def _add_partner_force_metrics(summary: dict[str, object], npz_path: Path) -> None:
    data = np.load(npz_path, allow_pickle=True)
    force = _flatten_npz(data, "partner_force_force")
    torque = _flatten_npz(data, "partner_force_torque")
    summary["E011_partner_force_metrics_present"] = bool(
        force is not None and torque is not None
    )
    if not summary["E011_partner_force_metrics_present"]:
        return
    ...
    torque_norm = np.linalg.norm(torque, axis=1)
    for name, arr in [
        ("partner_force_norm_n", force_norm),
        ("partner_force_horizontal_n", force_h),
        ("partner_force_z_n", force_z),
        ("partner_torque_norm_nm", torque_norm),
    ]:
        cw = _window(arr, summary)
        summary[f"E011_case_window_{name}_mean"] = float(cw.mean())
        summary[f"E011_case_window_{name}_max"] = float(cw.max())
...
def evaluate_variant(
    ...
    if variant not in variants:
        raise ValueError(f"Unknown E011 variant: {variant}")
    ...
    _add_partner_force_metrics(summary, npz_path)

    summary["E011_wave"] = meta["wave"]
    summary["E011_queue"] = meta["queue"]
    summary["E011_partner_force_scale"] = float(meta["partner_force_scale"])
    summary["E011_partner_force_spring_kp"] = float(
        meta["partner_force_spring_kp"]
    )
    summary["E011_partner_force_spring_kp_rot"] = float(
        meta["partner_force_spring_kp_rot"]
    )
    summary["E011_partner_force_ref_dt"] = float(meta["partner_force_ref_dt"])
    summary["E011_partner_force_force_clamp"] = float(
        meta["partner_force_force_clamp"]
    )
    summary["E011_partner_force_torque_clamp"] = float(
        meta["partner_force_torque_clamp"]
    )
    summary["E011_support_proxy_enabled"] = bool(cfg.support_proxy_enabled)
    summary["E011_support_proxy_mode"] = str(cfg.support_proxy_mode)
    summary["E011_support_proxy_gravity_scale"] = float(cfg.support_proxy_gravity_scale)
    summary["E011_support_proxy_connector_kp"] = float(cfg.support_proxy_connector_kp)
    summary["E011_support_proxy_connector_kd"] = float(cfg.support_proxy_connector_kd)
    summary["E011_support_proxy_xy_velocity_scale"] = float(
        cfg.support_proxy_xy_velocity_scale
    )
    summary["E011_support_proxy_max_xy_speed"] = float(
        meta["support_proxy_max_xy_speed"]
    )
    summary["E011_support_proxy_height_tau"] = float(cfg.support_proxy_height_tau)
    summary["E011_support_proxy_ref_dt"] = float(meta["support_proxy_ref_dt"])
    summary["E011_support_proxy_force_clamp"] = float(cfg.support_proxy_force_clamp)
    summary["E011_support_proxy_torque_clamp"] = float(cfg.support_proxy_torque_clamp)
    summary["E011_hold_contact_rew_scale"] = float(meta["hold_contact_rew_scale"])
    summary["E011_hold_contact_start_eval_time"] = float(
        meta["hold_contact_start_eval_time"]
    )
    summary["E011_hold_contact_end_eval_time"] = float(
        meta["hold_contact_end_eval_time"]
    )
    expected_support_enabled = str(meta["support_proxy_enabled"]).lower() == "true"
    summary["E011_freejoint_parity_ok"] = bool(
    ...
    majority_checks = {
        ...
        "xy_transport": summary.get("E011_object_xy_disp_ratio", 0.0) >= 0.75,
    }
    summary["E011_e081_majority_score"] = int(sum(majority_checks.values()))
    summary["E011_e081_majority_checks"] = json.dumps(
        majority_checks, sort_keys=True
    )
    summary["E011_beats_or_matches_E081_majority"] = bool(
        summary["role"] == "main" and summary["E011_e081_majority_score"] >= 4
    )

    if summary["role"] == "main":
        summary["E011_e008_best_variant"] = E008_BEST_MAIN["variant"]
        summary["E011_vs_E008_best_obj_mean_delta_m"] = float(
            summary["case_window_obj_err_mean_m"] - E008_BEST_MAIN["obj_mean"]
        )
        summary["E011_vs_E008_best_obj_max_delta_m"] = float(
            summary["case_window_obj_err_max_m"] - E008_BEST_MAIN["obj_max"]
        )
        summary["E011_vs_E008_best_hand_contact_delta_pp"] = float(
            summary["case_window_sim_contact_frames_pct"] - E008_BEST_MAIN["hand_pct"]
        )
        summary["E011_vs_E008_best_floor_contact_delta_pp"] = float(
            summary["case_window_sim_object_floor_contact_frames_pct"]
            - E008_BEST_MAIN["floor_pct"]
        )
        summary["E011_vs_E008_best_leg_intf_delta_pp"] = float(
            summary["case_window_sim_leg_box_interference_frames_pct"]
            - E008_BEST_MAIN["leg_intf_pct"]
        )
        summary["E011_vs_E008_best_xy_ratio_delta"] = float(
            summary.get("E011_object_xy_disp_ratio", 0.0)
            - E008_BEST_MAIN["xy_ratio"]
        )
        summary["E011_vs_E008_best_rot_delta_deg"] = float(
            summary.get("E011_object_rot_deg", 180.0) - E008_BEST_MAIN["rot_deg"]
        )
        summary["E011_improves_E008_best"] = bool(
            summary["case_window_obj_err_mean_m"] <= E008_BEST_MAIN["obj_mean"]
            and summary["case_window_obj_err_max_m"] <= E008_BEST_MAIN["obj_max"]
            and summary["case_window_sim_contact_frames_pct"] >= 80.0
            and summary.get("E011_object_xy_disp_ratio", 0.0)
            >= E008_BEST_MAIN["xy_ratio"]
            and summary["case_window_sim_leg_box_interference_frames_pct"]
            <= E081_BASELINES["main"]["leg_intf_pct"] + 5.0
        )
    else:
        summary["E011_improves_E008_best"] = False
    ...
    if summary["E011_partner_force_metrics_present"]:
        partner_effort_ok = bool(
            ...
            and summary["E011_case_window_partner_torque_norm_nm_max"] <= 20.0
        )
    summary["E011_partner_effort_reasonable"] = partner_effort_ok

    support_effort_ok = True
    if summary["E011_support_proxy_metrics_present"]:
        support_effort_ok = bool(
            summary["E011_case_window_force_norm_n_mean"] < 80.0
            and summary["E011_case_window_force_norm_n_max"] < 160.0
            and summary["E011_case_window_torque_norm_nm_max"] < 25.0
        )
    summary["E011_support_proxy_effort_reasonable"] = support_effort_ok
    support_tracking_ok = bool(
        (not summary["E011_support_proxy_enabled"])
        or summary.get("E011_proxy_support_tracking_ok", False)
    )
    summary["E011_reaches_E081_transport"] = bool(
        summary["role"] == "main"
        and support_tracking_ok
        ...
        and support_effort_ok
        and summary["E011_freejoint_parity_ok"]
    )
    object_gate_no_hand = bool(
        summary["role"] == "main"
        and summary["case_window_obj_err_mean_m"] <= 0.20
        and summary["case_window_obj_err_max_m"] <= 0.40
        and summary.get("E011_object_xy_disp_ratio", 0.0) >= 0.75
    ...
    if summary["E011_reaches_E081_transport"]:
        diagnostic_class = "physical_candidate"
    elif object_gate_no_hand:
        ...
        diagnostic_class = "rotation_or_floor_shortcut"
    elif float(meta["partner_force_spring_kp"]) >= 100.0:
        diagnostic_class = "robot_side_blocked"
    else:
        diagnostic_class = "insufficient_coupling"
    summary["E011_diagnostic_class"] = diagnostic_class

    summary["E011_guard_stable"] = bool(
        summary["role"] == "guard"
...
def main() -> None:
    ...
    if not summaries:
        raise SystemExit("No E011 variant results found.")
    ...
    aggregate = {
        ...
        "num_freejoint_parity_ok": sum(
            bool(r["E011_freejoint_parity_ok"]) for r in summaries
        ),
        "num_support_proxy_metrics_present": sum(
            bool(r["E011_support_proxy_metrics_present"]) for r in summaries
        ),
        ...
        "num_main_proxy_timebase_ok": sum(
            bool(r.get("E011_proxy_timebase_ok", False)) for r in main_rows
        ),
        "num_main_proxy_support_tracking_ok": sum(
            bool(r.get("E011_proxy_support_tracking_ok", False)) for r in main_rows
        ),
        "num_main_reaches_E081_transport": sum(
            bool(r["E011_reaches_E081_transport"]) for r in main_rows
        ),
        "num_main_beats_or_matches_E081_majority": sum(
            bool(r["E011_beats_or_matches_E081_majority"]) for r in main_rows
        ),
        "num_main_improves_E008_best": sum(
            bool(r.get("E011_improves_E008_best", False)) for r in main_rows
        ),
        "num_guard_stable": sum(
            bool(r["E011_guard_stable"]) for r in guard_rows
        ),
        "diagnostic_classes": {
            name: sum(r.get("E011_diagnostic_class") == name for r in summaries)
            for name in sorted({str(r.get("E011_diagnostic_class")) for r in summaries})
        },
