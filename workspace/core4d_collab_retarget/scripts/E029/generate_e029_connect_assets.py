#!/usr/bin/env python3
"""Generate E029 point-connect support-body diagnostic scenes."""
...
RESULT_ROOT = common.E029_RESULTS / "connect"
...
OBJECT_CONTACT_BODY = "e029_object_support_contact"
CONNECT_NAME = "e029_support_connect"

FIELDNAMES = d6_assets.FIELDNAMES + ["support_joint_layout", "constraint_type"]
...
PROFILE = {
    ...
    "support_dynamic_rot_kp": 0.0,
    "support_dynamic_rot_kd": 0.0,
    "support_dynamic_force_clamp": 1200.0,
    "support_dynamic_torque_clamp": 0.0,
...
def build_manifest(
    *,
    ...
    for e028 in common.candidate_rows(manifest_path=manifest_path, candidates_path=candidates_path):
        ...
        variant = f"{d6_assets._variant_stem(e028['variant'])}_connect_compliant"
        derived_task = f"{e028['source_task']}_freejoint_legobj_e029connect"
        scene_name = f"scene_e029_connect_{e028['source_task']}"
        data_relpath = (
            f"workspace/core4d_collab_retarget/results/E029/connect/data/"
...
def _initial_support_pose(task_dir: Path, point_local: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    src = task_dir / "0/trajectory_kinematic.npz"
    data = np.load(src, allow_pickle=True)
    qpos0 = data["qpos"][0].astype(np.float64)
    obj_pos = qpos0[-7:-4]
    obj_quat = qpos0[-4:]
    support_pos = obj_pos + common.quat_apply(obj_quat, point_local)
    return support_pos, obj_quat
...
def generate_scene(row: dict[str, str]) -> Path:
    ...
    support_pos0, support_quat0 = _initial_support_pose(task_dir, point)
    ...
    d6_assets._remove_named_children(equality, "connect", {CONNECT_NAME})
    ...
    d6_assets._remove_named_children(object_body, "body", {OBJECT_CONTACT_BODY})
    obj_contact = ET.SubElement(
        object_body,
        "body",
        {
            "name": OBJECT_CONTACT_BODY,
            "pos": f"{point[0]:.8g} {point[1]:.8g} {point[2]:.8g}",
        },
    )
    ET.SubElement(obj_contact, "site", {"name": "trace_e029_object_support_contact", "size": "0.018"})
    ...
    ET.SubElement(
        ...
        "connect",
        {
            "name": CONNECT_NAME,
            "body1": SUPPORT_BODY,
            "body2": OBJECT_CONTACT_BODY,
            "anchor": f"{support_pos0[0]:.8g} {support_pos0[1]:.8g} {support_pos0[2]:.8g}",
    ...
    connect_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_EQUALITY, CONNECT_NAME)
    contact_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, OBJECT_CONTACT_BODY)
    if min(support_id, object_id, connect_id, contact_id) < 0:
        raise ValueError(f"{out} missing support/object/connect")
