...
| E021 | 2026-05-20/21 | RL Export | **Holosoma RL export**: 按 E026 best dynamic selection `spider_best_E018b_E022_E025` 导出 13/13 Spider 动力学轨迹到 Holosoma RL 格式；最终输出使用原始 CORE4D 前缀命名，目录为 `/home/ubuntu/Workspace/holosoma/workspace/data/spider_best_E018b_E022_E025_for_rl_rename/`，load-level 验证 13/13 通过 | ✅ 详见 log 28 |
...
| E029 | 2026-05-27 | COLA Support Body Redesign | **COLA-style support body redesign sanity**: phase-1 确认 E018/E028 是 `mocap_pad` kinematic anchor 且 Box021 高度轴为 local `y`；scalar D6 compliant 5-case `1/5`，freejoint full-weld compliant `1/5`、locked raw `3/5`，rot gain 会触发 `QACC huge`。point-connect 初始 anchor residual 正常但 finite-force `0/5`、drift `0.3-1.0m`。multi-connect bounded 6D approximation 生成 5 candidates × 4 profiles，best `multi_cross_ultra` 仍 `0/5`。新增 baseline modes 与 `eval_E029.py`，扫描 30 个 candidate sanity summaries，best gate-eligible overall 仍只有 `3/5`，full gate=`stop_before_full_sanity_failed`。best `d6_locked raw` 的 5-case videos 已补齐。结论：不跑 full；已不是单纯数据预处理问题，而是 MuJoCo equality scaffolds 的 support-object load-path 语义不足 | ⚠️ sanity stopped，详见 log 30/31/32/33/34/35/36 |
...
| E029 phase-1/2/freejoint/connect/multiconnect/eval | Audit: 5/5 E028 candidates=`mocap_pad` + mocap `support_weld_anchor` + equality weld、0/5 dynamic support body、`support_proxy_force` max 0。Preflight: Box021 5/5 local height axis=`y`。Scalar D6 assets compiled `49/47/29`；candidates compliant `1/5`，locked raw `3/5`，rot gain 会 `QACC huge`。Freejoint assets compiled `50/47/29`；`mj_applyFT` 后 representative free compliant pass (`object 0.116/0.251m`)，但 candidates free compliant `1/5`、free locked raw `3/5`。Point-connect initial support/contact residual `1e-16m` but finite-force `0/5`。Multi-connect bounded 6D assets 20 rows；best `multi_cross_ultra` candidates `0/5`，representative object `0.106/0.228m`、drift `0.062/0.152m`。Baseline modes 已补：mocap current `0/5`、direct object wrench `0/5`，均 non-gate-eligible。`eval_E029.py --all` 扫描 30 个 candidate sanity summaries，best gate-eligible `3/5`；best `d6_locked raw` 5-case videos 已补齐，full gate stopped | 旧 local-`z` anchor 规则已废弃；corrected endpoint 几何闭合。Full weld load path 最强但 over-constrains full pose；single connect 欠约束；multi-connect 有单调改善但仍不过 sanity。E029 按 plan 34 停在 full gate；若继续必须上 true per-axis D6 / dedicated relative-pose impedance，并另写计划，不应继续 anchor/gain sweep |
| E021 RL export | 13/13 base conversion ok；13/13 load verification ok；Holosoma output `fps=[50]`、`joint_pos[T,36]`、`joint_vel[T,35]`、object/body velocity fields complete；filename style `{original_prefix}_v2_mj_w_obj.npz`；已修 MJWP qpos 半帧问题并生成 12 个双向 `_v2_mj_w_obj_w_partner.npz` | `spider_best_E018b_E022_E025` 已可作为 Holosoma RL 数据目录输入；V4 partner-hand 训练优先用 `_w_partner` 文件；`desk021_p1` 因无 paired p2 暂无 partner 版 |
...
- E029: `workspace/core4d_collab_retarget/plan/34_E029_cola_d6_support_body_redesign_plan.md`
...
- E021: `workspace/core4d_collab_retarget/log/28_E021_holosoma_rl_export_results.md`
...
- E029: `workspace/core4d_collab_retarget/log/30_E029_cola_d6_support_preflight_results.md`
- E029: `workspace/core4d_collab_retarget/log/31_E029_cola_d6_support_sanity_results.md`
- E029: `workspace/core4d_collab_retarget/log/32_E029_compliant_support_sweep_results.md`
- E029: `workspace/core4d_collab_retarget/log/33_E029_freejoint_support_body_results.md`
- E029: `workspace/core4d_collab_retarget/log/34_E029_point_connect_diagnostic_results.md`
- E029: `workspace/core4d_collab_retarget/log/35_E029_multiconnect_bounded_6d_results.md`
- E029: `workspace/core4d_collab_retarget/log/36_E029_plan34_gate_audit_results.md`
