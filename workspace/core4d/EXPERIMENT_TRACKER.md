...
|-----|------|-------|------|------|-----|
| R296 | 2026-09-05 | Phase 69 | E210 bucket007 aug×PRG+G1：复用 E202 平移增强资产 + 仅 object gravcomp(15 变体/5 case, A0, trans-only) | 🚧 **P0–P6 完成，full CEM 待用户执行**。契约 15/15（两份 authority sha pin；`075_p2` 三档 `runtime_initial_overlap` **显式排除**而非静默丢失，E202 回填会主动报错）；sidecar 15/15 过 `assert_gravcomp_diff`，并用篡改样本反向自测证守卫有效；Hydra compose 全 key diff **恰为 {scene_name}**；manifest 轨迹/掩码 sha 与 E202 逐条相等且与磁盘一致；smoke 1/1 + 运行时 **9/9**（`max_violation_pct=0.10`/`hard_floor=−0.020` 为 A0 非 A2，task 指 aug 目录）。补齐 2×2 第四格 → **`C(E202 aug×PRG)→D` 严格单变量**隔离 gravcomp；`B(E207 orig×G1)→D` 为主判定但带 v1/v2 retarget confound。**判据据 E209/E208 改写**：虚扶检测器改用 `eef_ori`（非 `contact_in_mask`），并新增 `hand_pen` 独立门（E208 F14：aug 主代价在手穿透，13/16 掉门翻转）；不设 z 数值门（E209 已证 E207 常量模型不可外推） | [plan240](plan/240_E210_bucket007_aug_g1only_gravcomp_plan.md) |
