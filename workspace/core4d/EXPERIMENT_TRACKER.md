...
|-----|------|-------|------|------|-----|
| R291/R292 | 2026-09-03 | Phase 66 | E206 desk+chair 走 dcv3 全流程 + 非凸碰撞代理 + noPRG/PRG 双arm(3cm/move2_*/core4d v1) | ⏳ **S0–S2 全闭合，CEM 未跑**：S1 落地74case/9物体→**人工弃 chair021 后 65case/8物体(130条CEM)**;desk005 归零退出。碰撞代理改为**人工3D自由编辑**(新 `edit_proxy_3d.py`,中心+双角点 gizmo/贴合/镜像/未覆盖红点云/逐box腔体过填;存绝对几何 `manual_boxes.json` 取代脆弱的索引式 box_edits) → 契约 **8/8 all_pass、G6豁免归零**(chair022过填.41→.12、chair006 .26→.06、desk021 p90 .042→.005);15/15模板实装+15行复审全签,65/65 case `clean_reviewed`。新增 **G10 支撑面共面门**(用户提出:3/4条腿必须同时着地;实测手编后8/8不合格,高低差0.4–16mm、desk020穿地3cm,`align_supports.py` 对齐后全归零)。**5个真bug**:dcv3材质替换静默no-op、6物体mesh资产从未物化(影响41/74case)、E176 center_inside断言卡死全部椅子、语义分支重放voxel时代索引致2物体build失败、**复审TSV只写person1→34/65 case会在Stage2b被静默hold**。**S1真瓶颈=rotation>=45°门卡掉56/164(34%)，非接触阈值**。待办:P3吞吐冻结→P5起S3–S6 | [295](log/295_E206_desk_chair_move2_dcv3_noprg_prg.md) · [plan236](plan/236_E206_desk_chair_move2_dcv3_noprg_prg_plan.md) |
