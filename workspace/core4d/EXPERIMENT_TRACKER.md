...
| Run | 日期 | Phase | 描述 | 状态 |
|-----|------|-------|------|------|
| E097 | 2026-05-29 | Phase 20 | **feature-based data_construction_v2 candidate refresh**: 把 E095/E096/E096b 的经验写回候选挖掘逻辑：score 只用 raw contact + geometry/reach proxy + known outcome exclusion，不再用 object key 或 source scene/template readiness 加分。新增 `workspace/core4d/scripts/E097/mine_feature_based_candidates.py`，输出 `38` 行 candidate/audit bank，其中 `14` 条 verified case 被排除，`8` 条未验证 candidate 进入 `candidate_target_posture_gate`，默认 enabled 前 `6` 条：`box021_028_p1/p2`、`020_p2`、`035_p1`、`030_p2`、`019_p2`。这些不是 positive，只是下一批应跑 Stage2b + inside/support gate 的候选；Box026 仍为 `large_reach_holdout`，Box022 先做 raw-contact preflight | ✅/📋 详见 log 120 + plan 104 |
