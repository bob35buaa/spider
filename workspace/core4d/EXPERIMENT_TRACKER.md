...
| Run | 日期 | Phase | 描述 | 状态 |
|-----|------|-------|------|------|
| E092 | 2026-05-29 | Phase 20 | **Three-case SPIDER dynamic + direct OmniRetarget smoke comparison**: 按 plan 98 对 E091 三条已可视化 case 做两条路线验证，并按本地 1 卡 + 远程 2 卡完成/回收。Stage A `spider_dyn smoke`: C1/C2/C3 全 FAIL，pelvis min `0.074/0.191/0.186m`，object mean `0.006/0.008/0.006m`，head/upper `0%`；C3 RH floor `39.0%`。因此无 Stage A full、无 Stage B `rl_from_spider` WORK 输入。Stage C `rl_from_omni smoke`: C1/C2/C3 也全 FAIL，pelvis min `0.073/0.135/0.189m`，object mean `0.006/0.008/0.006m`，head/upper `0%`；C3 RH floor `40.2%`，因此不跑 main。6 个 mp4、60 张 keyframes、6 张 contact sheets 非空；high visual review 确认失败是真实 pelvis/hip collapse，spider_dyn 与 direct Omni 视觉差异很小。结论：当前瓶颈是 pelvis/upright/contact/floor 约束，不是继续扩 medium-box 数据或直接跑 full/main | ❌/🔬 详见 log 114 + plan 98 |
