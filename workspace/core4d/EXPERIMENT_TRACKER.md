...
| Run | 日期 | Phase | 描述 | 状态 |
|-----|------|-------|------|------|
| E080 | 2026-05-15 | Phase 18 | **box025 大物体边界/负控复查**: 按 E079 no-hold + 3cm mask 口径跑 `box025_person1/person2`。两者 CEM 均完成；case-window 三阈值把 p1/p2 都判 True (`2/2=100%`)，fixed post2 均 False (`0/2`)。subagent 视觉复核：p2 明显好于 p1，但两者都不是严格真实搬运；p1 是趴箱/贴箱/推箱 false positive，p2 更像扶/推箱体。结论: box025 继续支持 Tier3/drop 历史判断，且证明 case-window 三阈值会误判大物体负控，下一步必须加入 object max/orientation/contact continuity/semantic visual label | ⚠️ 详见 log 101 |
...
- ⚠️ E080 results: box025 p1/p2 均完整运行并复核；case-window 数值 `2/2=True` 但视觉均非真实搬运，p1 明确 false positive，p2 仅较像扶/推箱。box025 作为 Tier3/drop 负控验证成功，并暴露 E079 三阈值在大物体上会误判: `workspace/core4d/log/101_E080_box025_boundary_control_results.md`
