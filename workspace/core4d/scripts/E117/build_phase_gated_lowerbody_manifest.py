#!/usr/bin/env python3
"""Build E117 phase/state-gated lower-body contact diagnostic manifest."""
...
SCRIPTS_ROOT = REPO / "workspace/core4d/scripts/E117"
RESULTS_ROOT = REPO / "workspace/core4d/results/E117"
...
class VariantSpec:
    ...
    hand_threshold: float = 0.06
    dynamic_ref_target: bool = False


RUNNABLE_VARIANTS = [
    VariantSpec("mask_gate_s2", gate_source="contact_mask"),
    VariantSpec(
        "mask_time_gate_s2",
        gate_source="contact_mask_time_window",
        start_time=0.6,
        end_time=3.0,
    ),
    VariantSpec(
        "handtarget_gate_s2",
        gate_source="contact_mask_and_hand_target",
        hand_threshold=0.08,
        dynamic_ref_target=True,
    ),
]

CASE_SPLITS = {
    "box021_029_p2": ("remote-gpu0", "main_state_gated"),
    ...
    for case_id, (split, phase_scope) in CASE_SPLITS.items():
        ...
        for spec in RUNNABLE_VARIANTS:
            rows.append(
                {
                    "ordinal": ordinal,
                    "variant": f"E117_{case_id}_{spec.ablation}",
...
def override_text(row: dict[str, Any]) -> str:
    ...
    dynamic = "true" if spec.dynamic_ref_target else "false"
    return f"""# @package _global_
# Auto-generated by workspace/core4d/scripts/E117/build_phase_gated_lowerbody_manifest.py.
# E117 phase/state-gated lower-body diagnostic; case={row['case_id']} ablation={row['ablation']}.
defaults:
  - {row['source_override']}
  - _self_
...
leg_object_penalty_scale: {spec.leg_scale:.1f}
leg_object_penalty_margin_m: {spec.leg_margin:.2f}
leg_object_penalty_geom_names: [{leg_names}]
leg_object_penalty_geom_ids: []
leg_object_penalty_gate_source: {spec.gate_source}
leg_object_penalty_start_eval_time: {spec.start_time:.1f}
leg_object_penalty_end_eval_time: {spec.end_time:.1f}
leg_object_penalty_hand_target_threshold_m: {spec.hand_threshold:.2f}
        ...
        out.append(
            {
                **{field: row[field] for field in FIELDS},
                ...
                "leg_gate_source": spec.gate_source,
                "leg_scale": spec.leg_scale,
                "dynamic_ref_target": spec.dynamic_ref_target,
                "leg_geom_count": len(LEG_GEOMS),
                **validate_mask(mask_path, person_idx),
            }
        )
    return out
...
def write_summary(rows: list[dict[str, Any]], pf_rows: list[dict[str, Any]], override_paths: list[str]) -> None:
    summary = {
        ...
        "splits": {
            ...
            for split, _ in set(CASE_SPLITS.values())
        },
    ...
    lines = [
        "# E117 Phase/State-Gated Lower-Body Manifest Summary",
        "",
    ...
    lines.extend(["", "| case | ablation | split | gate | mask either | phase | queue |", "|---|---|---|---|---:|---|---|"])
    for row in pf_rows:
        lines.append(
            f"| `{row['case_id']}` | `{row['ablation']}` | `{row['split']}` | "
            f"`{row['leg_gate_source']}` | {float(row['mask_active_either_frac']) * 100:.1f}% | "
...
def main() -> None:
    ...
    write_tsv(VARIANTS_TSV, public_rows, FIELDS, "E117 phase/state-gated lower-body variants")
    pf_rows = preflight(rows)
    write_tsv(PREFLIGHT_TSV, pf_rows, list(pf_rows[0]), "E117 Phase A preflight")
