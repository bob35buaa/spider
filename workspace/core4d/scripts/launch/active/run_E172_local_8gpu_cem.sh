#!/usr/bin/env bash
# E172 local 8-GPU CEM launcher (rubber_hull + E170 PRG cross-object candidate).
#
# Runs the frozen S5_READY CEM manifest on the local GPUs, sharded by the
# per-row assigned_gpu, one E169/E172 queue runner per GPU. Before launching it
...
#
# Usage:
#   MODE=canary bash workspace/core4d/scripts/launch/active/run_E172_local_8gpu_cem.sh
#   MODE=full   bash workspace/core4d/scripts/launch/active/run_E172_local_8gpu_cem.sh
set -euo pipefail
...
RUNNER=workspace/core4d/scripts/experiments/E172/run_cem_queue.py
...
mkdir -p "$SHARD_DIR" "logs/E172/cem/$MODE"
...
for g in $GPUS; do
  ...
  log="logs/E172/cem/$MODE/gpu${g}.log"
