#!/usr/bin/env bash
# E167 remote launcher for Holosoma z-only CEM arms.
set -euo pipefail
...
SESSION="${SESSION:-E167_${STAGE}_$(date +%H%M%S)}"
PYTHON_BIN="${PYTHON_BIN:-.venv/bin/python}"
REMOTE_GPU0_WORK="${REMOTE_GPU0_WORK:-}"
REMOTE_GPU1_WORK="${REMOTE_GPU1_WORK:-}"
WAIT_FOR_GPU_IDLE="${WAIT_FOR_GPU_IDLE:-0}"
E167_GPU_IDLE_MAX_MEM_MB="${E167_GPU_IDLE_MAX_MEM_MB:-3000}"
E167_GPU_IDLE_MAX_UTIL_PCT="${E167_GPU_IDLE_MAX_UTIL_PCT:-20}"
E167_GPU_IDLE_POLL_SEC="${E167_GPU_IDLE_POLL_SEC:-120}"
E167_GPU_IDLE_STABLE_POLLS="${E167_GPU_IDLE_STABLE_POLLS:-2}"
...
retry() {
  local attempt=1
  local max_attempts="${E167_REMOTE_RETRY_MAX:-4}"
  while true; do
    if "$@"; then
      return 0
    fi
...
echo "Syncing E167 artifacts to ${REMOTE}:${REMOTE_ROOT}"
retry ssh "$REMOTE" "mkdir -p \
  ...
  '$REMOTE_ROOT/workspace/core4d/scripts/experiments/E167' \
  ...
  '$REMOTE_ROOT/workspace/core4d/results/E167/holosoma_zonly/cem/${STAGE}' \
  '$REMOTE_ROOT/logs/E167/cem/${STAGE}'"
...
retry rsync -az spider/postprocess/smooth_handoff_zonly.py "${REMOTE}:${REMOTE_ROOT}/spider/postprocess/smooth_handoff_zonly.py"
retry rsync -az workspace/core4d/scripts/experiments/E167/ "${REMOTE}:${REMOTE_ROOT}/workspace/core4d/scripts/experiments/E167/"
retry rsync -az workspace/core4d/scripts/experiments/E163/ "${REMOTE}:${REMOTE_ROOT}/workspace/core4d/scripts/experiments/E163/"
retry rsync -az workspace/core4d/scripts/launch/active/run_E167_local.sh "${REMOTE}:${REMOTE_ROOT}/workspace/core4d/scripts/launch/active/run_E167_local.sh"
...
retry rsync -az examples/config/override/core4d_E167_*.yaml "${REMOTE}:${REMOTE_ROOT}/examples/config/override/"
...
retry ssh "$REMOTE" "cd '$REMOTE_ROOT' && bash workspace/core4d/scripts/convert/snapshot_scenes.sh E167 ${SNAPSHOT_TASKS[*]}"
...
retry ssh "$REMOTE" "cd '$REMOTE_ROOT' && \
  .venv/bin/python -m py_compile \
    ...
    spider/postprocess/smooth_handoff.py spider/postprocess/smooth_handoff_zonly.py \
    workspace/core4d/scripts/experiments/E167/build_zonly_manifest.py && \
  bash -n workspace/core4d/scripts/launch/active/run_E167_local.sh && \
  .venv/bin/python workspace/core4d/scripts/experiments/E167/build_zonly_manifest.py && \
  echo preflight-ok"

retry ssh "$REMOTE" "cd '$REMOTE_ROOT' && \
  tmux new-session -d -s '$SESSION' \"bash -lc 'set -euo pipefail; \
    echo E167 remote session $SESSION stage=$STAGE; \
    (E167_SKIP_SCENE_SNAPSHOT=1 WAIT_FOR_GPU_IDLE=$WAIT_FOR_GPU_IDLE E167_GPU_IDLE_MAX_MEM_MB=$E167_GPU_IDLE_MAX_MEM_MB E167_GPU_IDLE_MAX_UTIL_PCT=$E167_GPU_IDLE_MAX_UTIL_PCT E167_GPU_IDLE_POLL_SEC=$E167_GPU_IDLE_POLL_SEC E167_GPU_IDLE_STABLE_POLLS=$E167_GPU_IDLE_STABLE_POLLS E167_SPLIT=remote-gpu0 CASE_ARMS=\\\"$REMOTE_GPU0_WORK\\\" LOCAL_GPU=0 bash workspace/core4d/scripts/launch/active/run_E167_local.sh $STAGE) & PID0=\\\$!; \
    (E167_SKIP_SCENE_SNAPSHOT=1 WAIT_FOR_GPU_IDLE=$WAIT_FOR_GPU_IDLE E167_GPU_IDLE_MAX_MEM_MB=$E167_GPU_IDLE_MAX_MEM_MB E167_GPU_IDLE_MAX_UTIL_PCT=$E167_GPU_IDLE_MAX_UTIL_PCT E167_GPU_IDLE_POLL_SEC=$E167_GPU_IDLE_POLL_SEC E167_GPU_IDLE_STABLE_POLLS=$E167_GPU_IDLE_STABLE_POLLS E167_SPLIT=remote-gpu1 CASE_ARMS=\\\"$REMOTE_GPU1_WORK\\\" LOCAL_GPU=1 bash workspace/core4d/scripts/launch/active/run_E167_local.sh $STAGE) & PID1=\\\$!; \
    echo launched PID0=\\\$PID0 PID1=\\\$PID1; \
    wait \\\$PID0; echo remote-gpu0 done; \
    wait \\\$PID1; echo remote-gpu1 done; \
    echo E167 remote complete'\""
