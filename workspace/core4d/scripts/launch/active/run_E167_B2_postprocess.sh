#!/usr/bin/env bash
# E167 CPU-only z-only B2 handoff postprocess.
set -euo pipefail
...
LOGS="logs/E167/postprocess/${STAGE}"

if [ "$STAGE" != "full" ]; then
  echo "Invalid STAGE=$STAGE (only full is supported for E167 B2 postprocess)" >&2
  exit 2
fi
...
for row in targets:
    ...
    log = repo / "logs/E167/postprocess/full" / f"{row['variant']}.log"
    if dst.is_file():
    ...
    cmd = [
        sys.executable,
        "spider/postprocess/smooth_handoff_zonly.py",
        "--input",
        str(src),
        "--output",
        str(dst),
...
print(f"E167 B2 z-only postprocess complete: rows={len(targets)}")
PY
