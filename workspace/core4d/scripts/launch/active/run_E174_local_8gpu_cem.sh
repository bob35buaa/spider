#!/usr/bin/env bash
# E174 local 8-GPU CEM launcher (rubber_hull + E170 PRG cross-object candidate).
#
# Runs the frozen S5_READY CEM manifest on the local GPUs, sharded by the
# per-row assigned_gpu, one E169/E174 queue runner per GPU. Before launching it
...
mkdir -p "$SHARD_DIR" "logs/E174/cem/$MODE"
...
for g in $GPUS; do
  ...
  log="logs/E174/cem/$MODE/gpu${g}.log"
