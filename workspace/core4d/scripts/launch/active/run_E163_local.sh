#!/usr/bin/env bash
# E163 local runner for narrow symmetric surfaceBand three-case probe.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

STAGE="${1:-full}"
LOCAL_GPU="${LOCAL_GPU:-0}"
E163_SPLIT="${E163_SPLIT:-local-gpu0}"
PYTHON_BIN="${PYTHON_BIN:-.venv/bin/python}"
VARIANTS="workspace/core4d/scripts/experiments/E163/variants.tsv"
RESULTS="workspace/core4d/results/E163/narrow_surface_band/cem/${STAGE}"
LOGS="logs/E163/cem/${STAGE}"
CASE_METHODS="${CASE_METHODS:-}"
WAIT_FOR_GPU_IDLE="${WAIT_FOR_GPU_IDLE:-0}"
GPU_IDLE_MAX_MEM_MB="${E163_GPU_IDLE_MAX_MEM_MB:-3000}"
GPU_IDLE_MAX_UTIL_PCT="${E163_GPU_IDLE_MAX_UTIL_PCT:-20}"
GPU_IDLE_POLL_SEC="${E163_GPU_IDLE_POLL_SEC:-120}"
GPU_IDLE_STABLE_POLLS="${E163_GPU_IDLE_STABLE_POLLS:-2}"
...
selected_rows() {
  "$PYTHON_BIN" - "$VARIANTS" "$E163_SPLIT" "$CASE_METHODS" <<'PY'
import csv
import sys
...
echo "=== E163 ${STAGE} local runner split=${E163_SPLIT} GPU=${LOCAL_GPU} ==="
...
echo "=== E163 ${STAGE} local runner complete; selected_rows=${row_count} ==="
