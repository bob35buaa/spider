#!/usr/bin/env bash
# E159 remote launcher for clean6 gateA + surfaceBand-A2 rows.
set -euo pipefail
...
SESSION="${SESSION:-E159_${STAGE}_$(date +%H%M%S)}"
PYTHON_BIN="${PYTHON_BIN:-.venv/bin/python}"
REMOTE_GPU0_WORK="${REMOTE_GPU0_WORK:-}"
REMOTE_GPU1_WORK="${REMOTE_GPU1_WORK:-}"
WAIT_FOR_GPU_IDLE="${WAIT_FOR_GPU_IDLE:-0}"
E159_GPU_IDLE_MAX_MEM_MB="${E159_GPU_IDLE_MAX_MEM_MB:-3000}"
E159_GPU_IDLE_MAX_UTIL_PCT="${E159_GPU_IDLE_MAX_UTIL_PCT:-20}"
E159_GPU_IDLE_POLL_SEC="${E159_GPU_IDLE_POLL_SEC:-120}"
E159_GPU_IDLE_STABLE_POLLS="${E159_GPU_IDLE_STABLE_POLLS:-2}"
...
"$PYTHON_BIN" workspace/core4d/scripts/experiments/E159/build_gate_surface_band_A2_manifest.py >/dev/null

retry() {
  local attempt=1
  local max_attempts="${E159_REMOTE_RETRY_MAX:-4}"
  while true; do
    if "$@"; then
      return 0
    fi
...
rows = list(csv.DictReader(open("workspace/core4d/scripts/experiments/E159/variants.tsv", newline=""), delimiter="\t"))
...
echo "Syncing E159 artifacts to ${REMOTE}:${REMOTE_ROOT}"
retry ssh "$REMOTE" "mkdir -p \
  ...
  '$REMOTE_ROOT/workspace/core4d/results/E159/gate_surface_band_A2/cem/${STAGE}' \
  '$REMOTE_ROOT/logs/E159/cem/${STAGE}'"
...
retry rsync -az workspace/core4d/scripts/launch/active/run_E159_local.sh "${REMOTE}:${REMOTE_ROOT}/workspace/core4d/scripts/launch/active/run_E159_local.sh"
retry rsync -az examples/config/override/core4d_E159_*.yaml "${REMOTE}:${REMOTE_ROOT}/examples/config/override/"
...
retry ssh "$REMOTE" "cd '$REMOTE_ROOT' && \
  .venv/bin/python -m py_compile \
    ...
    workspace/core4d/scripts/experiments/E159/build_gate_surface_band_A2_manifest.py && \
  bash -n workspace/core4d/scripts/launch/active/run_E159_local.sh && \
  echo preflight-ok"

retry ssh "$REMOTE" "cd '$REMOTE_ROOT' && \
  tmux new-session -d -s '$SESSION' \"bash -lc 'set -euo pipefail; \
    echo E159 remote session $SESSION stage=$STAGE; \
    (WAIT_FOR_GPU_IDLE=$WAIT_FOR_GPU_IDLE E159_GPU_IDLE_MAX_MEM_MB=$E159_GPU_IDLE_MAX_MEM_MB E159_GPU_IDLE_MAX_UTIL_PCT=$E159_GPU_IDLE_MAX_UTIL_PCT E159_GPU_IDLE_POLL_SEC=$E159_GPU_IDLE_POLL_SEC E159_GPU_IDLE_STABLE_POLLS=$E159_GPU_IDLE_STABLE_POLLS E159_SPLIT=remote-gpu0 CASE_METHODS=\\\"$REMOTE_GPU0_WORK\\\" LOCAL_GPU=0 bash workspace/core4d/scripts/launch/active/run_E159_local.sh $STAGE) & PID0=\\\$!; \
    (WAIT_FOR_GPU_IDLE=$WAIT_FOR_GPU_IDLE E159_GPU_IDLE_MAX_MEM_MB=$E159_GPU_IDLE_MAX_MEM_MB E159_GPU_IDLE_MAX_UTIL_PCT=$E159_GPU_IDLE_MAX_UTIL_PCT E159_GPU_IDLE_POLL_SEC=$E159_GPU_IDLE_POLL_SEC E159_GPU_IDLE_STABLE_POLLS=$E159_GPU_IDLE_STABLE_POLLS E159_SPLIT=remote-gpu1 CASE_METHODS=\\\"$REMOTE_GPU1_WORK\\\" LOCAL_GPU=1 bash workspace/core4d/scripts/launch/active/run_E159_local.sh $STAGE) & PID1=\\\$!; \
    echo launched PID0=\\\$PID0 PID1=\\\$PID1; \
    wait \\\$PID0; echo remote-gpu0 done; \
    wait \\\$PID1; echo remote-gpu1 done; \
    echo E159 remote complete'\""
