#!/usr/bin/env bash
# E200 arm noPRG (E167A) -- full CEM 8-GPU priority queue (all feasible E199 trans variants).
# Reuses E199 aug trajectories; scene = rubber-hull (no-PRG) intermediate; override =
# E199 augmented base task yaml (chains E167A) + CLI scene_name + leg penalty/gate OFF.
# orig baseline (eval) = E190 38-case noPRG (only-read; only 38 of 87 cases pairable).
#
...
#
# Usage:
#   bash workspace/core4d/scripts/launch/active/run_E200_noprg_8gpu.sh
#   DRY_RUN=1 bash .../run_E200_noprg_8gpu.sh          # preview commands
#   LIMIT=1  bash .../run_E200_noprg_8gpu.sh           # smoke: one case then exit
