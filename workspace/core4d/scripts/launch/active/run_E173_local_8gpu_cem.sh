#!/usr/bin/env bash
# E173 local 8-GPU CEM launcher (rubber_hull + E170 PRG cross-object candidate).
#
# Runs the frozen S5_READY CEM manifest on the local GPUs, sharded by the
# per-row assigned_gpu, one E169/E173 queue runner per GPU. Before launching it
...
RUNNER=workspace/core4d/scripts/experiments/E173/run_cem_queue.py
...
mkdir -p "$SHARD_DIR" "logs/E173/cem/$MODE"
...
for g in $GPUS; do
  ...
  log="logs/E173/cem/$MODE/gpu${g}.log"
