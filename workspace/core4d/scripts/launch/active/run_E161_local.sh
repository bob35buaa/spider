#!/usr/bin/env bash
# E161 local runner for surfaceBand release ablation clean8 rows.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

STAGE="${1:-full}"
LOCAL_GPU="${LOCAL_GPU:-0}"
E161_SPLIT="${E161_SPLIT:-local-gpu0}"
PYTHON_BIN="${PYTHON_BIN:-.venv/bin/python}"
VARIANTS="workspace/core4d/scripts/experiments/E161/variants.tsv"
...
GPU_IDLE_MAX_MEM_MB="${E161_GPU_IDLE_MAX_MEM_MB:-3000}"
GPU_IDLE_MAX_UTIL_PCT="${E161_GPU_IDLE_MAX_UTIL_PCT:-20}"
GPU_IDLE_POLL_SEC="${E161_GPU_IDLE_POLL_SEC:-120}"
GPU_IDLE_STABLE_POLLS="${E161_GPU_IDLE_STABLE_POLLS:-2}"
...
selected_rows() {
  "$PYTHON_BIN" - "$VARIANTS" "$E161_SPLIT" "$CASE_METHODS" <<'PY'
import csv
import sys
...
echo "=== E161 ${STAGE} local runner split=${E161_SPLIT} GPU=${LOCAL_GPU} ==="
...
echo "=== E161 ${STAGE} local runner complete; selected_rows=${row_count} ==="
