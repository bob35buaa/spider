#!/usr/bin/env bash
# E163 remote launcher for narrow symmetric surfaceBand three-case probe.
set -euo pipefail
...
SESSION="${SESSION:-E163_${STAGE}_$(date +%H%M%S)}"
PYTHON_BIN="${PYTHON_BIN:-.venv/bin/python}"
REMOTE_GPU0_WORK="${REMOTE_GPU0_WORK:-}"
REMOTE_GPU1_WORK="${REMOTE_GPU1_WORK:-}"
WAIT_FOR_GPU_IDLE="${WAIT_FOR_GPU_IDLE:-0}"
E163_GPU_IDLE_MAX_MEM_MB="${E163_GPU_IDLE_MAX_MEM_MB:-3000}"
E163_GPU_IDLE_MAX_UTIL_PCT="${E163_GPU_IDLE_MAX_UTIL_PCT:-20}"
E163_GPU_IDLE_POLL_SEC="${E163_GPU_IDLE_POLL_SEC:-120}"
E163_GPU_IDLE_STABLE_POLLS="${E163_GPU_IDLE_STABLE_POLLS:-2}"
...
"$PYTHON_BIN" workspace/core4d/scripts/experiments/E163/build_narrow_surface_band_manifest.py >/dev/null

retry() {
  local attempt=1
  local max_attempts="${E163_REMOTE_RETRY_MAX:-4}"
  while true; do
    if "$@"; then
      return 0
    fi
...
rows = list(csv.DictReader(open("workspace/core4d/scripts/experiments/E163/variants.tsv", newline=""), delimiter="\t"))
...
echo "Syncing E163 artifacts to ${REMOTE}:${REMOTE_ROOT}"
retry ssh "$REMOTE" "mkdir -p \
  ...
  '$REMOTE_ROOT/workspace/core4d/results/E163/narrow_surface_band/cem/${STAGE}' \
  '$REMOTE_ROOT/logs/E163/cem/${STAGE}'"
...
retry rsync -az workspace/core4d/scripts/launch/active/run_E163_local.sh "${REMOTE}:${REMOTE_ROOT}/workspace/core4d/scripts/launch/active/run_E163_local.sh"
...
retry ssh "$REMOTE" "cd '$REMOTE_ROOT' && \
  tmux new-session -d -s '$SESSION' \"bash -lc 'set -euo pipefail; \
    echo E163 remote session $SESSION stage=$STAGE; \
    (WAIT_FOR_GPU_IDLE=$WAIT_FOR_GPU_IDLE E163_GPU_IDLE_MAX_MEM_MB=$E163_GPU_IDLE_MAX_MEM_MB E163_GPU_IDLE_MAX_UTIL_PCT=$E163_GPU_IDLE_MAX_UTIL_PCT E163_GPU_IDLE_POLL_SEC=$E163_GPU_IDLE_POLL_SEC E163_GPU_IDLE_STABLE_POLLS=$E163_GPU_IDLE_STABLE_POLLS E163_SPLIT=remote-gpu0 CASE_METHODS=\\\"$REMOTE_GPU0_WORK\\\" LOCAL_GPU=0 bash workspace/core4d/scripts/launch/active/run_E163_local.sh $STAGE) & PID0=\\\$!; \
    (WAIT_FOR_GPU_IDLE=$WAIT_FOR_GPU_IDLE E163_GPU_IDLE_MAX_MEM_MB=$E163_GPU_IDLE_MAX_MEM_MB E163_GPU_IDLE_MAX_UTIL_PCT=$E163_GPU_IDLE_MAX_UTIL_PCT E163_GPU_IDLE_POLL_SEC=$E163_GPU_IDLE_POLL_SEC E163_GPU_IDLE_STABLE_POLLS=$E163_GPU_IDLE_STABLE_POLLS E163_SPLIT=remote-gpu1 CASE_METHODS=\\\"$REMOTE_GPU1_WORK\\\" LOCAL_GPU=1 bash workspace/core4d/scripts/launch/active/run_E163_local.sh $STAGE) & PID1=\\\$!; \
    echo launched PID0=\\\$PID0 PID1=\\\$PID1; \
    wait \\\$PID0; echo remote-gpu0 done; \
    wait \\\$PID1; echo remote-gpu1 done; \
    echo E163 remote complete'\""
