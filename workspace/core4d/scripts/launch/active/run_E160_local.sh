#!/usr/bin/env bash
# E160 local runner for 3-case gateA + surfaceBand-A2 + postureRerankA CEM rows.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

STAGE="${1:-full}"
LOCAL_GPU="${LOCAL_GPU:-0}"
E160_SPLIT="${E160_SPLIT:-local-gpu0}"
PYTHON_BIN="${PYTHON_BIN:-.venv/bin/python}"
VARIANTS="workspace/core4d/scripts/experiments/E160/variants.tsv"
...
GPU_IDLE_MAX_MEM_MB="${E160_GPU_IDLE_MAX_MEM_MB:-3000}"
GPU_IDLE_MAX_UTIL_PCT="${E160_GPU_IDLE_MAX_UTIL_PCT:-20}"
GPU_IDLE_POLL_SEC="${E160_GPU_IDLE_POLL_SEC:-120}"
GPU_IDLE_STABLE_POLLS="${E160_GPU_IDLE_STABLE_POLLS:-2}"
...
selected_rows() {
  "$PYTHON_BIN" - "$VARIANTS" "$E160_SPLIT" "$CASE_METHODS" <<'PY'
import csv
import sys
...
echo "=== E160 ${STAGE} local runner split=${E160_SPLIT} GPU=${LOCAL_GPU} ==="
...
echo "=== E160 ${STAGE} local runner complete; selected_rows=${row_count} ==="
