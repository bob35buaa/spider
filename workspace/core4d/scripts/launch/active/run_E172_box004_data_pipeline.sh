#!/usr/bin/env bash
# E172 S0-S5 local data pipeline (Box022 + Box026, fresh from CORE4D raw).
#
# Canonical reproducible entry for the E172 data-construction chain. Drives the
# shared data_construction_v3 stage scripts + the E172 v1->v2 rescue queue, on
...
E172=workspace/core4d/scripts/experiments/E172

E=workspace/core4d/results/E172
REG=$E/registries
...
[ -f "$RCPASS" ] || \
  $PYTHON_BIN $S1/run_raw_contact.py --core4d-raw-root "$CORE4D_RAW_ROOT" \
    ...
    --queue object-key --object-keys box004 --thresholds-m 0.03,0.05
...
[ -f "$V1M" ] || $PYTHON_BIN $S3/run_stage2b.py \
  ...
  --out-dir "$V1"
$PYTHON_BIN $E172/run_stage2b_queue.py --manifest-tsv "$V1M" \
...
$PYTHON_BIN $E172/build_omnirt_rescue_manifest.py \
...
if [ -f "$RESCUE_IN" ]; then
  ...
  [ -f "$V2M" ] || $PYTHON_BIN $S3/run_stage2b.py \
    ...
    --smplx-model-dir "$SMPLX_MODEL_DIR" --retarget-python-bin "$RETARGET_PYTHON_BIN" \
    --out-dir "$V2"
  $PYTHON_BIN $E172/run_stage2b_queue.py --manifest-tsv "$V2M" \
    --holosoma-repo "$HOLOSOMA_REPO" --core4d-raw-root "$CORE4D_RAW_ROOT" \
    --smplx-model-dir "$SMPLX_MODEL_DIR" --retarget-python-bin "$RETARGET_PYTHON_BIN"
