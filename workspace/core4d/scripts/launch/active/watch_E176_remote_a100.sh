#!/usr/bin/env bash
# Monitor one E176 A100 session; pull and validate after two absent checks.
set -euo pipefail
...
PULL_SCRIPT="${PULL_SCRIPT:-workspace/core4d/scripts/launch/active/pull_E176_remote_a100_results.sh}"
...
while true; do
  ...
  if [ "$remote_rc" -eq 0 ]; then
    absent_checks=0
    echo "[$timestamp] $EXPERIMENT_ID $MODE session=$SESSION running"
    ssh "${SSH_OPTS[@]}" "$REMOTE" \
    ...
    echo "[$timestamp] $EXPERIMENT_ID $MODE session absent confirmation=$absent_checks/2"
