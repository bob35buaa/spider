#!/usr/bin/env bash
# E173 S0-S5 local data pipeline (box024 + box023 + box001, fresh from CORE4D raw).
#
# Canonical reproducible entry for the E173 data-construction chain. Drives the
# shared data_construction_v3 stage scripts + the E173 v1->v2 rescue queue, on
...
E173=workspace/core4d/scripts/experiments/E173

E=workspace/core4d/results/E173
REG=$E/registries
...
[ -f "$RCPASS" ] || \
  $PYTHON_BIN $S1/run_raw_contact.py --core4d-raw-root "$CORE4D_RAW_ROOT" \
    ...
    --queue object-key --object-keys box024,box023,box001 --thresholds-m 0.03,0.05
...
# --- move-only enforcement (E173): drop non-move rows before Stage2b ----------
# The raw_contact stage scores contact for ALL candidates, so non-move actions
# (join/leave/rot/pass/strike -> inventory_decision=reject_action_not_first_line)
# can pass 3cm contact. Move-only policy: only inventory_pass/review_to_raw_contact
# rows enter S3. E172 (box004) never hit this (its non-move failed raw contact);
# E173's box001 has non-move rows passing 3cm, so we filter explicitly.
RCPASS_MOVEONLY=$RC/raw_contact_pass_3cm_moveonly.tsv
$PYTHON_BIN - "$RCPASS" "$RCPASS_MOVEONLY" <<'PYEOF'
import csv, sys
src, dst = sys.argv[1], sys.argv[2]
MOVE_OK = {"inventory_pass_to_raw_contact", "inventory_review_to_raw_contact"}
rows = list(csv.DictReader(open(src), delimiter="\t"))
kept = [r for r in rows if r.get("inventory_decision") in MOVE_OK]
dropped = len(rows) - len(kept)
with open(dst, "w", newline="") as f:
    w = csv.DictWriter(f, fieldnames=rows[0].keys(), delimiter="\t", lineterminator="\n")
    w.writeheader()
    w.writerows(kept)
print(f"[move-only] pass_3cm={len(rows)} -> move-only kept={len(kept)} dropped_nonmove={dropped}")
PYEOF
RCPASS=$RCPASS_MOVEONLY

# --- S2: box template fresh build/audit -------------------------------------
# E173 rebuilds box source templates so collision half-extents == mesh AABB
# (clean geometry). box024/box001 person2 are missing -> created; box023 p1/p2
# and box024 p1 fail geometry_review (collision box drifts >8% from mesh) -> the
# existing scene.xml is overwritten with a clean rebuild. box001 p1 is already
# clean; --overwrite-existing regenerates it to mesh_aabb for a uniform baseline.
say "S2 template build/audit (rebuild box templates: apply-build + overwrite-existing)"
...
[ -f "$V1M" ] || $PYTHON_BIN $S3/run_stage2b.py \
  ...
  --out-dir "$V1"
$PYTHON_BIN $E173/run_stage2b_queue.py --manifest-tsv "$V1M" \
...
$PYTHON_BIN $E173/build_omnirt_rescue_manifest.py \
...
if [ -f "$RESCUE_IN" ]; then
  ...
  [ -f "$V2M" ] || $PYTHON_BIN $S3/run_stage2b.py \
    ...
    --smplx-model-dir "$SMPLX_MODEL_DIR" --retarget-python-bin "$RETARGET_PYTHON_BIN" \
    --out-dir "$V2"
  $PYTHON_BIN $E173/run_stage2b_queue.py --manifest-tsv "$V2M" \
    --holosoma-repo "$HOLOSOMA_REPO" --core4d-raw-root "$CORE4D_RAW_ROOT" \
    --smplx-model-dir "$SMPLX_MODEL_DIR" --retarget-python-bin "$RETARGET_PYTHON_BIN"
