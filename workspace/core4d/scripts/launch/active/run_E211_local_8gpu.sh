#!/usr/bin/env bash
# E211 Stage A -- desk007 partial-gravcomp sweep (g = 0.4 / 0.6 / 0.8) on 8 GPUs.
#
...
# E211 manifest is tier P0, which would KeyError on dispatch.
#
...
#   shard is a disjoint manifest; A is 8 rows (local), B is 7 (remote). Both
#   span all 5 cases and all 3 g values, so a dead machine still leaves an
...
#
# Usage:
#   local  (8 rows):  SHARD=A bash workspace/core4d/scripts/launch/active/run_E211_local_8gpu.sh
#   remote (7 rows):  SHARD=B SKIP_SNAPSHOT=1 bash workspace/core4d/scripts/launch/active/run_E211_local_8gpu.sh
#   dry run:          SHARD=B DRY_RUN=1 bash .../run_E211_local_8gpu.sh
#   smoke:            STAGE=smoke SHARD=A GPUS=0 bash .../run_E211_local_8gpu.sh
#
...
MANIFEST_DIR="workspace/core4d/results/E211/s6_downstream/manifests"
SNAPSHOT_MANIFEST="workspace/core4d/results/E211/scene_snapshot/manifest.txt"
...
MANIFEST="$MANIFEST_DIR/e211_stageA_${STAGE}_manifest.shard${SHARD}.tsv"
[[ -f "$MANIFEST" ]] || {
  echo "missing shard manifest: $MANIFEST (run E211/build_manifest.py --stage $STAGE)" >&2
  exit 2
}

# rules/experiment.md §7 Safeguard 2: freeze the scenes before spending GPU.
if [[ "$SKIP_SNAPSHOT" != "1" && "$DRY_RUN" != "1" ]]; then
  bash workspace/core4d/scripts/experiments/E211/snapshot_E211_scenes.sh
elif [[ "$DRY_RUN" != "1" ]]; then
  ...
  echo "[run_E211] snapshot check OK (HEAD=$HEAD_SHA)"
fi
...
print(f"[run_E211] stamped host={host} on {len(rows)} rows")
PY
fi
...
echo "[run_E211] STAGE=$STAGE SHARD=$SHARD GPUS=$GPUS PER_GPU_MEM_MIB=$PER_GPU_MEM_MIB MAX_PER_GPU=$MAX_PER_GPU"
echo "[run_E211] manifest=$MANIFEST $EXTRA"
