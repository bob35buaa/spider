#!/usr/bin/env bash
# Pull E155 remote CEM results/logs back to local workspace.
set -euo pipefail
...
echo "Pulled E155 ${STAGE} results to ${RESULTS}"
find "$RESULTS" -maxdepth 1 -name 'E155_*.npz' | sort | wc -l | awk '{print "root_npz_count="$1}'
find "$RESULTS" -maxdepth 1 -name 'E155_*_'"${STAGE}"'.mp4' | sort | wc -l | awk '{print "video_count="$1}'
