#!/usr/bin/env bash
# E166 remote launcher for foot/smooth retarget CEM arms.
set -euo pipefail
...
SESSION="${SESSION:-E166_${STAGE}_$(date +%H%M%S)}"
...
"$PYTHON_BIN" workspace/core4d/scripts/experiments/E166/build_foot_smooth_manifest.py >/dev/null
...
rows = list(csv.DictReader(open("workspace/core4d/scripts/experiments/E166/variants.tsv", newline=""), delimiter="\t"))
...
echo "Syncing E166 artifacts to ${REMOTE}:${REMOTE_ROOT}"
retry ssh "$REMOTE" "mkdir -p \
  ...
  '$REMOTE_ROOT/logs/E166/cem/${STAGE}'"
...
retry rsync -az workspace/core4d/scripts/launch/active/run_E166_local.sh "${REMOTE}:${REMOTE_ROOT}/workspace/core4d/scripts/launch/active/run_E166_local.sh"
...
retry ssh "$REMOTE" "cd '$REMOTE_ROOT' && \
  .venv/bin/python -m py_compile \
    ...
    workspace/core4d/scripts/experiments/E166/build_foot_smooth_manifest.py && \
  bash -n workspace/core4d/scripts/launch/active/run_E166_local.sh && \
  .venv/bin/python workspace/core4d/scripts/experiments/E166/build_foot_smooth_manifest.py && \
  echo preflight-ok"

retry ssh "$REMOTE" "cd '$REMOTE_ROOT' && \
  tmux new-session -d -s '$SESSION' \"bash -lc 'set -euo pipefail; \
    echo E166 remote session $SESSION stage=$STAGE; \
    (WAIT_FOR_GPU_IDLE=$WAIT_FOR_GPU_IDLE E166_GPU_IDLE_MAX_MEM_MB=$E166_GPU_IDLE_MAX_MEM_MB E166_GPU_IDLE_MAX_UTIL_PCT=$E166_GPU_IDLE_MAX_UTIL_PCT E166_GPU_IDLE_POLL_SEC=$E166_GPU_IDLE_POLL_SEC E166_GPU_IDLE_STABLE_POLLS=$E166_GPU_IDLE_STABLE_POLLS E166_SPLIT=remote-gpu0 CASE_ARMS=\\\"$REMOTE_GPU0_WORK\\\" LOCAL_GPU=0 bash workspace/core4d/scripts/launch/active/run_E166_local.sh $STAGE) & PID0=\\\$!; \
    (WAIT_FOR_GPU_IDLE=$WAIT_FOR_GPU_IDLE E166_GPU_IDLE_MAX_MEM_MB=$E166_GPU_IDLE_MAX_MEM_MB E166_GPU_IDLE_MAX_UTIL_PCT=$E166_GPU_IDLE_MAX_UTIL_PCT E166_GPU_IDLE_POLL_SEC=$E166_GPU_IDLE_POLL_SEC E166_GPU_IDLE_STABLE_POLLS=$E166_GPU_IDLE_STABLE_POLLS E166_SPLIT=remote-gpu1 CASE_ARMS=\\\"$REMOTE_GPU1_WORK\\\" LOCAL_GPU=1 bash workspace/core4d/scripts/launch/active/run_E166_local.sh $STAGE) & PID1=\\\$!; \
    echo launched PID0=\\\$PID0 PID1=\\\$PID1; \
    wait \\\$PID0; echo remote-gpu0 done; \
    wait \\\$PID1; echo remote-gpu1 done; \
    echo E166 remote complete'\""
