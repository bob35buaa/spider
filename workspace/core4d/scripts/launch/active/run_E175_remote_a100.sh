#!/usr/bin/env bash
# E175 A100 launcher: dynamic policy allowlist, exact-file sync, serial queues.
set -euo pipefail
...
E175_FIXED_GPUS="${E175_FIXED_GPUS:-0}"
E175_PREP_ONLY="${E175_PREP_ONLY:-0}"
E175_SNAPSHOT_ONLY="${E175_SNAPSHOT_ONLY:-0}"
SESSION="${SESSION:-e175_a100_${MODE}_$(date +%Y%m%d_%H%M%S)}"
REMOTE_RUN_PARENT="${REMOTE_RUN_PARENT:-/home/dataset-assist-0/xiayb/workspace/e175_spider_runs}"
REMOTE_RUN_ROOT="${REMOTE_RUN_ROOT:-${REMOTE_RUN_PARENT}/${SESSION}}"

if [ "$E175_PREP_ONLY" != "0" ] && [ "$E175_PREP_ONLY" != "1" ]; then
  echo "E175_PREP_ONLY must be 0 or 1." >&2
  exit 2
fi
if [ "$E175_FIXED_GPUS" != "0" ] && [ "$E175_FIXED_GPUS" != "1" ]; then
  echo "E175_FIXED_GPUS must be 0 or 1." >&2
  exit 2
fi
if [ "$E175_SNAPSHOT_ONLY" != "0" ] && [ "$E175_SNAPSHOT_ONLY" != "1" ]; then
  echo "E175_SNAPSHOT_ONLY must be 0 or 1." >&2
  exit 2
fi
if [ "$E175_SNAPSHOT_ONLY" = "1" ] && [ "$E175_PREP_ONLY" = "1" ]; then
  echo "E175_SNAPSHOT_ONLY and E175_PREP_ONLY are mutually exclusive." >&2
  exit 2
fi
if [ "$E175_SNAPSHOT_ONLY" = "0" ] && [ -z "$A100_AVAILABILITY_CMD" ] && [ -z "$A100_POLICY_GPUS" ]; then
  echo "E175 A100 launch blocked: provide A100_AVAILABILITY_CMD or explicit A100_POLICY_GPUS." >&2
  echo "Low memory alone is not reservation/owner authorization." >&2
  exit 3
fi
if [ "$E175_SNAPSHOT_ONLY" = "0" ] && [ -n "$A100_AVAILABILITY_CMD" ] && [ -n "$A100_POLICY_GPUS" ]; then
...
if [ "$MODE" = "canary" ]; then
  MANIFEST="${MANIFEST_ROOT}/nonbox_multigeom_canary_manifest.tsv"
  CEM_SUBDIR="canary"
  NUM_SAMPLES=64
  NUM_ITERATIONS=4
else
  MANIFEST="${MANIFEST_ROOT}/nonbox_multigeom_full_manifest.tsv"
  CEM_SUBDIR="full"
  NUM_SAMPLES=1024
  NUM_ITERATIONS=32
  if [ "$E175_SNAPSHOT_ONLY" = "0" ]; then
    CANARY_GATE="${RESULT_ROOT}/s6_downstream/cem/canary/canary_runtime_gate.json"
    [ -f "$CANARY_GATE" ] || {
      echo "Full blocked: missing canary runtime gate $CANARY_GATE" >&2
      exit 4
    }
    "$LOCAL_PYTHON_BIN" - "$CANARY_GATE" <<'PY'
import json, sys
gate = json.load(open(sys.argv[1], encoding="utf-8"))
if gate.get("status") != "pass" or gate.get("passed_rows") != 6:
    raise SystemExit(f"Full blocked by canary gate: {gate}")
PY
  fi
fi
...
retry() {
  local attempt=1
  local max_attempts="${E175_REMOTE_RETRY_MAX:-4}"
  while true; do
    if "$@"; then
      return 0
    fi
...
files = {
    ...
    Path("workspace/core4d/scripts/experiments/E175/run_cem_queue.py"),
    Path("workspace/core4d/scripts/experiments/E175/verify_sync_inventory.py"),
}
...
print(f"E175 sync files: {len(files)}")
PY
}
...
expected = 6 if os.environ["MODE"] == "canary" else 39
...
LOG_DIR="logs/E175/cem/$CEM_SUBDIR"
mkdir -p "\$LOG_DIR"
worker_pids=()
...
echo "E175 \$MODE completed status=\$status at \$(date -Is)"
exit "\$status"
EOF
...
remote_preflight() {
  ...
  retry ssh_remote "cd '$REMOTE_RUN_ROOT' && \
    ...
    '$REMOTE_PYTHON_BIN' workspace/core4d/scripts/experiments/E175/verify_sync_inventory.py \
...
if [ "$E175_SNAPSHOT_ONLY" = "1" ]; then
  write_sync_list
  echo "E175 local snapshot inventory complete; no SSH/rsync/tmux."
...
sync_pool
if [ "$E175_PREP_ONLY" = "1" ]; then
  echo "E175 $MODE remote preflight passed; E175_PREP_ONLY=1, no tmux/CEM launched."
...
echo "Started E175 $MODE on A100 session=$SESSION GPUs=$SELECTED_GPUS"
echo "Execution manifest: $EXECUTION_JSON"
echo "Monitor: bash workspace/core4d/scripts/launch/active/watch_E175_remote_a100.sh $MODE"
