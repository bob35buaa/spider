#!/usr/bin/env bash
# E166 local CEM runner for foot/smooth retarget arms.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

STAGE="${1:-full}"
LOCAL_GPU="${LOCAL_GPU:-0}"
E166_SPLIT="${E166_SPLIT:-local-gpu0}"
PYTHON_BIN="${PYTHON_BIN:-.venv/bin/python}"
VARIANTS="workspace/core4d/scripts/experiments/E166/variants.tsv"
...
GPU_IDLE_MAX_MEM_MB="${E166_GPU_IDLE_MAX_MEM_MB:-3000}"
GPU_IDLE_MAX_UTIL_PCT="${E166_GPU_IDLE_MAX_UTIL_PCT:-20}"
GPU_IDLE_POLL_SEC="${E166_GPU_IDLE_POLL_SEC:-120}"
GPU_IDLE_STABLE_POLLS="${E166_GPU_IDLE_STABLE_POLLS:-2}"
...
selected_rows() {
  "$PYTHON_BIN" - "$VARIANTS" "$E166_SPLIT" "$CASE_ARMS" <<'PY'
import csv
import sys
...
echo "=== E166 ${STAGE} local runner split=${E166_SPLIT} GPU=${LOCAL_GPU} ==="
if [ -n "$CASE_ARMS" ]; then
...
echo "=== E166 ${STAGE} local runner complete; selected_rows=${row_count} ==="
