#!/usr/bin/env bash
# E173 Stage2b parallel retarget driver.
#
# The shared run_stage2b_queue.py runs cases SERIALLY. E173 has 56 move cases;
# on this 192-core / 2TB host a serial run wastes hours. Cases are independent
# and deterministic (fixed OmniRetarget seed + SPIDER preprocess), so sharding
# the manifest across N parallel runners changes only wall-clock, not results.
#
# Each shard runner writes per-case outputs keyed by case_id (no cross-shard
# collision) and persists status into its own shard manifest; we then merge the
# shard rows back into the canonical manifest for downstream S4/S5.
#
# Usage:
#   VARIANT=omnirt_v1 NSHARDS=24 bash .../run_E173_stage2b_parallel.sh
#   VARIANT=omnirt_v2 ... (for rescue manifest)
set -euo pipefail
...
QUEUE=workspace/core4d/scripts/experiments/E173/run_stage2b_queue.py
[ -f "$MANIFEST" ] || { echo "manifest missing: $MANIFEST" >&2; exit 2; }
mkdir -p "$SHARD_ROOT" "logs/E173/stage2b/$VARIANT"
...
for sd in "$SHARD_ROOT"/shard*/; do
  ...
  [ -f "$sm" ] || continue
  sid=$(basename "$sd")
  log="logs/E173/stage2b/$VARIANT/${sid}.log"
...
import collections
print(f"merged_rows={merged} status={dict(collections.Counter(r.get('stage2b_status','') for r in rows))}")
PYEOF
echo "=== done: $VARIANT ==="
