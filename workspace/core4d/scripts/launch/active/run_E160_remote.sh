#!/usr/bin/env bash
# E160 remote launcher for 3-case gateA + surfaceBand-A2 + postureRerankA rows.
set -euo pipefail
...
SESSION="${SESSION:-E160_${STAGE}_$(date +%H%M%S)}"
PYTHON_BIN="${PYTHON_BIN:-.venv/bin/python}"
REMOTE_GPU0_WORK="${REMOTE_GPU0_WORK:-}"
REMOTE_GPU1_WORK="${REMOTE_GPU1_WORK:-}"
WAIT_FOR_GPU_IDLE="${WAIT_FOR_GPU_IDLE:-0}"
E160_GPU_IDLE_MAX_MEM_MB="${E160_GPU_IDLE_MAX_MEM_MB:-3000}"
E160_GPU_IDLE_MAX_UTIL_PCT="${E160_GPU_IDLE_MAX_UTIL_PCT:-20}"
E160_GPU_IDLE_POLL_SEC="${E160_GPU_IDLE_POLL_SEC:-120}"
E160_GPU_IDLE_STABLE_POLLS="${E160_GPU_IDLE_STABLE_POLLS:-2}"
...
"$PYTHON_BIN" workspace/core4d/scripts/experiments/E160/build_posture_rerank_manifest.py >/dev/null

retry() {
  local attempt=1
  local max_attempts="${E160_REMOTE_RETRY_MAX:-4}"
  while true; do
    if "$@"; then
      return 0
    fi
...
rows = list(csv.DictReader(open("workspace/core4d/scripts/experiments/E160/variants.tsv", newline=""), delimiter="\t"))
...
echo "Syncing E160 artifacts to ${REMOTE}:${REMOTE_ROOT}"
retry ssh "$REMOTE" "mkdir -p \
  '$REMOTE_ROOT/spider/simulators' \
  '$REMOTE_ROOT/spider/optimizers' \
  '$REMOTE_ROOT/examples/config/override' \
  '$REMOTE_ROOT/workspace/core4d/scripts/experiments/E160' \
  '$REMOTE_ROOT/workspace/core4d/scripts/launch/active' \
  '$REMOTE_ROOT/workspace/core4d/results/E160/posture_rerank/cem/${STAGE}' \
  '$REMOTE_ROOT/logs/E160/cem/${STAGE}'"
...
retry rsync -az workspace/core4d/scripts/experiments/E160/ "${REMOTE}:${REMOTE_ROOT}/workspace/core4d/scripts/experiments/E160/"
retry rsync -az workspace/core4d/scripts/launch/active/run_E160_local.sh "${REMOTE}:${REMOTE_ROOT}/workspace/core4d/scripts/launch/active/run_E160_local.sh"
retry rsync -az examples/config/override/core4d_E160_*.yaml "${REMOTE}:${REMOTE_ROOT}/examples/config/override/"
...
retry ssh "$REMOTE" "cd '$REMOTE_ROOT' && \
  .venv/bin/python -m py_compile \
    ...
    workspace/core4d/scripts/experiments/E160/build_posture_rerank_manifest.py && \
  bash -n workspace/core4d/scripts/launch/active/run_E160_local.sh && \
  echo preflight-ok"

retry ssh "$REMOTE" "cd '$REMOTE_ROOT' && \
  tmux new-session -d -s '$SESSION' \"bash -lc 'set -euo pipefail; \
    echo E160 remote session $SESSION stage=$STAGE; \
    (WAIT_FOR_GPU_IDLE=$WAIT_FOR_GPU_IDLE E160_GPU_IDLE_MAX_MEM_MB=$E160_GPU_IDLE_MAX_MEM_MB E160_GPU_IDLE_MAX_UTIL_PCT=$E160_GPU_IDLE_MAX_UTIL_PCT E160_GPU_IDLE_POLL_SEC=$E160_GPU_IDLE_POLL_SEC E160_GPU_IDLE_STABLE_POLLS=$E160_GPU_IDLE_STABLE_POLLS E160_SPLIT=remote-gpu0 CASE_METHODS=\\\"$REMOTE_GPU0_WORK\\\" LOCAL_GPU=0 bash workspace/core4d/scripts/launch/active/run_E160_local.sh $STAGE) & PID0=\\\$!; \
    (WAIT_FOR_GPU_IDLE=$WAIT_FOR_GPU_IDLE E160_GPU_IDLE_MAX_MEM_MB=$E160_GPU_IDLE_MAX_MEM_MB E160_GPU_IDLE_MAX_UTIL_PCT=$E160_GPU_IDLE_MAX_UTIL_PCT E160_GPU_IDLE_POLL_SEC=$E160_GPU_IDLE_POLL_SEC E160_GPU_IDLE_STABLE_POLLS=$E160_GPU_IDLE_STABLE_POLLS E160_SPLIT=remote-gpu1 CASE_METHODS=\\\"$REMOTE_GPU1_WORK\\\" LOCAL_GPU=1 bash workspace/core4d/scripts/launch/active/run_E160_local.sh $STAGE) & PID1=\\\$!; \
    echo launched PID0=\\\$PID0 PID1=\\\$PID1; \
    wait \\\$PID0; echo remote-gpu0 done; \
    wait \\\$PID1; echo remote-gpu1 done; \
    echo E160 remote complete'\""
