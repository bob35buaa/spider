#!/usr/bin/env bash
# E159 local runner for clean6 gateA + surfaceBand-A2 CEM rows.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

STAGE="${1:-full}"
LOCAL_GPU="${LOCAL_GPU:-0}"
E159_SPLIT="${E159_SPLIT:-local-gpu0}"
PYTHON_BIN="${PYTHON_BIN:-.venv/bin/python}"
VARIANTS="workspace/core4d/scripts/experiments/E159/variants.tsv"
RESULTS="workspace/core4d/results/E159/gate_surface_band_A2/cem/${STAGE}"
LOGS="logs/E159/cem/${STAGE}"
CASE_METHODS="${CASE_METHODS:-}"
WAIT_FOR_GPU_IDLE="${WAIT_FOR_GPU_IDLE:-0}"
GPU_IDLE_MAX_MEM_MB="${E159_GPU_IDLE_MAX_MEM_MB:-3000}"
GPU_IDLE_MAX_UTIL_PCT="${E159_GPU_IDLE_MAX_UTIL_PCT:-20}"
GPU_IDLE_POLL_SEC="${E159_GPU_IDLE_POLL_SEC:-120}"
GPU_IDLE_STABLE_POLLS="${E159_GPU_IDLE_STABLE_POLLS:-2}"
...
selected_rows() {
  "$PYTHON_BIN" - "$VARIANTS" "$E159_SPLIT" "$CASE_METHODS" <<'PY'
import csv
import sys
...
with variants.open("r", encoding="utf-8", newline="") as f:
    for row in csv.DictReader(f, delimiter="\t"):
        if row["run_status"] != "to_run":
            continue
        if specs:
            if row["short_case_id"] not in specs:
                continue
...
run_one() {
  ...
  mkdir -p "$out_dir"
  echo "[$(date '+%H:%M:%S')] === START ${variant} case=${case_id} GPU=${LOCAL_GPU} stage=${STAGE} ==="
...
echo "=== E159 ${STAGE} local runner split=${E159_SPLIT} GPU=${LOCAL_GPU} ==="
if [ -n "$CASE_METHODS" ]; then
  echo "CASE_METHODS=${CASE_METHODS}"
fi

row_count=0
while IFS=$'\t' read -r variant case_id task override min_sdf max_viol hard_floor; do
  row_count=$((row_count + 1))
  run_one "$variant" "$case_id" "$task" "$override" "$min_sdf" "$max_viol" "$hard_floor"
done < <(selected_rows)

echo "=== E159 ${STAGE} local runner complete; selected_rows=${row_count} ==="
