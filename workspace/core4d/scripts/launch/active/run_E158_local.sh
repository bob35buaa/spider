#!/usr/bin/env bash
# E158 local runner for clean6 gateA + surfaceBand-A CEM rows.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

STAGE="${1:-full}"
LOCAL_GPU="${LOCAL_GPU:-0}"
E158_SPLIT="${E158_SPLIT:-local-gpu0}"
PYTHON_BIN="${PYTHON_BIN:-.venv/bin/python}"
VARIANTS="workspace/core4d/scripts/experiments/E158/variants.tsv"
RESULTS="workspace/core4d/results/E158/gate_surface_band/cem/${STAGE}"
LOGS="logs/E158/cem/${STAGE}"
CASE_METHODS="${CASE_METHODS:-}"
WAIT_FOR_GPU_IDLE="${WAIT_FOR_GPU_IDLE:-0}"
GPU_IDLE_MAX_MEM_MB="${E158_GPU_IDLE_MAX_MEM_MB:-3000}"
GPU_IDLE_MAX_UTIL_PCT="${E158_GPU_IDLE_MAX_UTIL_PCT:-20}"
GPU_IDLE_POLL_SEC="${E158_GPU_IDLE_POLL_SEC:-120}"
GPU_IDLE_STABLE_POLLS="${E158_GPU_IDLE_STABLE_POLLS:-2}"
...
selected_rows() {
  "$PYTHON_BIN" - "$VARIANTS" "$E158_SPLIT" "$CASE_METHODS" <<'PY'
import csv
import sys
...
echo "=== E158 ${STAGE} local runner split=${E158_SPLIT} GPU=${LOCAL_GPU} ==="
...
echo "=== E158 ${STAGE} local runner complete; selected_rows=${row_count} ==="
