#!/usr/bin/env bash
# E165-D remote launcher for peak-margin CEM rerank.
set -euo pipefail
...
SESSION="${SESSION:-E165D_${STAGE}_$(date +%H%M%S)}"
PYTHON_BIN="${PYTHON_BIN:-.venv/bin/python}"
REMOTE_GPU0_WORK="${REMOTE_GPU0_WORK:-}"
REMOTE_GPU1_WORK="${REMOTE_GPU1_WORK:-}"
WAIT_FOR_GPU_IDLE="${WAIT_FOR_GPU_IDLE:-0}"
E165D_GPU_IDLE_MAX_MEM_MB="${E165D_GPU_IDLE_MAX_MEM_MB:-3000}"
E165D_GPU_IDLE_MAX_UTIL_PCT="${E165D_GPU_IDLE_MAX_UTIL_PCT:-20}"
E165D_GPU_IDLE_POLL_SEC="${E165D_GPU_IDLE_POLL_SEC:-120}"
E165D_GPU_IDLE_STABLE_POLLS="${E165D_GPU_IDLE_STABLE_POLLS:-2}"
...
retry() {
  local attempt=1
  local max_attempts="${E165D_REMOTE_RETRY_MAX:-4}"
  while true; do
    if "$@"; then
      return 0
    fi
...
rows = list(csv.DictReader(open("workspace/core4d/scripts/experiments/E165/variants.tsv", newline=""), delimiter="\t"))
...
retry ssh "$REMOTE" "mkdir -p \
  ...
  '$REMOTE_ROOT/workspace/core4d/results/E165/peak_margin_rerank/cem/${STAGE}' \
  '$REMOTE_ROOT/logs/E165D/cem/${STAGE}'"
...
retry rsync -az workspace/core4d/scripts/experiments/E165/ "${REMOTE}:${REMOTE_ROOT}/workspace/core4d/scripts/experiments/E165/"
...
retry rsync -az workspace/core4d/scripts/launch/active/run_E165D_local.sh "${REMOTE}:${REMOTE_ROOT}/workspace/core4d/scripts/launch/active/run_E165D_local.sh"
retry rsync -az examples/config/override/core4d_E165D_*.yaml "${REMOTE}:${REMOTE_ROOT}/examples/config/override/"
...
mapfile -t BASELINE_PATHS < <("$PYTHON_BIN" - <<'PY'
import csv
rows = list(csv.DictReader(open("workspace/core4d/scripts/experiments/E165/variants.tsv", newline=""), delimiter="\t"))
...
retry ssh "$REMOTE" "cd '$REMOTE_ROOT' && \
  .venv/bin/python -m py_compile \
    ...
    workspace/core4d/scripts/experiments/E165/build_peak_margin_manifest.py && \
  bash -n workspace/core4d/scripts/launch/active/run_E165D_local.sh && \
  echo preflight-ok"

retry ssh "$REMOTE" "cd '$REMOTE_ROOT' && \
  tmux new-session -d -s '$SESSION' \"bash -lc 'set -euo pipefail; \
    echo E165D remote session $SESSION stage=$STAGE; \
    (WAIT_FOR_GPU_IDLE=$WAIT_FOR_GPU_IDLE E165D_GPU_IDLE_MAX_MEM_MB=$E165D_GPU_IDLE_MAX_MEM_MB E165D_GPU_IDLE_MAX_UTIL_PCT=$E165D_GPU_IDLE_MAX_UTIL_PCT E165D_GPU_IDLE_POLL_SEC=$E165D_GPU_IDLE_POLL_SEC E165D_GPU_IDLE_STABLE_POLLS=$E165D_GPU_IDLE_STABLE_POLLS E165D_SPLIT=remote-gpu0 CASE_METHODS=\\\"$REMOTE_GPU0_WORK\\\" LOCAL_GPU=0 bash workspace/core4d/scripts/launch/active/run_E165D_local.sh $STAGE) & PID0=\\\$!; \
    (WAIT_FOR_GPU_IDLE=$WAIT_FOR_GPU_IDLE E165D_GPU_IDLE_MAX_MEM_MB=$E165D_GPU_IDLE_MAX_MEM_MB E165D_GPU_IDLE_MAX_UTIL_PCT=$E165D_GPU_IDLE_MAX_UTIL_PCT E165D_GPU_IDLE_POLL_SEC=$E165D_GPU_IDLE_POLL_SEC E165D_GPU_IDLE_STABLE_POLLS=$E165D_GPU_IDLE_STABLE_POLLS E165D_SPLIT=remote-gpu1 CASE_METHODS=\\\"$REMOTE_GPU1_WORK\\\" LOCAL_GPU=1 bash workspace/core4d/scripts/launch/active/run_E165D_local.sh $STAGE) & PID1=\\\$!; \
    echo launched PID0=\\\$PID0 PID1=\\\$PID1; \
    wait \\\$PID0; echo remote-gpu0 done; \
    wait \\\$PID1; echo remote-gpu1 done; \
    echo E165D remote complete'\""
