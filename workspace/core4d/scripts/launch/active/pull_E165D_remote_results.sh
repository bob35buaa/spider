#!/usr/bin/env bash
# Pull E165-D remote CEM results/logs back to local workspace.
set -euo pipefail
...
echo "Pulled E165D ${STAGE} results to ${RESULTS}"
find "$RESULTS" -maxdepth 1 -name 'E165D_*.npz' | sort | wc -l | awk '{print "root_npz_count="$1}'
find "$RESULTS" -maxdepth 1 -name 'E165D_*_'"${STAGE}"'.mp4' | sort | wc -l | awk '{print "video_count="$1}'
