#!/usr/bin/env bash
# Launch E085 remote split without requiring a clean git tree.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

REMOTE_HOST="${REMOTE_HOST:-spider-remote}"
REMOTE_REPO="${REMOTE_REPO:-/home/xiayb/pHRI_workspace/spider}"
REMOTE_SESSION="${REMOTE_SESSION:-E085_rawtarget_gate}"
SSH_OPTS=(
  -o BatchMode=yes
  -o ConnectTimeout=20
...
echo "[$(date '+%H:%M:%S')] rsync E085 code/data to ${REMOTE_HOST}:${REMOTE_REPO}"
rsync -avR \
  spider/config.py \
  ...
  examples/config/override/core4d_E085A_rawtarget_guard.yaml \
...
echo "[$(date '+%H:%M:%S')] launching remote E085 session"
ssh "${SSH_OPTS[@]}" "$REMOTE_HOST" "cd '$REMOTE_REPO' && mkdir -p logs/E085 workspace/core4d/results/E085 && tmux new-session -d -s '$REMOTE_SESSION' 'bash workspace/core4d/scripts/E085/run_remote_inside.sh'"

echo "Remote launched."
echo "Monitor:"
echo "  ssh ${REMOTE_HOST} \"tmux capture-pane -t ${REMOTE_SESSION} -p | tail -40\""
echo "Pull results when done:"
echo "  REMOTE_HOST=${REMOTE_HOST} REMOTE_REPO=${REMOTE_REPO} bash workspace/core4d/scripts/pull_E085_remote_results.sh"
