#!/usr/bin/env bash
# E096b: mask-on full CEM rerun for E096 P1/P2. Does not manage/stop other jobs.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

MODE="${1:-local}"       # list | local | remote-gpu0 | single | eval
STAGE="${2:-full}"       # smoke | full
GPU="${3:-0}"
VARIANTS_FILE="${VARIANTS_FILE:-workspace/core4d/scripts/E096b/variants.tsv}"
RESULTS="${RESULTS:-workspace/core4d/results/E096b/cem/${STAGE}}"
LOGS="${LOGS:-logs/E096b/cem/${STAGE}}"
...
variant_row() {
  local variant=$1
  awk -F '\t' -v v="$variant" 'NF && $1 !~ /^#/ && $1 == "cem_ref_fk" && $3 == v {print; exit}' "$VARIANTS_FILE"
}
...
variants_for_split() {
  local split_name=$1
  awk -F '\t' -v want="$split_name" 'NF && $1 !~ /^#/ && $1 == "cem_ref_fk" && $7 == want && tolower($11) == "true" {print $3}' "$VARIANTS_FILE"
}
...
snapshot_variants() {
  ...
  for variant in "${variants[@]}"; do
    ...
    [ -n "$row" ] || { echo "Unknown E096b variant: $variant" >&2; exit 2; }
    task="$(field_from_row "$row" 5)"
    tasks+=("$task")
  done
  if [ "${#tasks[@]}" -gt 0 ]; then
    echo "[$(date '+%H:%M:%S')] === E096b scene snapshot ${STAGE} ==="
    bash workspace/core4d/scripts/convert/snapshot_scenes.sh E096b "${tasks[@]}" > "$LOGS/snapshot_${MODE}_${STAGE}.log" 2>&1
    for task in "${tasks[@]}"; do
      meta="example_datasets/processed/core4d/unitree_g1/humanoid_object/${task}/upper_object_collision_meta.json"
      if [ -f "$meta" ]; then
        mkdir -p "workspace/core4d/results/E096b/scene_snapshot/$task"
        cp "$meta" "workspace/core4d/results/E096b/scene_snapshot/$task/upper_object_collision_meta.json"
      fi
    done
  fi
}
...
run_one() {
  ...
  row="$(variant_row "$variant")"
  if [ -z "$row" ]; then
    echo "Unknown E096b variant: $variant" >&2
    return 2
  fi
  task="$(field_from_row "$row" 5)"
  ...
  echo "[$(date '+%H:%M:%S')] === E096b ${STAGE} ${variant} task=${task} GPU=${gpu} ==="
  if [ "$STAGE" = "smoke" ]; then
  ...
  echo "[$(date '+%H:%M:%S')] === E096b ${STAGE} ${variant} done ==="
}
...
eval_variants() {
  ...
  RESULTS="$RESULTS" VARIANTS_FILE="$VARIANTS_FILE" .venv/bin/python \
    workspace/core4d/scripts/eval/eval_E096_cem.py --stage "$STAGE" "${variants[@]}" \
    | tee "$LOGS/eval_${MODE}_${STAGE}.log"
  if [ -f "$RESULTS/${STAGE}_eval_summary.md" ]; then
    perl -pi -e 's/# E096 full CEM/# E096b mask-on full CEM/' "$RESULTS/${STAGE}_eval_summary.md"
  fi
}
...
case "$MODE" in
  ...
  local|remote-gpu0)
    ...
    if [ "${#variants[@]}" -eq 0 ]; then
      echo "No E096b variants for split=${MODE}; nothing to run."
      exit 0
    fi
  ...
  eval)
    shift 3 || true
    RESULTS="$RESULTS" VARIANTS_FILE="$VARIANTS_FILE" .venv/bin/python \
      workspace/core4d/scripts/eval/eval_E096_cem.py --stage "$STAGE" "$@" \
      | tee "$LOGS/eval_E096b_cem_${STAGE}.log"
    if [ -f "$RESULTS/${STAGE}_eval_summary.md" ]; then
      perl -pi -e 's/# E096 full CEM/# E096b mask-on full CEM/' "$RESULTS/${STAGE}_eval_summary.md"
    fi
    ;;
...
echo "=== E096b CEM ${MODE} ${STAGE} done ==="
