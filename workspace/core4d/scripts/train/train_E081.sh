#!/usr/bin/env bash
# E081: leg/foot-object collision CEM runs.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

MODE="${1:-local}"
GPU="${2:-0}"
VARIANTS_FILE="${VARIANTS_FILE:-workspace/core4d/scripts/E081/variants.tsv}"
RESULTS="${RESULTS:-workspace/core4d/results/E081}"
LOGS="${LOGS:-logs/E081}"
...
snapshot_split() {
  ...
  if [ "${#tasks[@]}" -gt 0 ]; then
    ...
    bash workspace/core4d/scripts/convert/snapshot_scenes.sh E081 "${tasks[@]}"
...
run_one() {
  ...
  if [ -z "$row" ]; then
    echo "Unknown E081 variant: $variant" >&2
    return 2
  fi
...
case "$MODE" in
  single)
    ...
    run_one "$VARIANT" "$GPU"
    .venv/bin/python workspace/core4d/scripts/eval/eval_E081.py "$VARIANT" | tee "$LOGS/eval_${VARIANT}.log"
    ;;
  local|remote)
    snapshot_split "$MODE"
    mapfile -t variants < <(variants_for_split "$MODE")
    if [ "${#variants[@]}" -eq 0 ]; then
      echo "No E081 variants for split=${MODE}" >&2
      exit 2
    fi
    for variant in "${variants[@]}"; do
      run_one "$variant" "$GPU"
    done
    .venv/bin/python workspace/core4d/scripts/eval/eval_E081.py "${variants[@]}" | tee "$LOGS/eval_${MODE}.log"
    ;;
  eval)
    shift || true
    .venv/bin/python workspace/core4d/scripts/eval/eval_E081.py "$@" | tee "$LOGS/eval_E081.log"
    ;;
  *)
    echo "Usage:"
    echo "  $0 local 0"
    echo "  $0 remote 1"
    echo "  $0 single 0 E081_box025_p2_legobj"
    echo "  $0 eval [variant ...]"
    exit 2
    ;;
esac
