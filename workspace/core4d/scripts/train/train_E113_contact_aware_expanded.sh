#!/usr/bin/env bash
# E113 Phase A: hold-band contact-aware CEM expanded workset.
set -euo pipefail
...
GPU="${3:-0}"
VARIANTS_FILE="${VARIANTS_FILE:-workspace/core4d/scripts/E113/variants.tsv}"
RESULTS="${RESULTS:-workspace/core4d/results/E113/cem/${STAGE}}"
LOGS="${LOGS:-logs/E113/cem/${STAGE}}"
...
require_manifest_ready() {
  if [ ! -f "$VARIANTS_FILE" ]; then
    echo "Missing E113 variants file: $VARIANTS_FILE. Run build_expanded_contact_manifest.py first." >&2
    exit 2
  fi
  local count
  count="$(awk -F '\t' 'NF && $1 !~ /^#/ {n++} END {print n+0}' "$VARIANTS_FILE")"
  if [ "$count" -ne 6 ]; then
    echo "E113 variants.tsv has $count rows, expected 6. Rebuild the manifest." >&2
    exit 2
  fi
  local preflight="workspace/core4d/results/E113/preflight/phaseA_preflight.tsv"
  if [ ! -f "$preflight" ]; then
    echo "Missing E113 preflight: $preflight. Rebuild the manifest." >&2
    exit 2
  fi
  if awk -F '\t' 'NF && $1 !~ /^#/ && $0 ~ /False/ {bad=1} END {exit bad ? 1 : 0}' "$preflight"; then
    :
  else
    echo "E113 preflight contains a False field: $preflight" >&2
    exit 2
  fi
}

snapshot_variants() {
  [ "${E113_SKIP_SCENE_SNAPSHOT:-0}" = "1" ] && return 0
  local variants=("$@")
  ...
  bash workspace/core4d/scripts/convert/snapshot_scenes.sh E113 "${tasks[@]}"
}
...
run_one() {
  ...
  row="$(variant_row "$variant")"
  if [ -z "$row" ]; then
    echo "Unknown E113 variant: $variant" >&2
    return 2
  fi
  task="$(field_from_row "$row" 4)"
  ...
  mkdir -p "$out_dir"
  echo "[$(date '+%H:%M:%S')] === E113 ${STAGE} ${variant} task=${task} GPU=${gpu} ==="
  if [ "$STAGE" = "smoke" ]; then
  ...
  echo "[$(date '+%H:%M:%S')] === E113 ${STAGE} ${variant} done ==="
}
...
case "$MODE" in
  ...
  local|remote-gpu0|remote-gpu1)
    ...
    if [ "${#variants[@]}" -eq 0 ]; then
      echo "No E113 variants for split=${split}; nothing to run."
      exit 0
    fi
    echo "E113 split=${split} variants=${#variants[@]} stage=${STAGE} gpu=${GPU}"
  ...
  eval)
    shift 3 || true
    bash workspace/core4d/scripts/eval/eval_E113_contact_aware_expanded.sh "$STAGE" "$@"
    ;;
  *)
