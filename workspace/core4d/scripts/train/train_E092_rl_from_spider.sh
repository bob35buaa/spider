#!/usr/bin/env bash
# E092 Stage B: training from Stage A SPIDER dynamic WORK references.
set -euo pipefail
...
GPU="${3:-0}"
VARIANTS_FILE="${VARIANTS_FILE:-workspace/core4d/scripts/E092/variants_rl_spider.tsv}"
RESULTS="${RESULTS:-workspace/core4d/results/E092/rl_from_spider/${STAGE}}"
LOGS="${LOGS:-logs/E092/rl_from_spider/${STAGE}}"
...
if [ ! -f "$VARIANTS_FILE" ]; then
  echo "Missing $VARIANTS_FILE. Run workspace/core4d/scripts/E092/build_rl_tasks.py after Stage A full WORK results." >&2
  exit 2
fi

variant_row() {
  local variant=$1
  awk -F '\t' -v v="$variant" 'NF && $1 !~ /^#/ && $1 == "rl_spider" && $3 == v {print; exit}' "$VARIANTS_FILE"
}
...
variants_for_split() {
  local split_name=$1
  awk -F '\t' -v want="$split_name" 'NF && $1 !~ /^#/ && $1 == "rl_spider" && $7 == want {print $3}' "$VARIANTS_FILE"
}
...
snapshot_variants() {
  ...
  for variant in "${variants[@]}"; do
    ...
    [ -n "$row" ] || { echo "Unknown E092 rl_spider variant: $variant" >&2; exit 2; }
    task="$(field_from_row "$row" 5)"
    tasks+=("$task")
  done
  if [ "${#tasks[@]}" -gt 0 ]; then
    echo "[$(date '+%H:%M:%S')] === E092 scene snapshot rl_spider ${STAGE} ==="
...
run_one() {
  ...
  row="$(variant_row "$variant")"
  if [ -z "$row" ]; then
    echo "Unknown E092 rl_spider variant: $variant" >&2
    return 2
  fi
  task="$(field_from_row "$row" 5)"
  ...
  mkdir -p "$out_dir"
  echo "[$(date '+%H:%M:%S')] === E092 rl_spider ${STAGE} ${variant} task=${task} GPU=${gpu} ==="
  if [ "$STAGE" = "smoke" ]; then
  ...
  echo "[$(date '+%H:%M:%S')] === E092 rl_spider ${STAGE} ${variant} done ==="
}
...
eval_variants() {
  ...
  RESULTS="$RESULTS" VARIANTS_FILE="$VARIANTS_FILE" .venv/bin/python \
    workspace/core4d/scripts/eval/eval_E092_rl.py --route rl_spider --stage "$STAGE" "${variants[@]}" \
...
case "$MODE" in
  ...
  local|remote-gpu0|remote-gpu1)
    ...
    if [ "${#variants[@]}" -eq 0 ]; then
      echo "No E092 rl_spider variants for split=${MODE}; nothing to run."
      exit 0
    fi
  ...
  eval)
    shift 3 || true
    RESULTS="$RESULTS" VARIANTS_FILE="$VARIANTS_FILE" .venv/bin/python \
      workspace/core4d/scripts/eval/eval_E092_rl.py --route rl_spider --stage "$STAGE" "$@" \
      | tee "$LOGS/eval_E092_rl_spider_${STAGE}.log"
    ;;
  *)
...
echo "=== E092 rl_from_spider ${MODE} ${STAGE} done ==="
