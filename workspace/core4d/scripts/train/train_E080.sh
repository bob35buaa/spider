#!/usr/bin/env bash
# E080: box025 boundary-control CEM runs.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

MODE="${1:-local}"
GPU="${2:-0}"
VARIANTS_FILE="${VARIANTS_FILE:-workspace/core4d/scripts/E080/variants.tsv}"
RESULTS="${RESULTS:-workspace/core4d/results/E080}"
LOGS="${LOGS:-logs/E080}"
...
snapshot_split() {
  ...
  if [ "${#tasks[@]}" -gt 0 ]; then
    ...
    bash workspace/core4d/scripts/convert/snapshot_scenes.sh E080 "${tasks[@]}"
  fi
}
...
run_one() {
  ...
  if [ -z "$row" ]; then
    echo "Unknown E080 variant: $variant" >&2
    return 2
  fi
...
case "$MODE" in
  single)
    ...
    run_one "$VARIANT" "$GPU"
    .venv/bin/python workspace/core4d/scripts/eval/eval_E080.py "$VARIANT" | tee "$LOGS/eval_${VARIANT}.log"
    ;;
  local|remote)
    snapshot_split "$MODE"
    mapfile -t variants < <(variants_for_split "$MODE")
    if [ "${#variants[@]}" -eq 0 ]; then
      echo "No E080 variants for split=${MODE}" >&2
      exit 2
    fi
    for variant in "${variants[@]}"; do
      run_one "$variant" "$GPU"
    done
    .venv/bin/python workspace/core4d/scripts/eval/eval_E080.py "${variants[@]}" | tee "$LOGS/eval_${MODE}.log"
    ;;
  eval)
    shift || true
    .venv/bin/python workspace/core4d/scripts/eval/eval_E080.py "$@" | tee "$LOGS/eval_E080.log"
    ;;
  *)
    echo "Usage:"
    echo "  $0 local 0"
    echo "  $0 remote 1"
    echo "  $0 single 0 E080_box025_p1"
    echo "  $0 eval [variant ...]"
    exit 2
    ;;
esac
