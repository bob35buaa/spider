#!/usr/bin/env bash
# E118 Phase A: carry-corridor soft-gate diagnostic CEM.
set -euo pipefail
...
GPU="${3:-0}"
VARIANTS_FILE="${VARIANTS_FILE:-workspace/core4d/scripts/E118/variants.tsv}"
RESULTS="${RESULTS:-workspace/core4d/results/E118/cem/${STAGE}}"
LOGS="${LOGS:-logs/E118/cem/${STAGE}}"
...
require_manifest_ready() {
  if [ ! -f "$VARIANTS_FILE" ]; then
    echo "Missing E118 variants file: $VARIANTS_FILE. Run build_carry_corridor_manifest.py first." >&2
    exit 2
  fi
  local count
  count="$(awk -F '\t' 'NF && $1 !~ /^#/ {n++} END {print n+0}' "$VARIANTS_FILE")"
  if [ "$count" -ne 15 ]; then
    echo "E118 variants.tsv has $count rows, expected 15. Rebuild the manifest." >&2
    exit 2
  fi
  local preflight="workspace/core4d/results/E118/preflight/phaseA_preflight.tsv"
  if [ ! -f "$preflight" ]; then
    echo "Missing E118 preflight: $preflight. Rebuild the manifest." >&2
    exit 2
  fi
  if awk -F '\t' 'NF && $1 !~ /^#/ {for (i=19; i<=26; i++) if ($i != "True") bad=1; if ($27 == "True" && $28 != "True") bad=1} END {exit bad ? 1 : 0}' "$preflight"; then
    :
  else
    echo "E118 preflight contains a failed required asset check: $preflight" >&2
    exit 2
  fi
}

snapshot_variants() {
  [ "${E118_SKIP_SCENE_SNAPSHOT:-0}" = "1" ] && return 0
  local variants=("$@")
  ...
  bash workspace/core4d/scripts/convert/snapshot_scenes.sh E118 "${tasks[@]}"
}
...
run_one() {
  ...
  row="$(variant_row "$variant")"
  if [ -z "$row" ]; then
    echo "Unknown E118 variant: $variant" >&2
    return 2
  fi
  task="$(field_from_row "$row" 4)"
  ...
  if [ -f "$RESULTS/${variant}.npz" ] && [ -f "$RESULTS/${variant}_${STAGE}.mp4" ] && [ -f "$out_dir/trajectory_mjwp_act.npz" ]; then
    echo "[$(date '+%H:%M:%S')] === E118 ${STAGE} ${variant} already complete; skip ==="
    return 0
  fi
  echo "[$(date '+%H:%M:%S')] === E118 ${STAGE} ${variant} task=${task} GPU=${gpu} ==="
  if [ "$STAGE" = "smoke" ]; then
  ...
  echo "[$(date '+%H:%M:%S')] === E118 ${STAGE} ${variant} done ==="
}
...
case "$MODE" in
  ...
  local|remote-gpu0|remote-gpu1)
    ...
    if [ "${#variants[@]}" -eq 0 ]; then
      echo "No E118 variants for split=${split}; nothing to run."
      exit 0
    fi
    echo "E118 split=${split} variants=${#variants[@]} stage=${STAGE} gpu=${GPU}"
  ...
  eval)
    shift 3 || true
    bash workspace/core4d/scripts/eval/eval_E118_carry_corridor.sh "$STAGE" "$@"
    ;;
  *)
