#!/usr/bin/env bash
# E086: raw-target failure-iteration runs.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

MODE="${1:-local}"
GPU="${2:-0}"
VARIANTS_FILE="${VARIANTS_FILE:-workspace/core4d/scripts/E086/variants.tsv}"
RESULTS="${RESULTS:-workspace/core4d/results/E086}"
LOGS="${LOGS:-logs/E086}"
...
run_one() {
  ...
  if [ -z "$row" ]; then
    echo "Unknown E086 variant: $variant" >&2
    return 2
  fi
...
case "$MODE" in
  ...
  local)
    ...
    RESULTS="$RESULTS" VARIANTS_FILE="$VARIANTS_FILE" .venv/bin/python workspace/core4d/scripts/eval/eval_E086.py "${variants[@]}" | tee "$LOGS/eval_${MODE}.log"
    ;;
  single)
    VARIANT="${3:-}"
    if [ -z "$VARIANT" ]; then
    ...
    run_one "$VARIANT" "$GPU"
    RESULTS="$RESULTS" VARIANTS_FILE="$VARIANTS_FILE" .venv/bin/python workspace/core4d/scripts/eval/eval_E086.py "$VARIANT" | tee "$LOGS/eval_${VARIANT}.log"
    ;;
  eval)
    shift || true
    RESULTS="$RESULTS" VARIANTS_FILE="$VARIANTS_FILE" .venv/bin/python workspace/core4d/scripts/eval/eval_E086.py "$@" | tee "$LOGS/eval_E086.log"
    ;;
  *)
    echo "Usage: $0 list | smoke <gpu> [variant ...] | local 0 | single 0 <variant> | eval [variant ...]" >&2
    exit 2
    ;;
esac
