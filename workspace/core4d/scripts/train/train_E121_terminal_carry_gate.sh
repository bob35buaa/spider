#!/usr/bin/env bash
# E121 Phase A: terminal carry gate diagnostic CEM.
set -euo pipefail
...
GPU="${3:-0}"
VARIANTS_FILE="${VARIANTS_FILE:-workspace/core4d/scripts/E121/variants.tsv}"
RESULTS="${RESULTS:-workspace/core4d/results/E121/cem/${STAGE}}"
LOGS="${LOGS:-logs/E121/cem/${STAGE}}"
...
require_manifest_ready() {
  if [ ! -f "$VARIANTS_FILE" ]; then
    echo "Missing E121 variants file: $VARIANTS_FILE. Run build_terminal_carry_gate_manifest.py first." >&2
    exit 2
  fi
  local count
  count="$(awk -F '\t' 'NF && $1 !~ /^#/ {n++} END {print n+0}' "$VARIANTS_FILE")"
  if [ "$count" -ne 12 ]; then
    echo "E121 variants.tsv has $count rows, expected 12. Rebuild the manifest." >&2
    exit 2
  fi
  local preflight="workspace/core4d/results/E121/preflight/phaseA_preflight.tsv"
  local summary="workspace/core4d/results/E121/preflight/phaseA_manifest_summary.json"
  if [ ! -f "$preflight" ]; then
    echo "Missing E121 preflight: $preflight. Rebuild the manifest." >&2
    exit 2
  fi
  if [ ! -f "$summary" ]; then
    echo "Missing E121 preflight summary: $summary. Rebuild the manifest." >&2
    exit 2
  fi
...
summary = Path("workspace/core4d/results/E121/preflight/phaseA_manifest_summary.json")
data = json.loads(summary.read_text())
if not data.get("all_preflight_ok"):
    raise SystemExit("E121 preflight summary all_preflight_ok is false")
PY
}

snapshot_variants() {
  [ "${E121_SKIP_SCENE_SNAPSHOT:-0}" = "1" ] && return 0
  local variants=("$@")
  ...
  bash workspace/core4d/scripts/convert/snapshot_scenes.sh E121 "${tasks[@]}"
}
...
run_one() {
  ...
  row="$(variant_row "$variant")"
  if [ -z "$row" ]; then
    echo "Unknown E121 variant: $variant" >&2
    return 2
  fi
  task="$(field_from_row "$row" 4)"
  ...
  if [ -f "$RESULTS/${variant}.npz" ] && [ -f "$RESULTS/${variant}_${STAGE}.mp4" ] && [ -f "$out_dir/trajectory_mjwp_act.npz" ]; then
    echo "[$(date '+%H:%M:%S')] === E121 ${STAGE} ${variant} already complete; skip ==="
    return 0
  fi
  echo "[$(date '+%H:%M:%S')] === E121 ${STAGE} ${variant} task=${task} GPU=${gpu} ==="
  if [ "$STAGE" = "smoke" ]; then
  ...
  echo "[$(date '+%H:%M:%S')] === E121 ${STAGE} ${variant} done ==="
}
...
case "$MODE" in
  ...
  local|remote-gpu0|remote-gpu1)
    ...
    if [ "${#variants[@]}" -eq 0 ]; then
      echo "No E121 variants for split=${split}; nothing to run."
      exit 0
    fi
    echo "E121 split=${split} variants=${#variants[@]} stage=${STAGE} gpu=${GPU}"
  ...
  eval)
    shift 3 || true
    bash workspace/core4d/scripts/eval/eval_E121_terminal_carry_gate.sh "$STAGE" "$@"
    ;;
  *)
