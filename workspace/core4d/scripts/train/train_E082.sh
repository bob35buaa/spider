#!/usr/bin/env bash
# E082: E081-style leg/foot-object collision CEM for 3 D003 Box021 cases.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

MODE="${1:-local}"
GPU="${2:-0}"
VARIANTS_FILE="${VARIANTS_FILE:-workspace/core4d/scripts/E082/variants.tsv}"
RESULTS="${RESULTS:-workspace/core4d/results/E082}"
LOGS="${LOGS:-logs/E082}"
...
snapshot_split() {
  ...
  if [ "${#tasks[@]}" -gt 0 ]; then
    ...
    bash workspace/core4d/scripts/convert/snapshot_scenes.sh E082 "${tasks[@]}"
    for task in "${tasks[@]}"; do
      meta="example_datasets/processed/core4d/unitree_g1/humanoid_object/${task}/leg_object_collision_meta.json"
      if [ -f "$meta" ]; then
        cp "$meta" "$RESULTS/scene_snapshot/${task}/leg_object_collision_meta.json"
      fi
    done
  fi
}
...
run_one() {
  ...
  if [ -z "$row" ]; then
    echo "Unknown E082 variant: $variant" >&2
    return 2
  fi
...
case "$MODE" in
  ...
  single)
    ...
    run_one "$VARIANT" "$GPU"
    RESULTS="$RESULTS" VARIANTS_FILE="$VARIANTS_FILE" .venv/bin/python workspace/core4d/scripts/eval/eval_E082.py "$VARIANT" | tee "$LOGS/eval_${VARIANT}.log"
    ;;
  local|remote-gpu0|remote-gpu1)
    snapshot_split "$MODE"
    mapfile -t variants < <(variants_for_split "$MODE")
    if [ "${#variants[@]}" -eq 0 ]; then
      echo "No E082 variants for split=${MODE}" >&2
      exit 2
    fi
    for variant in "${variants[@]}"; do
      run_one "$variant" "$GPU"
    done
    RESULTS="$RESULTS" VARIANTS_FILE="$VARIANTS_FILE" .venv/bin/python workspace/core4d/scripts/eval/eval_E082.py "${variants[@]}" | tee "$LOGS/eval_${MODE}.log"
    ;;
  eval)
    shift || true
    RESULTS="$RESULTS" VARIANTS_FILE="$VARIANTS_FILE" .venv/bin/python workspace/core4d/scripts/eval/eval_E082.py "$@" | tee "$LOGS/eval_E082.log"
    ;;
  *)
    echo "Usage:"
    echo "  $0 list"
    ...
    echo "  $0 single 0 E082_d003_box021_20231018_029_p2_legobj"
    echo "  $0 eval [variant ...]"
    exit 2
    ;;
esac
