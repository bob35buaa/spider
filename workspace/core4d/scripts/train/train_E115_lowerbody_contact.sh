#!/usr/bin/env bash
# E115 Phase A: lower-body-aware contact diagnostic CEM.
set -euo pipefail
...
GPU="${3:-0}"
VARIANTS_FILE="${VARIANTS_FILE:-workspace/core4d/scripts/E115/variants.tsv}"
RESULTS="${RESULTS:-workspace/core4d/results/E115/cem/${STAGE}}"
LOGS="${LOGS:-logs/E115/cem/${STAGE}}"
...
require_manifest_ready() {
  if [ ! -f "$VARIANTS_FILE" ]; then
    echo "Missing E115 variants file: $VARIANTS_FILE. Run build_lowerbody_contact_manifest.py first." >&2
    exit 2
  fi
  local count
  count="$(awk -F '\t' 'NF && $1 !~ /^#/ {n++} END {print n+0}' "$VARIANTS_FILE")"
  if [ "$count" -ne 15 ]; then
    echo "E115 variants.tsv has $count rows, expected 15. Rebuild the manifest." >&2
    exit 2
  fi
  local preflight="workspace/core4d/results/E115/preflight/phaseA_preflight.tsv"
  if [ ! -f "$preflight" ]; then
    echo "Missing E115 preflight: $preflight. Rebuild the manifest." >&2
    exit 2
  fi
  if awk -F '\t' 'NF && $1 !~ /^#/ && $0 ~ /False/ {bad=1} END {exit bad ? 1 : 0}' "$preflight"; then
    :
  else
    echo "E115 preflight contains a False field: $preflight" >&2
    exit 2
  fi
}

snapshot_variants() {
  [ "${E115_SKIP_SCENE_SNAPSHOT:-0}" = "1" ] && return 0
  local variants=("$@")
  ...
  bash workspace/core4d/scripts/convert/snapshot_scenes.sh E115 "${tasks[@]}"
}
...
run_one() {
  ...
  row="$(variant_row "$variant")"
  if [ -z "$row" ]; then
    echo "Unknown E115 variant: $variant" >&2
    return 2
  fi
  task="$(field_from_row "$row" 4)"
  ...
  mkdir -p "$out_dir"
  echo "[$(date '+%H:%M:%S')] === E115 ${STAGE} ${variant} task=${task} GPU=${gpu} ==="
  if [ "$STAGE" = "smoke" ]; then
  ...
  echo "[$(date '+%H:%M:%S')] === E115 ${STAGE} ${variant} done ==="
}
...
case "$MODE" in
  ...
  local|remote-gpu0|remote-gpu1)
    ...
    if [ "${#variants[@]}" -eq 0 ]; then
      echo "No E115 variants for split=${split}; nothing to run."
      exit 0
    fi
    echo "E115 split=${split} variants=${#variants[@]} stage=${STAGE} gpu=${GPU}"
  ...
  eval)
    shift 3 || true
    bash workspace/core4d/scripts/eval/eval_E115_lowerbody_contact.sh "$STAGE" "$@"
    ;;
  *)
