#!/usr/bin/env bash
# E144: full-nonbox raw_mask_ref_fk CEM runner. Consumes workspace/core4d/scripts/E144/variants.tsv.
set -euo pipefail
...
GPU="${3:-0}"
VARIANTS_FILE="${VARIANTS_FILE:-workspace/core4d/scripts/E144/variants.tsv}"
RESULTS="${RESULTS:-workspace/core4d/results/E144/cem/${STAGE}}"
LOGS="${LOGS:-logs/E144/cem/${STAGE}}"
...
variants_for_split() {
  local split=$1
  awk -F '\t' -v want="$split" 'NF && $1 !~ /^#/ && NR > 2 && $8 == want && $21 != "already_done" {print $2}' "$VARIANTS_FILE"
}

require_manifest_ready() {
  if [ ! -f "$VARIANTS_FILE" ]; then
    echo "Missing E144 variants file: $VARIANTS_FILE. Run build_full_nonbox_raw_mask_ref_fk_cem_ready.py first." >&2
    exit 2
  fi
  local count
  count="$(awk -F '\t' 'NF && $1 !~ /^#/ && NR > 2 {n++} END {print n+0}' "$VARIANTS_FILE")"
  local preflight="workspace/core4d/results/E144/cem_ready/raw_mask_ref_fk_cem_ready_preflight.tsv"
  if [ ! -f "$preflight" ]; then
    echo "Missing E144 preflight: $preflight. Rebuild the manifest." >&2
    exit 2
  fi
  if awk -F '\t' 'NR == 1 {for (i=1;i<=NF;i++) if ($i=="preflight_ok") c=i; next} c && $c != "True" {bad=1} END {exit bad ? 1 : 0}' "$preflight"; then
    :
  else
    echo "E144 preflight contains non-True preflight_ok rows: $preflight" >&2
    exit 2
  fi
}

snapshot_variants() {
  [ "${E144_SKIP_SCENE_SNAPSHOT:-0}" = "1" ] && return 0
  local variants=("$@")
  ...
  bash workspace/core4d/scripts/convert/snapshot_scenes.sh E144 "${tasks[@]}"
}
...
run_one() {
  ...
  row="$(variant_row "$variant")"
  if [ -z "$row" ]; then
    echo "Unknown E144 variant: $variant" >&2
    return 2
  fi
  task="$(field_from_row "$row" 12)"
  if outputs_exist "$variant"; then
    echo "[$(date '+%H:%M:%S')] === E144 ${variant} outputs exist; skip ==="
    return 0
  fi
  override="core4d_${variant}"
  out_dir="$RESULTS/${variant}_outdir_${STAGE}"
  mkdir -p "$out_dir"
  echo "[$(date '+%H:%M:%S')] === E144 ${STAGE} ${variant} task=${task} GPU=${gpu} ==="
  if [ "$STAGE" = "smoke" ]; then
  ...
  echo "[$(date '+%H:%M:%S')] === E144 ${STAGE} ${variant} done ==="
}
...
case "$MODE" in
  ...
  local|remote-gpu0|remote-gpu1)
    ...
    if [ "${#variants[@]}" -eq 0 ]; then
      echo "No E144 variants for split=${split}; nothing to run."
      exit 0
    fi
    echo "E144 split=${split} variants=${#variants[@]} stage=${STAGE} gpu=${GPU}"
  ...
  eval)
    shift 3 || true
    bash workspace/core4d/scripts/eval/eval_E144_raw_mask_ref_fk_full_cem.sh "$STAGE" "$@"
    ;;
  *)
