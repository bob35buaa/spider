#!/usr/bin/env bash
# E083: upper-body-object collision CEM for D003 Box021 cases plus box023 guard.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

MODE="${1:-local}"
GPU="${2:-0}"
VARIANTS_FILE="${VARIANTS_FILE:-workspace/core4d/scripts/E083/variants.tsv}"
RESULTS="${RESULTS:-workspace/core4d/results/E083}"
LOGS="${LOGS:-logs/E083}"
...
snapshot_split() {
  ...
  if [ "${#tasks[@]}" -gt 0 ]; then
    ...
    bash workspace/core4d/scripts/convert/snapshot_scenes.sh E083 "${tasks[@]}"
...
run_one() {
  ...
  if [ -z "$row" ]; then
    echo "Unknown E083 variant: $variant" >&2
    return 2
  fi
...
case "$MODE" in
  ...
  single)
    ...
    run_one "$VARIANT" "$GPU"
    RESULTS="$RESULTS" VARIANTS_FILE="$VARIANTS_FILE" .venv/bin/python workspace/core4d/scripts/eval/eval_E083.py "$VARIANT" | tee "$LOGS/eval_${VARIANT}.log"
    ;;
  local|remote-gpu0|remote-gpu1)
    snapshot_split "$MODE"
    mapfile -t variants < <(variants_for_split "$MODE")
    if [ "${#variants[@]}" -eq 0 ]; then
      echo "No E083 variants for split=${MODE}" >&2
      exit 2
    fi
    for variant in "${variants[@]}"; do
      run_one "$variant" "$GPU"
    done
    RESULTS="$RESULTS" VARIANTS_FILE="$VARIANTS_FILE" .venv/bin/python workspace/core4d/scripts/eval/eval_E083.py "${variants[@]}" | tee "$LOGS/eval_${MODE}.log"
    ;;
  eval)
    shift || true
    RESULTS="$RESULTS" VARIANTS_FILE="$VARIANTS_FILE" .venv/bin/python workspace/core4d/scripts/eval/eval_E083.py "$@" | tee "$LOGS/eval_E083.log"
    ;;
  *)
    echo "Usage:"
    echo "  $0 list"
    ...
    echo "  $0 single 0 E083_d003_box021_20231018_029_p2_upperobj"
    echo "  $0 eval [variant ...]"
    exit 2
    ;;
esac
