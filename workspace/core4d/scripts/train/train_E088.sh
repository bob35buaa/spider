#!/usr/bin/env bash
# E088: hard safety gate and absolute object clearance CEM runs.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

MODE="${1:-local}"
GPU="${2:-0}"
VARIANTS_FILE="${VARIANTS_FILE:-workspace/core4d/scripts/E088/variants.tsv}"
RESULTS="${RESULTS:-workspace/core4d/results/E088}"
LOGS="${LOGS:-logs/E088}"
...
run_one() {
  ...
  if [ -z "$row" ]; then
    echo "Unknown E088 variant: $variant" >&2
    return 2
  fi
...
case "$MODE" in
  ...
  local|remote-gpu0|remote-gpu1)
    ...
    RESULTS="$RESULTS" VARIANTS_FILE="$VARIANTS_FILE" .venv/bin/python workspace/core4d/scripts/eval/eval_E088.py "${variants[@]}" | tee "$LOGS/eval_${MODE}.log"
    ;;
  single)
    VARIANT="${3:-}"
    if [ -z "$VARIANT" ]; then
    ...
    run_one "$VARIANT" "$GPU"
    RESULTS="$RESULTS" VARIANTS_FILE="$VARIANTS_FILE" .venv/bin/python workspace/core4d/scripts/eval/eval_E088.py "$VARIANT" | tee "$LOGS/eval_${VARIANT}.log"
    ;;
  eval)
    shift || true
    RESULTS="$RESULTS" VARIANTS_FILE="$VARIANTS_FILE" .venv/bin/python workspace/core4d/scripts/eval/eval_E088.py "$@" | tee "$LOGS/eval_E088.log"
    ;;
  *)
