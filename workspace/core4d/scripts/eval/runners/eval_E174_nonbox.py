...
DEFAULT_MANIFEST = REPO / "workspace/core4d/results/E174/s6_downstream/manifests/cem_full_manifest.tsv"
DEFAULT_BASELINE = REPO / "workspace/core4d/results/E174/s6_downstream/eval/full/__no_baseline__.tsv"  # E174 has no paired A/B baseline (E106 is historical overlap only)
DEFAULT_OUT = REPO / "workspace/core4d/results/E174/s6_downstream/eval/full"
...
def codex_template(
    ...
    complete = len(metrics) == len(manifest_rows)
...
def recommendation(rows: list[dict[str, Any]]) -> dict[str, Any]:
    # E174 is fresh cross-object screening (no E168 paired baseline). Machine
    # recommendation stays PENDING_USER_REVIEW until the user supplies final
    # USE/DO_NOT_USE labels (plan 8.3); yield taxonomy is derived from labels later.
    reviewed = sum(row.get("user_manual_review_status") == "reviewed" for row in rows)
    numeric_pass = sum(bool(row.get("numeric_release_pass")) for row in rows)
    overall = sum(bool(row.get("strict_release_usable")) for row in rows)
    return {"machine_recommendation": "PENDING_USER_REVIEW", "numeric_pass": numeric_pass, "strict_overall_user_gated": overall, "manual_reviewed": reviewed, "manual_operational_use": sum(bool(row.get("manual_operational_use")) for row in rows), "gate_health_pass": sum(bool(row.get("leg_gate_health_pass")) for row in rows)}
...
def main() -> int:
    ...
    baselines = {row["case_id"]: row for row in read_tsv(baseline_path)} if baseline_path.is_file() else {}
    ...
    write_tsv(out_dir / "e174_case_metrics.tsv", metrics, fields)
    write_tsv(out_dir / "e174_paired_deltas.tsv", paired)
    write_tsv(out_dir / "e174_group_summary.tsv", summaries)
    write_tsv(out_dir / "e174_worst_cases.tsv", rankings)
    write_tsv(out_dir / "e174_not_ready.tsv", not_ready)
    write_tsv(out_dir / "e174_evaluation_errors.tsv", errors)
    ...
    if errors:
        return 1
    if args.require_all and (len(metrics) != len(manifest_rows) or not_ready):
        return 2
    return 0
