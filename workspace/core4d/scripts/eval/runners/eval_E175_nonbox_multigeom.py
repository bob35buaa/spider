#!/usr/bin/env python3
"""Evaluate E175 multi-geom non-box CEM and compare it with frozen E174."""
...
def add_baseline(
    ...
    paired: dict[str, Any] = {
        ...
        "e175_numeric_release_pass": item["numeric_release_pass"],
        "e174_numeric_failure_modes": baseline.get(
            "numeric_failure_modes", ""
        ),
        "e175_numeric_failure_modes": item["numeric_failure_modes"],
    }
...
def main() -> int:
    ...
    expected_rows = 6 if args.mode == "canary" else 39
    manifest = repo_path(
        args.manifest
        or RESULT_ROOT
        / "manifests"
    ...
    for row in paired:
        ...
        new = bool(row["e175_numeric_release_pass"])
        transitions[
            f"e174_{'pass' if old else 'fail'}_to_"
            f"e175_{'pass' if new else 'fail'}"
        ] += 1
    failures = Counter(
        failure
        for row in metrics
    ...
    write_tsv(out_dir / "e175_case_metrics.tsv", metrics, fields)
    write_tsv(out_dir / "e175_vs_e174_paired_deltas.tsv", paired)
    write_tsv(out_dir / "e175_group_summary.tsv", group_summary(metrics))
    write_tsv(out_dir / "e175_not_ready.tsv", not_ready)
    write_tsv(out_dir / "e175_evaluation_errors.tsv", errors)
