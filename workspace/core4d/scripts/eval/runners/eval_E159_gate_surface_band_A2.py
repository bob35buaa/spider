#!/usr/bin/env python3
"""Evaluate E159 gateA + surfaceBand-A2 clean6 benchmark."""
...
RESULT_ROOT = REPO / "workspace/core4d/results/E159/gate_surface_band_A2"
...
def build_delta_rows(metric_rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for case in CLEAN6_CASES:
        ...
        for method in [
            ...
            if gate and run and method in {"E155_decay", "gateA+surfaceBand-A", "gateA+surfaceBand-A2"}:
...
def success_summary(method_summary: list[dict[str, Any]], delta_vs_gate: list[dict[str, Any]]) -> dict[str, Any]:
    surf = next((row for row in method_summary if row["method"] == "gateA+surfaceBand-A2"), None)
    if not surf:
        return {"promote_to_clean8_probe": False, "reason": "missing surfaceBand-A2 summary"}
    surf_deltas = [row for row in delta_vs_gate if row["method"] == "gateA+surfaceBand-A2"]
    ...
    checks = {
        ...
        "physPen3_delta_le_0_05": phys_pen3_delta <= 0.05,
        "pen2_delta_le_0_02": pen2_delta <= 0.02,
        "release_false3_delta_le_0_03": rel3_delta <= 0.03,
        "inmaskC3_delta_ge_0_10": inmask_delta >= 0.10,
    }
    return {
...
def main() -> None:
    ...
    e159_rows = read_tsv(E159_VARIANTS)
    rows = e156_rows + e158_rows + e159_rows
    metric_rows: list[dict[str, Any]] = []
    missing: list[str] = []
    ...
    for row in baseline_rows:
        ...
        if metrics is None:
            missing.append(f"E159_{row['short_case_id']}_omniretarget")
            continue
    ...
    write_tsv(eval_dir / "e159_method_metrics.tsv", metric_rows, metric_fields)
    write_tsv(eval_dir / "e159_method_summary.tsv", method_summary, summary_fields)
    write_tsv(eval_dir / "e159_delta_vs_omniretarget.tsv", delta_vs_omni, delta_fields)
    write_tsv(eval_dir / "e159_delta_vs_spider_rubberhand.tsv", delta_vs_baseline, delta_fields)
    write_tsv(eval_dir / "e159_delta_vs_gateA.tsv", delta_vs_gate, delta_fields)
    write_json(
        eval_dir / "e159_eval_summary.json",
        {
    ...
    write_xlsx(
        eval_dir / "E159_gate_surface_band_A2_clean6.xlsx",
        method_summary,
        metric_rows,
        delta_vs_omni,
        delta_vs_baseline,
    ...
    print(
        f"E159 eval: metric_rows={len(metric_rows)} methods={len(method_summary)} "
