#!/usr/bin/env python3
"""E059 evaluation: compare baseline vs warmstart CEM rollouts on box023.

Reads:
  workspace/core4d/results/E059/E059_baseline.npz
  workspace/core4d/results/E059/E059_warm.npz

Computes per-trajectory:
  - contact% (intent: palm-box distance < 5cm fraction)
...
Outputs:
  workspace/core4d/results/E059/eval_summary.csv     (1 row per trajectory)
  workspace/core4d/results/E059/face_dist_E059.png   (2x2 face dist time-series)
"""
...
RESULTS = REPO / "workspace/core4d/results/E059"

INTENT = (21, 78)                # source-resolution intent window from E055/E056 (box023)
PELVIS_OK = 0.50                 # pelvis_z stability threshold (m)
CONTACT_DIST = 0.05              # palm-to-box-surface contact threshold (m)
FACE_NAMES = ["+yz", "-yz", "+xz", "-xz", "+xy", "-xy"]
FACE_COLORS = ["#d62728", "#ff9896", "#2ca02c", "#98df8a", "#1f77b4", "#9ecae1"]
MAIN_FACE_DIST = 0.07
FACE_STABILITY = 0.60
EXPECTED_L = "-xy"               # box023 grasp_type=垂直: L 在 box 底面 (E055/E056)
...
def main() -> None:
    ...
    rows = []
    for name in ("E059_baseline", "E059_warm"):
    ...
    if len(rows) == 2:
        render_face_dist(rows, [r["intent"] for r in rows], RESULTS / "face_dist_E059.png")
