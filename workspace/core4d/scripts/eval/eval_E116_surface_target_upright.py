#!/usr/bin/env python3
"""Evaluate E116 surface-target + upright-guard diagnostic outputs.

This intentionally reuses the E115 evaluator implementation because E116 keeps
the same variants.tsv schema and the same baseline/E113/test comparison logic.
"""
...
E116_VARIANTS = REPO / "workspace/core4d/scripts/E116/variants.tsv"
...
def load_e115_eval():
    spec = importlib.util.spec_from_file_location("eval_E115_reused_for_E116", E115_EVAL)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Could not load {E115_EVAL}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    module.VARIANTS_TSV = E116_VARIANTS
    return module
...
def retitle_summary(path: Path) -> None:
    ...
    text = text.replace(
        ...
        "E116 Surface Target + Upright Guard",
    )
...
def main() -> None:
    ...
    results_dir = repo_path_from_env(
        "RESULTS",
        args.results_dir or REPO / f"workspace/core4d/results/E116/cem/{args.stage}",
    )
