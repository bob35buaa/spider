#!/usr/bin/env bash
# Build compact E083 frame sheets from extracted keyframes.
set -euo pipefail
...
if ls "$OUT"/*_sheet.jpg >/dev/null 2>&1; then
  ffmpeg -nostdin -y -loglevel error \
    ...
    -vf "scale=960:-1,tile=1x4:padding=8:margin=8:color=white" \
    -frames:v 1 "$OUT/E083_all_cases_sheet.jpg" || true
fi
