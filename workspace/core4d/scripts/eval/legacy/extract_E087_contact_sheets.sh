#!/usr/bin/env bash
# Build compact E087 frame sheets from extracted keyframes.
set -euo pipefail
...
make_sheet() {
  ...
  [ -d "$dir" ] || return 0
  mapfile -t frames < <(for f in 16 32 50 75 90 100 115 120 125 145; do
    [ -f "$dir/f${f}.jpg" ] && echo "$dir/f${f}.jpg"
  done)
  [ "${#frames[@]}" -gt 0 ] || return 0
  ffmpeg -nostdin -y -loglevel error \
    -pattern_type glob -i "$dir/f*.jpg" \
    -vf "scale=320:-1,tile=5x2:padding=4:margin=4:color=white" \
    -frames:v 1 "$output"
  echo "$output"
}
...
if ls "$OUT"/*_sheet.jpg >/dev/null 2>&1; then
  ffmpeg -nostdin -y -loglevel error \
    ...
    -vf "scale=960:-1,tile=1x3:padding=8:margin=8:color=white" \
    -frames:v 1 "$OUT/E087_all_cases_sheet.jpg" || true
fi
