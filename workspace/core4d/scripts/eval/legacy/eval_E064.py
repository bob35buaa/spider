#!/usr/bin/env python3
"""E064 evaluation: Tier 2 (root_sigma=0.3 + contact_gain=3.0 + threshold=0.65) vs E063 baseline.

Reads:
  workspace/core4d/results/E064/E063_box023.npz
  workspace/core4d/results/E064/E063_box025.npz
  (and E062 npz from workspace/core4d/results/E063/ for diff vs baseline)
...
Outputs:
  workspace/core4d/results/E064/eval_summary.csv
  workspace/core4d/results/E064/obj_trace_E063_box023.csv  (full ref vs sim trace)
  workspace/core4d/results/E064/obj_trace_E063_box025.csv
  workspace/core4d/results/E064/face_dist_E063_box023.png
  workspace/core4d/results/E064/face_dist_E063_box025.png
  workspace/core4d/results/E064/pelvis_z_E063_box023.png
  workspace/core4d/results/E064/pelvis_z_E063_box025.png
"""
...
RESULTS = REPO / "workspace/core4d/results/E064"
E063_RESULTS = REPO / "workspace/core4d/results/E063"

# Per-case config: (name, case_dir, intent_src, src_T, expected faces, e063_npz_baseline_name)
CASES = [
    {
        "name": "E064_box023",
...
def main() -> None:
    e064_rows = []
    all_rows_per_case = {}  # case name → [E063 baseline row, E064 row]
    for cfg in CASES:
        e063_npz_baseline = E063_RESULTS / f"{cfg['e063_name']}.npz"
        e064_npz = RESULTS / f"{cfg['name']}.npz"
        e063_row_baseline = evaluate_one(cfg, e063_npz_baseline, label=f"{cfg['e063_name']} (E063 baseline)")
        e064_row = evaluate_one(cfg, e064_npz, label=cfg["name"])
        if e064_row is not None:
            e064_rows.append((cfg, e064_row))
        all_rows_per_case[cfg["name"]] = [e063_row_baseline, e064_row]

    if not e064_rows:
        print("No E064 npz to evaluate.")
        return
    ...
    for cfg, r in e064_rows:
        ...
        render_pelvis_obj(all_rows_per_case[cfg["name"]],
                          ...
                          title=f"{cfg['name']}: pelvis_z + obj err vs time (E063 baseline vs E064)")
    ...
    # Pass criteria for E064 box023
    print()
    print("=" * 70)
    print("E064 PASS criteria (log 80 §8):")
    for cfg, r in e064_rows:
        if cfg["name"] == "E064_box023":
