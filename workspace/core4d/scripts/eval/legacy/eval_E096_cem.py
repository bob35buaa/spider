#!/usr/bin/env python3
"""Evaluate E096 full-CEM rollouts."""
...
DEFAULT_VARIANTS = REPO / "workspace/core4d/scripts/E096/variants.tsv"
...
def load_e090_eval():
    spec = importlib.util.spec_from_file_location("eval_E090_for_E096", E090_EVAL)
...
def read_variants(path: Path) -> dict[str, dict[str, str]]:
    ...
    with path.open("r", encoding="utf-8", newline="") as f:
        ...
        for row in reader:
            if row["route"] == "cem_ref_fk" and str(row.get("ready", "")).lower() == "true":
...
def print_table(results: dict[str, dict[str, object]]) -> None:
    print("=" * 128)
    print(f"{'variant':<32} {'case':<15} {'T':>4} {'cont':>7} {'obj_mean':>9} {'obj_max':>8} {'pelv':>7} {'head':>7} {'upper':>7} {'LH':>7} {'RH':>7} {'status':>8}")
    print("-" * 128)
    for variant, row in results.items():
        print(
            f"{variant:<32} {row['case_id']:<15} {int(row['T']):>4} "
...
def main() -> None:
    ...
    results_dir = repo_path_from_env("RESULTS", REPO / f"workspace/core4d/results/E096/cem/{args.stage}")
    ...
    for variant in selected:
        ...
        if row is None:
            print(f"[SKIP] unknown E096 variant {variant}")
            continue
    ...
    if not results:
        raise SystemExit("No E096 rollout results found.")
    ...
    lines = [
        f"# E096 full CEM {args.stage} summary",
        "",
