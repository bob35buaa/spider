#!/usr/bin/env python3
"""E173: generate the refined Markdown main report + machine JSON.
...
Reads only committed E173 evidence; does not write manual_* user fields.
"""
...
import sys
sys.path.insert(0, str(Path(__file__).resolve().parents[2] / "experiments/E173"))
...
def main() -> int:
    ...
    ap.add_argument("--out", type=Path, default=C.RESULTS / "s6_downstream/eval/full/E173_report.md")
    args = ap.parse_args()
    R = C.RESULTS
    ...
    metrics = C.read_tsv(R / "s6_downstream/eval/full/e173_case_metrics.tsv")
    ...
    OBJS = ("box024", "box023", "box001")

    L = []
    L.append("# E173 结果报告：box024 / box023 / box001 全流程筛选 + Full CEM（move-only）")
    L.append("")
    L.append("_Core4D Phase 36 · rubber_hull + E170 PRG cross-object candidate · machine recommendation = `PENDING_USER_REVIEW`_")
    L.append("")
    L.append("> 本报告为机器核验 + 视觉复核交付物；最终 `USE/DO_NOT_USE` 由用户裁决。E173 沿用 E170/E171/E172 冻结算法，move-only，不做 reward/route sweep，不导出 RL；science yield 与执行完成解耦。首次纳入大箱（box024/box001 ~0.25m³），box023 小箱对照。")
    L.append("")
    # Q1 funnel
    ...
    L.append(md_table(["阶段", "计数"], [
        ...
        ["raw-contact pass (move-only, 3cm)", funnel["raw_contact_pass"]],
    ...
    L.append("- 大箱（box024/box001）v1-infeasible 率显著高于小箱 box023；v2 Phase4 rescue 是否在大箱产生下游 numeric-pass 见 §3。")
    L.append("")
    # Q6 PRG
    L.append("## 5. PRG 跨物体（含大箱）是否成立 + contact trade-off")
    L.append("")
    ...
    L.append("- 大箱 vs 小箱 contact/lower-body trade-off、与 box021(E170)/box026(E171)/box004(E172) 对照见日志分析。最终 yield 分级以用户裁决为准。")
    L.append("")
    L.append("## 6. 复现锚点")
    L.append("")
    ...
    L.append("- metrics：`s6_downstream/eval/full/e173_case_metrics.tsv`；summary：`.../summary.json`")
