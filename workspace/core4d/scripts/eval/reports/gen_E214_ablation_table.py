...
def build_block(cases: list[str], full: dict, e214: dict, title: str) -> list[str]:
    ...
    for key, label, direction, kind in METRICS:
        cells = []
        full_mean = agg(series_values(per_series["full"], key), direction)["mean"]
        for s in SERIES:
            a = agg(series_values(per_series[s], key), direction)
            cell = f"{fmt(a['mean'], kind)}±{fmt(a['std'], kind)} (w {fmt(a['worst'], kind)})"
            if s != "full":
                p = wilcoxon(per_series["full"], per_series[s], key)
                if math.isfinite(p):
                    star = "*" if p < 0.05 else ""
                    cell += f" p={p:.3f}{star}"
            cells.append(cell)
...
def main() -> int:
    ...
    md = ["# E214 四消融结果表（full vs A1–A4，50 case）", "",
          ...
          "- 单元格：mean±std (worst)；消融列附 full-vs-消融 配对 Wilcoxon p（* = p<0.05）。",
          "- 方向 ↑=越大越好，↓=越小越好。", ""]
    ...
    tlines = ["metric\tseries\tmean\tstd\tworst\tn\twilcoxon_p"]
    per_series = {s: {c: (full.get(c, {}) if s == "full" else e214.get((c, s), {})) for c in cases}
                  for s in SERIES}
    for key, label, direction, kind in METRICS:
        for s in SERIES:
            a = agg(series_values(per_series[s], key), direction)
            p = "" if s == "full" else f"{wilcoxon(per_series['full'], per_series[s], key):.4f}"
            tlines.append(f"{key}\t{s}\t{a['mean']:.6g}\t{a['std']:.6g}\t{a['worst']:.6g}\t{a['n']}\t{p}")
    OUT_TSV.write_text("\n".join(tlines) + "\n", encoding="utf-8")
    ...
    try:
        ...
        ws.append(["metric", "series", "mean", "std", "worst", "n", "wilcoxon_p"])
