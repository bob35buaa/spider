#!/usr/bin/env python3
"""E212: direction-aware g-curve workbook over the requested four arms.

Reads `e212_g_sweep_rollout.tsv` (written by eval_E212_partial_gravcomp.py) and
...
Usage:
    .venv/bin/python workspace/core4d/scripts/eval/reports/gen_E212_g_sweep_workbook.py
...
for _p in ("workspace/core4d/scripts",
           ...
           "workspace/core4d/scripts/experiments/E212"):
...
ARM_LABEL = {
    "prg": "g=0.0  E206 PRG (baseline)", "G04": "g=0.4  E212",
    "G06": "g=0.6  E212", "G08": "g=0.8  E212", "g1": "g=1.0  E209 G1",
}
BASELINE = "prg"
...
def main() -> int:
    ...
    ap.add_argument("--rollout", type=Path,
                    default=C.S6_DIR / "eval/g_sweep/e212_g_sweep_rollout.tsv")
    ...
    if not args.rollout.is_file():
        raise SystemExit(f"missing rollout TSV: {args.rollout} (run eval_E212_partial_gravcomp.py)")
    ...
    (out_dir / "e212_g_sweep_workbook.json").write_text(
        json.dumps(payload, ensure_ascii=False, indent=2, default=str) + "\n",
        encoding="utf-8")

    # --- xlsx ----------------------------------------------------------------
    out_xlsx = args.out or (out_dir / "E212_g_sweep_comparison.xlsx")
    try:
    ...
    ws.title = "README"
    readme = [
        "E212 Stage A - desk023 partial-gravcomp g-curve",
        "",
        ...
        "",
        "HOW TO READ THIS",
        "  This is a TRADE-OFF curve, not a rescue. desk023's PRG baseline is already",
        "  narrow 4/4, and g=1.0 nearly nulls the object z bias (|bias| 0.41 cm,",
        "  per-case g* median 0.93). So lowering g necessarily gives back z; the",
        "  question C1 asks is whether some g keeps most of the z gain while holding",
        "  the robot side near PRG.",
        "",
        "  Object side improves monotonically with g: |z_bias| 3.65 -> 2.36 -> 1.53",
        "  -> 0.60 -> 0.35 cm. Robot side degrades with g, essentially monotonically",
        "  (eef_ori 16.96 -> 16.94 -> 17.54 -> 18.95 -> 21.28; the single non-monotone",
        "  step is -0.02 deg, i.e. noise). This DIFFERS from desk007 in E211, where the",
        "  robot side swung 6 deg non-monotonically and was NOT a function of g.",
        "",
        "  Mechanism carries over anyway: partial r(gate fallback, eef_ori | g) = +0.61",
        "  here vs +0.50 on desk007, while r(g, fallback) = +0.06. But on desk023 the",
        "  fallback is almost entirely the LEG gate, not body/hand as on desk007 -",
        "  see the leg_fallback_frac column. Both g and fallback contribute here.",
        "",
        "  RESULT: C1 FAILS on all arms, but read the margins, not the verdict.",
        "  g=0.6 passes 5 of 6 clauses and misses C1a by 0.026 cm (|z_bias| 1.5263 vs",
        "  the 1.50 bound). g=0.4 holds the robot side completely (narrow 4/4, same as",
        "  PRG) but only recovers a third of the z. There is a usable knee near g=0.5.",
        "",
        "  CAVEAT on the g=1.0 row: its C1a shows FAIL, but that is an artifact - the",
        "  C1a z_mae bound was frozen as g=1.0's own value rounded to 4 dp (3.1752),",
        "  and the exact value is 3.175202, over by 2e-6 cm. It is not a real failure",
        "  and it does not change the winner set (g=1.0 is a frozen endpoint, never a",
        "  candidate). The bound is left as frozen; see log301.",
        "",
        "DIRECTION",
        ...
        "",
        "POPULATION",
        "  n=4, so ONE case carries 25% of every macro mean. desk023_20231030_019_p1",
        "  is the worst case and dominates eef_pos at g=1.0 (15.73 all-4 vs 13.58",
        "  without it). The summary JSON carries both views under 'sensitivity_outlier';",
        "  the MAIN GATE always uses all 4. Unlike desk007, no case has an empty",
        "  release window, so release is a true n=4 mean.",
        "",
        "SOURCE",
        f"  {args.rollout.relative_to(REPO)}",
        "  scored by workspace/core4d/scripts/eval/runners/eval_E212_partial_gravcomp.py",
        "  log: workspace/core4d/log/301_E212_desk023_partial_gravcomp_sweep.md",
    ]
    ...
    print(f"[done] {(out_dir / 'e212_g_sweep_workbook.json').relative_to(REPO)}")
