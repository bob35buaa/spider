#!/usr/bin/env bash
# Build compact E085 frame sheets from extracted keyframes.
set -euo pipefail
...
if ls "$OUT"/*_sheet.jpg >/dev/null 2>&1; then
  ffmpeg -nostdin -y -loglevel error \
    ...
    -vf "scale=960:-1,tile=1x2:padding=8:margin=8:color=white" \
    -frames:v 1 "$OUT/E085_all_cases_sheet.jpg" || true
fi
