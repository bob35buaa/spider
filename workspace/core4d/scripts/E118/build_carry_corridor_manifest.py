#!/usr/bin/env python3
"""Build E118 carry-corridor soft-gate diagnostic manifest."""
...
SCRIPTS_ROOT = REPO / "workspace/core4d/scripts/E118"
RESULTS_ROOT = REPO / "workspace/core4d/results/E118"
...
CASE_SPLITS = {
    "box021_029_p2": ("remote-gpu0", "main_carry_corridor"),
    ...
    for case_id, (split, phase_scope) in CASE_SPLITS.items():
        ...
        for spec in RUNNABLE_VARIANTS:
            rows.append(
                {
                    "ordinal": ordinal,
                    "variant": f"E118_{case_id}_{spec.ablation}",
...
# Auto-generated by workspace/core4d/scripts/E118/build_carry_corridor_manifest.py.
# E118 carry-corridor soft gate; case={row['case_id']} ablation={row['ablation']}.
defaults:
  - {row['source_override']}
  - _self_
...
carry_corridor_rew_scale: {spec.corridor_scale:.1f}
...
leg_object_penalty_gate_source: contact_mask
...
# Keep inherited safety/upright knobs explicit and neutral unless this ablation
# enables them above. This keeps E118 distinct from E116.
...
task_obj_pos_rew_scale: 0.0
task_obj_rot_rew_scale: 0.0
...
def write_preflight(rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for row in rows:
        ...
        out = {
            **{field: row[field] for field in FIELDS},
            ...
            "corridor_scale": spec.corridor_scale,
    ...
    write_tsv(PREFLIGHT_TSV, preflight_rows, preflight_fields, "E118 Phase A preflight")
    required = [
        "scene_act_exists",
        "scene_act_loads",
        "trajectory_exists",
...
def write_summary_md(rows: list[dict[str, Any]], preflight_rows: list[dict[str, Any]], summary: dict[str, Any]) -> None:
    ...
    lines = [
        "# E118 Carry Corridor Manifest Summary",
        "",
    ...
    lines.extend(
        [
            "",
            "| case | ablation | split | target | corridor | leg | mask either | phase | queue |",
            "|---|---|---|---|---:|---:|---:|---|---|",
        ]
    )
    ...
    for row in rows:
        ...
        lines.append(
            ...
            f"`{pf['target_source']}` | {float(pf['corridor_scale']):.1f} | "
            f"{float(pf['leg_scale']):.1f} | {float(pf['mask_active_either_frac']) * 100:.1f}% | "
...
def main() -> None:
    ...
    write_tsv(VARIANTS_TSV, rows, FIELDS, "E118 carry-corridor soft-gate variants")
