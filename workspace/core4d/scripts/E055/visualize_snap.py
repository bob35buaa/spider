#!/usr/bin/env python3
"""E055: Render side-by-side ref vs snap MuJoCo videos for box023_person1.

Top row = ref qpos (original mocap-IK trajectory).
Bottom row = snapped qpos (arm joints replaced inside intent window).
Two cameras per row: front + side.

Output:
  workspace/core4d/results/E055/box023_person1/snap_visualization.mp4
"""
...
RESULTS = REPO / f"workspace/core4d/results/E055/{CASE}"
...
def main() -> None:
    ...
    # stack: ref on top, snap on bottom
    combined = []
    for t in range(len(ref_frames)):
        in_intent = intent[0] <= t <= intent[1]
        # add a thin colored bar at top to indicate in_intent
