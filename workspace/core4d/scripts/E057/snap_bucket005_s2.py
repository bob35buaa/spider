#!/usr/bin/env python3
"""E057 Path B: Generate hand-snap warmstart for bucket005_s2_person1.

Mirrors workspace/core4d/scripts/E055/snap_box023.py — only the case path
differs. bucket005_s2 was selected by E056 multi-case diagnosis as the only
true 对侧 grasp (L on -yz, R on +yz) with the longest intent window (88f).

Outputs:
  workspace/core4d/results/E057/bucket005_s2_person1/warmstart_qpos.npz
    - qpos_ref:    (T, nq) original mocap-IK trajectory
    - qpos_snap:   (T, nq) snapped trajectory (only arm joints differ)
    - intent_window: (t_start, t_end)
    - snap_mask:   (T,) bool, True for frames where snap was applied
  workspace/core4d/results/E057/bucket005_s2_person1/snap_diagnostics.csv
...
OUT_DIR = REPO / f"workspace/core4d/results/E057/{CASE}"


def find_intent_window(qpos_ref: np.ndarray, scene_xml: Path) -> tuple[int, int]:
    """v3 detector — kept inline (no cross-script coupling), identical to E055."""
    import mujoco
    ...
    if not runs:
        raise RuntimeError(f"No intent window detected for {CASE}")
    return max(runs, key=lambda x: x[1] - x[0])


def main() -> None:
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    print(f"[E057] case={CASE}")
    print(f"[E057] scene={SCENE}")
    print(f"[E057] traj={TRAJ}")

    qpos_ref = np.load(TRAJ)["qpos"]
    T, nq = qpos_ref.shape
    print(f"[E057] loaded ref qpos: T={T}, nq={nq}")

    intent = find_intent_window(qpos_ref, SCENE)
    print(f"[E057] intent_window={intent} ({intent[1] - intent[0] + 1} frames)")

    mesh_path = find_object_mesh(str(SCENE))
    print(f"[E057] object mesh: {mesh_path}")
    ...
    print(f"[E057] snapped {len(diag)} (frame, hand) pairs")
