#!/usr/bin/env python3
"""E171: build the frozen pipeline authority, funnel and terminal-state audit.

Reads the E171 S1-S6 manifests and produces, for every expected Box022/Box026
...
def build() -> dict:
    ...
    for src in sorted(inv, key=lambda r: r["case_id"]):
        ...
        in_cem = cid in cem and cem[cid].get("status") == "READY_FOR_FULL"
