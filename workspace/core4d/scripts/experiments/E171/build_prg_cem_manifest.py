#!/usr/bin/env python3
"""E171: build the rubber-hull + E170-PRG CEM scenes, overrides and manifests.

Input authority is the E171 S5 handoff manifest (the frozen S5_READY_SET) plus
...
For each S5-ready case this:
  1. builds a FRESH rubber_hull hand sidecar from the target scene_act.xml
     ...
     pairs -> scene_act_E171_rubberHull_PRG.xml (never overwrites scene_act.xml);
...
Only object=box022/box026 rows are in scope. Full expected = count(S5_READY).
"""
...
import e171_common as C  # noqa: E402
...
PAIR_PREFIX = "E171_"
NUM_GPUS = int(__import__("os").environ.get("E171_NUM_GPUS", "8"))
AXIS_FIELDS = {
...
def write_override(case_id: str, dcv3_override_id: str) -> Path:
    override_id = C.safe_id(f"core4d_E171_{case_id}_PRG")
    output = C.OVERRIDE_DIR / f"{override_id}.yaml"
    header = "# @package _global_\n# Auto-generated by E171 build_prg_cem_manifest.py.\n"
...
def build_scene(case_id: str, base_scene_act: Path, trajectory: Path, *, overwrite: bool) -> dict[str, Any]:
    """sphere scene_act -> rubber_hull patch -> +16 lower-body pairs -> E171 sidecar."""
    ...
    rubber_name = "scene_act_E171_rubberHull"
    patch_scene(
    ...
    if output.exists() and not overwrite:
        if tree_signature(ET.parse(output).getroot()) != tree_signature(root):
            raise FileExistsError(f"different E171 sidecar exists: {output}")
    else:
...
def artifact_paths(case_id: str, stage: str) -> dict[str, str]:
    ...
    variant = f"E171_{case_id}_PRG{'_canary' if is_smoke else ''}"
    root = f"workspace/core4d/results/E171/s6_downstream/cem/{stage}"
    return {
        "variant": variant,
        ...
        "video": f"workspace/core4d/results/E171/s6_downstream/render/{stage}/{variant}_{suffix}.mp4",
        "log": f"logs/E171/cem/{stage}/{variant}.log",
...
def build(*, overwrite_scenes: bool, include_review: bool) -> tuple[dict[str, Any], int]:
    ...
    for ordinal, src in enumerate(s5_ready, 1):
        ...
        base = {
            ...
            "spider_method_id": C.E171_METHOD_ID,
        ...
        scene: dict[str, Any] = {}
        override = C.OVERRIDE_DIR / f"{C.safe_id(f'core4d_E171_{case_id}_PRG')}.yaml"
