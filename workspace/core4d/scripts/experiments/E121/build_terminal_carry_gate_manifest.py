#!/usr/bin/env python3
"""Build E121 terminal carry gate diagnostic manifest."""
...
SCRIPTS_ROOT = REPO / "workspace/core4d/scripts/E121"
RESULTS_ROOT = REPO / "workspace/core4d/results/E121"
    ...
    for case_id, (split, phase_scope) in CASE_SPLITS.items():
        ...
        for spec in RUNNABLE_VARIANTS:
            rows.append(
                {
                    "ordinal": ordinal,
                    "variant": f"E121_{case_id}_{spec.ablation}",
...
# Auto-generated by workspace/core4d/scripts/E121/build_terminal_carry_gate_manifest.py.
# E121 terminal carry gate; case={row['case_id']} ablation={row['ablation']}.
defaults:
  - {row['source_override']}
  - _self_
...
terminal_carry_gate_enabled: true
terminal_carry_gate_mode: {spec.terminal_mode}
terminal_carry_gate_soft_scale: {spec.terminal_soft_scale:.1f}
...
# E121 keeps E120 support decomposition, but the primary variable is terminal carry gating.
...
def write_preflight(rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for row in rows:
        ...
        out = {
            **{field: row[field] for field in FIELDS},
            ...
            "terminal_carry_gate": True,
            "terminal_carry_gate_mode": spec.terminal_mode,
            "terminal_carry_gate_soft_scale": spec.terminal_soft_scale,
    ...
    write_tsv(PREFLIGHT_TSV, preflight_rows, preflight_fields, "E121 Phase A preflight")
    required = [
        "scene_act_exists",
        "scene_act_loads",
        "trajectory_exists",
...
def write_summary_md(rows: list[dict[str, Any]], preflight_rows: list[dict[str, Any]], summary: dict[str, Any]) -> None:
    ...
    lines = [
        "# E121 Terminal Carry Gate Manifest Summary",
        "",
    ...
    lines.extend(
        [
            "",
            "| case | ablation | split | target | terminal mode | hand support | non-hand penalty | mask either | phase | queue |",
            "|---|---|---|---|---|---:|---:|---:|---|---|",
        ]
    )
    ...
    for row in rows:
        ...
        lines.append(
            ...
            f"`{pf['target_source']}` | `{row['_spec'].terminal_mode}` | {float(pf['hand_support_scale']):.1f} | "
...
def main() -> None:
    ...
    write_tsv(VARIANTS_TSV, rows, FIELDS, "E121 terminal carry gate variants")
