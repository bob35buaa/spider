#!/usr/bin/env python3
"""Build E160 gateA + surfaceBand-A2 + postureRerankA manifest and overrides."""
...
SCRIPT_ROOT = REPO / "workspace/core4d/scripts/experiments/E160"
...
def split_for(case_id: str) -> str:
    if case_id == "box021_029_p2":
        return "local-gpu0"
    if case_id == "box004_083_p2":
        return "remote-gpu0"
    if case_id == "box023_person2":
        return "remote-gpu1"
...
def write_surface_override(row: dict[str, Any], base_override: str) -> str:
    ...
    lines = [
        "# @package _global_",
        "# Auto-generated by workspace/core4d/scripts/experiments/E160/build_posture_rerank_manifest.py.",
        f"# E160 gateA+surfaceBand-A2+postureRerankA case={row['short_case_id']}.",
        "defaults:",
        f"  - {base_stem}",
        "  - _self_",
        "",
...
def build_row(case_id: str, gate_row: dict[str, str], ordinal: int) -> dict[str, Any]:
    ...
    row: dict[str, Any] = {
        ...
        "method": "gateA+surfaceBand-A2+postureRerankA",
        "method_display": "gateA+surfaceBand-A2+postureRerankA",
        "method_group": "postureRerankA",
        "run_status": "to_run",
        "source_exp": "E160",
...
def build() -> tuple[list[dict[str, Any]], list[dict[str, Any]], dict[str, Any]]:
    ...
    missing = [case for case in E160_CASES if (case, "+gateA") not in by_gate]
    if missing:
        raise RuntimeError(f"E156 +gateA rows missing E160 cases: {missing}")
    rows = [build_row(case, by_gate[(case, "+gateA")], idx + 1) for idx, case in enumerate(E160_CASES)]
    ...
    summary = {
        ...
        "posture_gate": {
            "mean_z_err_m": POSTURE_MEAN_Z_ERR_M,
            "terminal_z_err_m": POSTURE_TERMINAL_Z_ERR_M,
            "max_z_drop_m": POSTURE_MAX_Z_DROP_M,
            "terminal_frac": POSTURE_TERMINAL_FRAC,
            "min_valid_frac": POSTURE_MIN_VALID_FRAC,
            "fallback_lambda": POSTURE_FALLBACK_LAMBDA,
        },
    }
...
def main() -> None:
    ...
    write_tsv(PREFLIGHT_ROOT / "e160_posture_rerank_preflight.tsv", preflight, PREFLIGHT_FIELDS)
    write_json(PREFLIGHT_ROOT / "e160_posture_rerank_preflight_summary.json", summary)
    print(
        "E160 manifest: "
