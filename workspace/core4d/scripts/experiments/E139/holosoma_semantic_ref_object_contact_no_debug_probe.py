#!/usr/bin/env python3
"""Probe E138 semantic object_contact through Holosoma ref_object_contact."""
...
def replay_probe(tyro_config: ExperimentConfig) -> None:
    ...
    if os.environ.get("E139_DISABLE_REPLAY_SLEEP", "1") != "0":
        try:
            import holosoma.envs.wbt.wbt_manager as wbt_manager_module

            wbt_manager_module.time.sleep = lambda _dt: None
        except Exception as exc:  # pragma: no cover - diagnostic only
            print(f"E139_SLEEP_PATCH_SKIPPED reason={type(exc).__name__}:{exc}", flush=True)
    ...
    max_steps = int(os.environ.get("E139_MAX_STEPS", "250") or "250")
    steps = 0
    ...
    summary = {
        ...
        **ref_stats,
        ...
        "semantic_ref_mask_ready": True,
        "rl_ready": False,
    }
    print("E139_PROBE_SUMMARY " + json.dumps(summary, sort_keys=True), flush=True)
    print(f"E139_PROBE_DONE steps={steps} done={done}", flush=True)
