#!/usr/bin/env python3
"""E173: build the rubber-hull + E170-PRG CEM scenes, overrides and manifests.

Input authority is the E173 S5 handoff manifest (the frozen S5_READY_SET) plus
...
For each S5-ready case this:
  1. builds a FRESH rubber_hull hand sidecar from the target scene_act.xml
     ...
     pairs -> scene_act_E173_rubberHull_PRG.xml (never overwrites scene_act.xml);
...
Only object in {box024,box023,box001} rows are in scope. Full expected = count(S5_READY).
"""
...
import e173_common as C  # noqa: E402
...
PAIR_PREFIX = "E173_"
NUM_GPUS = int(__import__("os").environ.get("E173_NUM_GPUS", "8"))
AXIS_FIELDS = {
...
def write_override(case_id: str, dcv3_override_id: str) -> Path:
    override_id = C.safe_id(f"core4d_E173_{case_id}_PRG")
    output = C.OVERRIDE_DIR / f"{override_id}.yaml"
    header = "# @package _global_\n# Auto-generated by E173 build_prg_cem_manifest.py.\n"
...
def build_scene(case_id: str, base_scene_act: Path, trajectory: Path, *, overwrite: bool) -> dict[str, Any]:
    """sphere scene_act -> rubber_hull patch -> +16 lower-body pairs -> E173 sidecar."""
    ...
    rubber_name = "scene_act_E173_rubberHull"
    patch_scene(
    ...
    if output.exists() and not overwrite:
        if tree_signature(ET.parse(output).getroot()) != tree_signature(root):
            raise FileExistsError(f"different E173 sidecar exists: {output}")
    else:
...
def artifact_paths(case_id: str, stage: str) -> dict[str, str]:
    ...
    variant = f"E173_{case_id}_PRG{'_canary' if is_smoke else ''}"
    root = f"workspace/core4d/results/E173/s6_downstream/cem/{stage}"
    return {
        "variant": variant,
        ...
        "video": f"workspace/core4d/results/E173/s6_downstream/render/{stage}/{variant}_{suffix}.mp4",
        "log": f"logs/E173/cem/{stage}/{variant}.log",
...
def build(*, overwrite_scenes: bool, include_review: bool) -> tuple[dict[str, Any], int]:
    ...
    for ordinal, src in enumerate(s5_ready, 1):
        ...
        base = {
            ...
            "spider_method_id": C.E173_METHOD_ID,
        ...
        scene: dict[str, Any] = {}
        override = C.OVERRIDE_DIR / f"{C.safe_id(f'core4d_E173_{case_id}_PRG')}.yaml"
    ...
    # canary: >=1 per (object, person); >=1 v2 if any v2 ready; up to 5
    canary: list[dict[str, Any]] = []
    seen_groups: set[str] = set()
    have_v2 = False
    for r in ready:
        group = f"{r['object_key']}:{r['person']}"
        need_group = group not in seen_groups
        need_v2 = (r["selected_retarget_variant_id"] == C.RETARGET_RESCUE_VARIANT and not have_v2)
        if (need_group or need_v2) and len(canary) < 5:
            row = copy.deepcopy(r)
