#!/usr/bin/env python3
"""E172: build the rubber-hull + E170-PRG CEM scenes, overrides and manifests.

Input authority is the E172 S5 handoff manifest (the frozen S5_READY_SET) plus
...
For each S5-ready case this:
  1. builds a FRESH rubber_hull hand sidecar from the target scene_act.xml
     ...
     pairs -> scene_act_E172_rubberHull_PRG.xml (never overwrites scene_act.xml);
...
Only object=box004 rows are in scope. Full expected = count(S5_READY).
"""
...
import e172_common as C  # noqa: E402
...
PAIR_PREFIX = "E172_"
NUM_GPUS = int(__import__("os").environ.get("E172_NUM_GPUS", "8"))
AXIS_FIELDS = {
...
def write_override(case_id: str, dcv3_override_id: str) -> Path:
    override_id = C.safe_id(f"core4d_E172_{case_id}_PRG")
    output = C.OVERRIDE_DIR / f"{override_id}.yaml"
    header = "# @package _global_\n# Auto-generated by E172 build_prg_cem_manifest.py.\n"
...
def build_scene(case_id: str, base_scene_act: Path, trajectory: Path, *, overwrite: bool) -> dict[str, Any]:
    """sphere scene_act -> rubber_hull patch -> +16 lower-body pairs -> E172 sidecar."""
    ...
    rubber_name = "scene_act_E172_rubberHull"
    patch_scene(
    ...
    if output.exists() and not overwrite:
        if tree_signature(ET.parse(output).getroot()) != tree_signature(root):
            raise FileExistsError(f"different E172 sidecar exists: {output}")
    else:
...
def artifact_paths(case_id: str, stage: str) -> dict[str, str]:
    ...
    variant = f"E172_{case_id}_PRG{'_canary' if is_smoke else ''}"
    root = f"workspace/core4d/results/E172/s6_downstream/cem/{stage}"
    return {
        "variant": variant,
        ...
        "video": f"workspace/core4d/results/E172/s6_downstream/render/{stage}/{variant}_{suffix}.mp4",
        "log": f"logs/E172/cem/{stage}/{variant}.log",
...
def build(*, overwrite_scenes: bool, include_review: bool) -> tuple[dict[str, Any], int]:
    ...
    for ordinal, src in enumerate(s5_ready, 1):
        ...
        base = {
            ...
            "spider_method_id": C.E172_METHOD_ID,
        ...
        scene: dict[str, Any] = {}
        override = C.OVERRIDE_DIR / f"{C.safe_id(f'core4d_E172_{case_id}_PRG')}.yaml"
