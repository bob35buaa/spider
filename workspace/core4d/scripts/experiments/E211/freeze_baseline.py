#!/usr/bin/env python3
"""E211 P0: re-measure the two frozen endpoints and assert they match plan241.
...
``e211_common.BASELINE_DESK007`` from E209's delivered TSVs and fails on any
...
It also copies the desk007 slices of those TSVs into
``results/E211/baseline/`` with their sha256, so the comparison stays
reproducible even if E209's directory is later regenerated.

Usage:
    .venv/bin/python workspace/core4d/scripts/experiments/E211/freeze_baseline.py
"""
...
#: rollout-TSV column -> BASELINE_DESK007 key.
ROLLOUT_FIELDS = {
...
def measure() -> tuple[dict[str, dict[str, float]], dict[str, int]]:
    ...
    if {r["case_id"] for r in rollout} != cases:
        raise SystemExit(f"rollout TSV desk007 cases != E211 scope: {sorted({r['case_id'] for r in rollout})}")
    ...
    for arm, rollout_tag, z_tag in (("prg", "prg", "PRG"), ("g1", "g1", "G1")):
        ...
        if len(rows) != C.EXPECTED_CASES or len(zrows) != C.EXPECTED_CASES:
            raise SystemExit(f"{arm}: {len(rows)} rollout / {len(zrows)} z rows, want {C.EXPECTED_CASES}")
...
def main() -> int:
    ...
    problems: list[str] = []
    for arm, frozen in C.BASELINE_DESK007.items():
    ...
    # `release` is NaN on the one case with an empty release window; the frozen
    # value is a 4-case mean and every arm must use the SAME 4 cases or the
    # C1e comparison is between different populations.
    for arm in C.BASELINE_DESK007:
        n = counts[f"{arm}.release"]
        if n != C.BASELINE_RELEASE_N:
            problems.append(f"{arm}.release n={n} != {C.BASELINE_RELEASE_N}")

    # Per-case pre-bias / delta, the inputs to A-P1 and to the g* prediction.
    ...
    for src in (ROLLOUT_TSV, ZDIFF_TSV):
        ...
        dst = C.BASELINE_DIR / f"e211_{src.stem.replace('e209_', '')}_desk007.tsv"
    ...
    summary = {
        ...
        "measured": measured,
        "frozen": C.BASELINE_DESK007,
        "release_n": {a: counts[f"{a}.release"] for a in C.BASELINE_DESK007},
        "problems": problems,
    }
    out = C.BASELINE_DIR / "e211_baseline_summary.json"
    ...
    for arm in ("prg", "g1"):
        ...
        print(
            ...
            f"eef_ori={m['eef_ori_deg']:7.4f} contact={m['contact']:.4f} "
            f"release={m['release']:.4f} narrow={int(m['narrow_pass'])}/{C.EXPECTED_CASES}"
        )
    if problems:
        print("\n".join(f"  FAIL {p}" for p in problems))
        raise SystemExit(f"P0 baseline freeze FAILED ({len(problems)} problems)")
    print(f"\nP0 PASS: both endpoints reproduce plan241 -> {out.relative_to(REPO)}")
    return 0
