#!/usr/bin/env bash
# E211 P5 -- rules/experiment.md §7 Safeguard 2.
#
# Snapshots the 5 desk007 dcv3 *task* dirs (NOT the `<obj>_person<N>/` source
...
# three E211 partial sidecars, so the whole comparison stays recoverable even if
# the working copies are later overwritten.
#
# Only shard A's machine runs this.  Both machines share /mnt, and two concurrent
# snapshots would race on the same output dir; run_E211_local_8gpu.sh therefore
...
import sys
sys.path.insert(0, "workspace/core4d/scripts/experiments/E211")
import e211_common as C
...
if [ "${#TASKS[@]}" -ne 5 ]; then
  echo "ERROR: expected 5 desk007 task dirs, got ${#TASKS[@]}" >&2
  exit 1
fi

bash workspace/core4d/scripts/convert/snapshot_scenes.sh E211 "${TASKS[@]}" > /dev/null

MANIFEST="workspace/core4d/results/E211/scene_snapshot/manifest.txt"

# Exit check: every task dir must carry all five arms' scenes, and the three
# E211 hashes must match the CEM manifest -- otherwise the snapshot documents a
...
print(f"P5 PASS: {len(tasks)} task dirs snapshotted; all 5 g-curve scenes present, "
      f"E211 hashes matched to the CEM manifest")
print(f"  {manifest}")
PY
