#!/usr/bin/env python3
"""E211 P2: emit the 15 partial-gravcomp sidecars and prove they are single-variable.

Writes ``scene_act_E211_lowgeom_PRG_gc{04,06,08}.xml`` beside each desk007 case's
``scene_act_E206_lowgeom_PRG.xml``.  Four independent proofs that the gravcomp
value is the only change:

  1. XML-level   -- ``e211_common.assert_gravcomp_diff_value`` compares full
...
Usage:
    MUJOCO_GL=disable .venv/bin/python \
      workspace/core4d/scripts/experiments/E211/build_scenes.py [--overwrite] [--self-test]
"""
...
def main() -> int:
    ...
    out = C.MANIFEST_DIR / "e211_scene_audit.json"
