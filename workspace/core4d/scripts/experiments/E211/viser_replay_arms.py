#!/usr/bin/env python3
"""Replay E211 g-curve arms side-by-side in a headless viser server, with a
...
GUI
  Case      dropdown over the 5 desk007 cases; switching rebuilds the timeline.
...
def main() -> int:
    ...
    ap.add_argument("--cases", default="", help="restrict the picker (default: all 5)")
    ...
    if unknown_c:
        raise SystemExit(f"unknown case(s) {unknown_c}; E211 scope is {list(C.CASES)}")
    ...
    V._STATE.server = _viser.ViserServer(
        host=args.host, port=args.port, label="E211 g-sweep")
