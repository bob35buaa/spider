#!/usr/bin/env python3
"""Build the 15-row E211 Stage A manifest, plus the two execution shards.
...
The split is round-robin over the canonical (case_id, arm) order, not two
contiguous halves, so each shard spans all 5 cases and all 3 g values.  If one
machine dies, the surviving shard is still an interpretable cross-section of the
sweep instead of "only the 028 rows".

Usage:
    .venv/bin/python workspace/core4d/scripts/experiments/E211/build_manifest.py
...
#: shard label per round-robin index parity. A = local (8 rows), B = remote (7).
SHARDS = ("A", "B")
EXPECTED_SHARD_ROWS = {"A": 8, "B": 7}
...
def build(stage: str, cases: list[str], arms: list[str], samples: int, opt_steps: int):
    ...
    # the index in THIS order, so it is reproducible from the manifest alone.
    pairs = [(c, a) for c in cases for a in arms]
    pairs.sort()
    for index, (case_id, arm) in enumerate(pairs):
...
def check_shards(rows: list[dict[str, str]], stage: str) -> None:
    ...
    if len(rows) != C.EXPECTED_ROWS:
        return  # subset run (smoke / --cases); the counts below only apply to full
    ...
    # Round-robin should give each shard every case and every arm; if a future
...
def main() -> int:
    ...
    if scene_shas & base_shas:
        raise SystemExit("an E211 scene equals the E206 baseline scene byte-for-byte")
    ...
    print(f"  {len(scene_shas)} distinct E211 scene hashes, none equal to a baseline hash")
    return 0
