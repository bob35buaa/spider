#!/usr/bin/env python3
"""E211 P6/P7 exit check: prove at RUNTIME that each arm ran its own g value.
...
E211 adds one check E209's version could not need: it compiles the model the run
actually loaded and reads ``MjModel.body_gravcomp`` for the object body.  For a
...
Usage:
    .venv/bin/python workspace/core4d/scripts/experiments/E211/audit_runtime_contract.py \
...
def audit_case(case_id: str, arm: str, stage: str) -> list[str]:
    ...
    for label, cfg, want in (("E211", cand, want_scene), ("E206", base, C.BASE_SCENE)):
...
def main() -> int:
    ...
    # still-running rows as audited -- the audit would report "15/15 PASS" while
    ...
    out = C.S6_DIR / f"cem/{args.stage}/e211_runtime_contract.json"
