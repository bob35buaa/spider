#!/usr/bin/env python3
"""Export selected E156 +gateA rows as S6 RL handoff inputs."""
...
RESULT_ROOT = REPO / "workspace/core4d/results/E157/gateA_rl_export"
...
E156_METRICS_REF = "workspace/core4d/results/E156/clean8_gate_decay/eval/full/e156_method_metrics.tsv"
...
def load_sources() -> list[dict[str, str]]:
    variants = {
        ...
        if row.get("short_case_id") in TARGET_SHORT_CASES and row.get("method_group") == "gateA"
    }
    missing = [case for case in TARGET_SHORT_CASES if case not in variants]
    if missing:
        raise SystemExit(f"missing E156 gateA variants: {missing}")

    metrics = {
        row["short_case_id"]: row
        for row in read_tsv(E156_METRICS)
        if row.get("short_case_id") in TARGET_SHORT_CASES and row.get("method") == "+gateA"
    }
    missing_metrics = [case for case in TARGET_SHORT_CASES if case not in metrics]
    if missing_metrics:
        raise SystemExit(f"missing E156 +gateA metrics: {missing_metrics}")
        ...
        rows.append(
            {
                ...
                "candidate_decision": "E156_GATEA_READY",
                ...
                "stage2b_result_root": "workspace/core4d/results/E156/clean8_gate_decay",
                "stage2b_manifest_ref": "workspace/core4d/scripts/experiments/E156/variants.tsv",
                "raw_contact_threshold_label": "3cm",
                "source_exp": "E156",
                "source_variant": source["variant"],
                "source_metrics_ref": E156_METRICS_REF,
                "notes": notes(source),
            }
        )
    return rows
        ...
        rows.append(
            {
                ...
                "cem_metrics_ref": E156_METRICS_REF,
            }
        )
    return rows
...
def summarize(
    *,
    ...
    summary = {
        "experiment": "E157",
        "created_at": now(),
        "source": "E156 +gateA clean8 subset",
...
def main() -> None:
    ...
    source_path = manifest_dir / "gateA_source_rows.tsv"
    ...
    if args.execute_partner:
        partner_cmd.append("--execute")
    if args.force_partner:
        partner_cmd.append("--force")
    try:
        run_cmd(partner_cmd)
    except subprocess.CalledProcessError:
        if not args.allow_partner_failure:
            raise
        print("partner OmniRetarget command failed; keeping manifest if generated", file=sys.stderr)
