#!/usr/bin/env python3
"""E212 P7 exit check: reconcile the two-machine shards back into the main manifest.
...
enter P8 until the two halves are proven to compose into exactly the intended 12
...
Checks, in the order a wrong answer would be cheapest to catch:
  ...
  1. Coverage   -- shard keys are disjoint and their union is exactly the main
     manifest's 12 (case_id, arm) pairs.  Catches "someone ran shard A twice"
...
Usage:
    .venv/bin/python workspace/core4d/scripts/experiments/E212/merge_shards.py [--stage full]
"""
...
def _load_sibling(name: str):
    ...
    ``e212_common`` puts E209/E206/E200/E199 on sys.path ahead of E212, and every
    ...
    spec = importlib.util.spec_from_file_location(f"e212_{name}", path)
...
def main() -> int:
    ...
    out = C.MANIFEST_DIR / f"{C.EXP.lower()}_stageA_{args.stage}_merge.json"
