#!/usr/bin/env python3
"""E212: render the Stage A rollouts to mp4 (sim left / reference-FK right).
...
(`E212/viser_replay_arms.py`) -- it puts every arm in one scene on one shared
...
Usage:
    MUJOCO_GL=osmesa .venv/bin/python \
      workspace/core4d/scripts/experiments/E212/render_cem_results.py
...
def main() -> int:
    ...
    ap.add_argument("--arms", default="G06,G08", help="E212 arms to render")
    ...
    if unknown:
        raise SystemExit(f"unknown E212 arm(s) {unknown}; known: {sorted(C.ARMS)}")
    ...
    if not rows:
        raise SystemExit("no finished E212 rollouts match the filters")
    ...
    payload: dict[str, Any] = {
        ...
        "camera_caveat": (
            ...
            "one video, or use E212/viser_replay_arms.py for a shared-camera A/B."
        ),
    }
    (out_dir / "e212_render_summary.json").write_text(
