#!/usr/bin/env python3
"""Build E119 upright-carry support-guard diagnostic manifest."""
...
SCRIPTS_ROOT = REPO / "workspace/core4d/scripts/E119"
RESULTS_ROOT = REPO / "workspace/core4d/results/E119"
...
class VariantSpec:
    ...
    leg_scale: float = 0.0
    leg_gate_source: str = "contact_mask_time_window"
    target_uses_eef_offset: bool = True
    cem_safety_gate: bool = False
...
RUNNABLE_VARIANTS = [
    ...
    VariantSpec(
        ...
        contact_gain=5.0,
        leg_scale=0.4,
        leg_gate_source="contact_mask_time_window",
        target_uses_eef_offset=True,
    ...
    for case_id, (split, phase_scope) in CASE_SPLITS.items():
        ...
        for spec in RUNNABLE_VARIANTS:
            rows.append(
                {
                    "ordinal": ordinal,
                    "variant": f"E119_{case_id}_{spec.ablation}",
...
# Auto-generated by workspace/core4d/scripts/E119/build_upright_carry_manifest.py.
# E119 upright-carry support guard; case={row['case_id']} ablation={row['ablation']}.
defaults:
  - {row['source_override']}
  - _self_
...
leg_object_penalty_gate_source: {spec.leg_gate_source}
...
# E119 combines E116 posture/body support guard with E118 carry corridor.
...
cem_safety_gate_enabled: {str(spec.cem_safety_gate).lower()}
...
def write_preflight(rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for row in rows:
        ...
        out = {
            **{field: row[field] for field in FIELDS},
            ...
            "cem_safety_gate": spec.cem_safety_gate,
    ...
    write_tsv(PREFLIGHT_TSV, preflight_rows, preflight_fields, "E119 Phase A preflight")
    required = [
        "scene_act_exists",
        "scene_act_loads",
        "trajectory_exists",
...
def write_summary_md(rows: list[dict[str, Any]], preflight_rows: list[dict[str, Any]], summary: dict[str, Any]) -> None:
    ...
    lines = [
        "# E119 Upright Carry Manifest Summary",
        "",
    ...
    lines.extend(
        [
            "",
            "| case | ablation | split | target | corridor | safety gate | mask either | phase | queue |",
            "|---|---|---|---|---:|---:|---:|---|---|",
        ]
    )
    ...
    for row in rows:
        ...
        lines.append(
            ...
            f"`{pf['cem_safety_gate']}` | {float(pf['mask_active_either_frac']) * 100:.1f}% | "
...
def main() -> None:
    ...
    write_tsv(VARIANTS_TSV, rows, FIELDS, "E119 upright-carry support-guard variants")
