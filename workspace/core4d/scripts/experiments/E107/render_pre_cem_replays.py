#!/usr/bin/env python3
"""Render E107 selected-4 pre-CEM MuJoCo kinematic replays."""
...
from e107_common import RESULTS_ROOT, TASK_ROOT, rel, read_variants  # noqa: E402
...
def render_variant(row: dict[str, str], overwrite: bool) -> dict[str, Any]:
    ...
    if not (video.is_file() and sheet.is_file()) or overwrite:
        ...
        fig.suptitle(f"{row['variant']} | {row['derived_task']} | E107 Box021 clean kinematic replay")
    ...
    if not review.is_file():
        review.write_text(
            "\n".join(
                [
                    f"# E107 Pre-CEM Review: {row['variant']}",
                    "",
                    ...
                    f"- split: `{row['split']}`",
                    f"- gate frames: `{int(qpos.shape[0])}`",
...
def main() -> None:
    ...
    if not rendered:
        raise SystemExit("No E107 variants rendered.")
    write_gate(rendered)
