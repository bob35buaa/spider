#!/usr/bin/env python3
"""Strict preflight/runtime audit for E189 E167A without PRG."""
...
def scene_pair_leaks(path: Path) -> list[dict[str, str]]:
    ...
    for pair in root.findall("./contact/pair"):
        ...
        if (geom1 == "object_collision" and geom2 in LOWER_BODY_GEOMS) or (
            geom2 == "object_collision" and geom1 in LOWER_BODY_GEOMS
        ):
            leaks.append(dict(pair.attrib))
        elif pair.get("name", "").startswith(("E170_", "E172_", "E173_")):
            leaks.append(dict(pair.attrib))
    return leaks
...
def audit_row(
    row: dict[str, str], authority: dict[str, str], profile: dict[str, Any]
) -> dict[str, Any]:
    failures = []
    ...
    if override.is_file():
        ...
        prg_override = sorted(set(override_doc) & set(C.PRG_CONFIG_FIELDS))
        if prg_override:
        ...
        prg_composed = sorted(key for key in C.PRG_CONFIG_FIELDS if key in composed)
        if prg_composed:
    ...
    if scene.is_file():
        ...
        for hand in ("lh", "rh"):
            geom_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_GEOM, hand)
            hand_types[hand] = (
    ...
    if (
        authority["trajectory_sha256"] != row["trajectory_sha256"]
        or authority["contact_mask_sha256"] != row["contact_mask_sha256"]
    ):
        failures.append("paired_input_hash_drift")

    diagnostic_leaks = output_diagnostic_leaks(C.repo_path(row["outdir_npz"]))
    if diagnostic_leaks:
    ...
    return {
        ...
        "profile_mismatch_json": json.dumps(profile_mismatch, sort_keys=True),
        ...
        "prg_runtime_diagnostic_fields_json": json.dumps(diagnostic_leaks),
        "lh_geom_type": hand_types.get("lh", ""),
        "rh_geom_type": hand_types.get("rh", ""),
        "hash_checks_json": json.dumps(hash_checks, sort_keys=True),
        "no_prg_pass": not (prg_override or prg_composed or leaks or diagnostic_leaks),
...
def main() -> int:
    ...
    authority_rows = C.read_tsv(C.RESULTS / "input_authority/input_authority.tsv")
    ...
    if len(rows) != C.EXPECTED_PAIRED_ROWS:
        raise ValueError(f"Full manifest must have 43 rows: {len(rows)}")
    if set(authority) != {row["case_id"] for row in rows}:
        raise ValueError("manifest/authority case set mismatch")
    gpu_queues = C.compute_gpu_queues(list(authority))
    C.validate_queue_contract(set(authority), gpu_queues)

    audit_rows = [audit_row(row, authority[row["case_id"]], profile) for row in rows]
    ...
    failures = [
        row["case_id"] for row in audit_rows if row["audit_status"] != "pass"
    ]
    summary = {
        "created_at": C.now(),
        ...
        "method_parity_pass": sum(
            C.boolish(row["method_parity_pass"]) for row in audit_rows
        ),
        "no_prg_pass": sum(C.boolish(row["no_prg_pass"]) for row in audit_rows),
        "audit_pass": sum(row["audit_status"] == "pass" for row in audit_rows),
        ...
        "object_distribution": dict(
            Counter(row["object_key"] for row in rows)
        ),
    ...
    C.write_json(out_dir / "e167a_no_prg_audit_summary.json", summary)
