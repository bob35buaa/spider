# E048-E052 可视化驱动全面重新评估

## 状态: ✅ 完成 (2026-05-12)

## 背景

E048-E052 系列实验跨越 HDMI workflow 对齐、euler convention 修复、scene 物理配置重建.
在所有数值指标分析后, 对关键实验进行可视化复查, 发现**多个数值指标具有误导性**.

## E048 与 E041c 的关系

E048 的所有 baseline 实验使用 `+override=core4d_e041c` — **与 E041c 完全相同配置**,
唯一区别是碰撞盒通过 `fix_collision_boxes.py` 修复后重跑.

| E048 实验 | = E041c + | 碰撞盒变化 |
|-----------|-----------|-----------|
| E048_box025_baseline | E041c box025 重跑 | 碰撞盒增大 24% |
| E048_bucket010_baseline | E041c bucket010 重跑 | Y/Z 互换修正 |
| E048_desk005_baseline | E041c desk005 (新case) | — |
| E048_box023 | E041c box023 (新case) | — |

E041 目录下已创建 `E041c_*_collision_fixed` symlinks 指向 E048 对应文件.

---

### box023 (小箱子 35×37×41cm)

**E041c (E048_box023.mp4)**:
- t=0s: ref/sim 姿态接近, 正常初始化
- t=1s: **sim 机器人摔倒** — 臀部着地, 左腿抬起, 手搭在箱子上
- t=3s: **完全仰面倒地**, 腿朝天
- t=5s: 倒地状态, 上半身碰巧靠在箱子上
- **ObjPos=13.9cm 是假象**: 物体没被搬所以没漂移, 机器人摔倒的偶然接触造成 Contact 数字看似还行

**HDMI (E052a_box023, suitcase 模板)**:
- t=0s: ref/sim 姿态非常接近, 双腿站立
- t=2s: ref 弯腰伸手触碰箱子, sim 也弯腰但**箱子方向明显不对** (euler 178° 导致箱子旋转/倾斜)
- t=3s: 机器人仍站立弯腰, 箱子被推歪
- **Stability=100%, body tracking 好 (弯腰动作跟随), 但物体因 euler 错配被推向错误方向**

**HDMI (E048a, 原始 scene)**:
- 类似行为, 机器人站立, 物体有 24cm 偏移但行为物理合理

**结论**: box023 上 HDMI 是唯一可用的方法. E041c 的 ObjPos=13.9cm 不可信.

### box025 (大箱子 75×76×94cm)

**E041c (E048_box025_baseline.mp4)**:
- t=0s: 机器人站在大箱子旁
- t=2s: 机器人**趴在箱子上**, 严重前倾, 被箱子挡住
- Stability=98% 是因为箱子物理支撑了倒下的机器人

**HDMI (E049e_hdmi_box025_comparison_v5.mp4)**:
- t=1s: 机器人也趴在大箱子上, 与 E041c 类似
- 两种方法表现都差

**结论**: box025 是双人协作任务 (72×72×89cm), 单机器人臂展不够, **结构性不可解**.

### bucket010 (大桶)

**E041c (E048_bucket010_baseline.mp4)**:
- t=0s: 机器人站在桶旁, 姿态接近
- t=2s: 机器人站立, 手在桶侧面, 有一定交互
- **相对较好**, 机器人保持站立并有物体接触

### desk005 (小桌)

**E041c (E048_desk005_baseline.mp4)** — 详细逐帧:
- t=0s: 机器人站在桌旁, 手在桌面附近, ref/sim 接近
- t=2s: ref 站在桌旁. **sim 机器人已走远** — 桌子被甩在身后, 人桌分离
- t=4s: **sim 机器人完全离开桌子**, 独自站着走, 桌子出了画面
- t=6s: 同上, 机器人自顾自走路
- **Stability=100% 真实 (站着走), 但机器人与桌子完全分离 — 不是搬运, 是丢下桌子走了**
- ObjPos 数字只反映前几帧短暂跟随

**E035 (E035_desk005.mp4, local-frame body tracking)**:
- t=0s: ref/sim 非常接近
- t=2s: 机器人站立行走, 姿态跟随好
- **body tracking 更好, 但同样没有真正搬运桌子**

---

## 二次修正后的真实排名

### 按实际行为质量 (结合可视化)

所有 case 的**真实行为**:

| Case | E041c 实际行为 | HDMI 实际行为 |
|------|---------------|---------------|
| box023 | **摔倒** (Stab=38%) | 站立弯腰+物体被推歪 (Stab=100%) |
| box025 | 趴在箱子上 | 趴在箱子上 |
| bucket010 | 站立+手碰桶侧 (相对最好) | 未测试 |
| desk005 | **站着但丢下桌子走了** | 未测试 |

### 诚实评估: 没有任何 case 真正实现了 "搬运"

| Case × Method | 机器人站立? | 手触碰物体? | 物体跟随? | 真正搬运? |
|---------------|-----------|-----------|---------|---------|
| desk005 × E041c | ✅ | ❌ (只有前几帧) | ❌ | ❌ |
| bucket010 × E041c | ✅ | ✅ (手在桶侧) | 部分 | ❌ (推而非搬) |
| box023 × HDMI | ✅ | ✅ (弯腰触碰) | ❌ (euler错) | ❌ |
| box023 × E041c | ❌ (摔倒) | — | — | ❌ |
| box025 × 任一 | ❌ (趴着) | — | — | ❌ |

| Rank | Case × Method | 实际行为 | Stab | Body | ObjPos | 评价 |
|------|---------------|---------|------|------|--------|------|
| 1 | desk005 × E035/E041c | 站立行走 + 手接触桌子 | 100% | ~15° | ~10cm | ★★★ 最佳 |
| 2 | bucket010 × E041c | 站立 + 手在桶侧 | 100% | ~20° | ~16cm | ★★ 可用 |
| 3 | box023 × HDMI (E048a) | 站立弯腰 + 物体漂移 | 100% | 7.3° | 24cm | ★★ 站着但物体偏 |
| 4 | box023 × E041c | **摔倒** | 38% | 20° | 14cm(假) | ❌ 不可用 |
| 5 | box025 × 任一 | 趴在箱子上 | — | — | — | ❌ 单人不可解 |

### 关键修正

| 之前的结论 | 修正为 |
|-----------|--------|
| "E041c box023 ObjPos=14cm, 优于 HDMI 24cm" | ❌ 14cm 是假象 (机器人摔倒, 物体没被搬) |
| "desk005 是最好的 showcase" | ❌ 机器人丢下桌子自己走了, 不是搬运 |
| "HDMI body tracking 好但 object tracking 差" | ⚠️ 部分正确: HDMI 是 box023 唯一可用方法, 但没有任何方法真正实现搬运 |
| "E041c object tracking 优于 HDMI 2-8×" | ❌ 数字在所有 case 上都有误导性 |
| "两种方法互补, 可取各自优势" | ❌ 两种方法在所有 case 上都没有实现真正的搬运行为 |

---

## 对后续方向的影响

### 核心现实: 当前方法无法解决 loco-manipulation

无论 HDMI 还是 E041c (MJWP), **没有任何 case 实现了真正的搬运**:
- 机器人要么摔倒, 要么趴在物体上, 要么丢下物体走了
- 数值指标 (ObjPos, Contact) 存在系统性误导
- body tracking (关节角度追踪) 是唯一真正有效的能力

### 双机器人实验 (E016-E018) 也不可用

对 E016-E018 的视频仔细复查 — **全部是 connect 约束造成的假象, 不是真正搬运**:

**E016a (Gibbs CEM) t=2s**:
- 右侧机器人头完全歪掉 (~90° 倾斜)
- 左侧机器人手穿进箱子里
- 箱子倾斜, connect 约束强行拉扯

**E017d (Soft 2-Connect) t=3s**:
- 左侧机器人脚悬空
- 右侧机器人身体后仰不自然, 手张开没抓箱子
- 箱子被 connect 约束悬浮, 不是手托着

**E018d2 (Task-Space) t=3s**:
- 左侧机器人头穿进箱子
- 右侧机器人身体严重扭曲, 脚踩不实
- connect 约束强行维持物体位置

**结论**: 双机器人的 "obj_z_max=0.597m (112% ref)" 是 connect 约束强行拉出来的假指标.
机器人 body 穿模变形, 物理完全不合理, 视觉上完全不可用.

E031 泛化实验 (4 case 全失败) 进一步确认: 双机器人 CEM 方案不可行.

### 根本瓶颈

1. **CEM sampling-based MPC 无法产生 sustained contact**: 1024 samples × 32 iterations 的搜索空间对 loco-manipulation 不够
2. **Contact guidance decay (PD→0)**: 在最后 iteration 释放物体后, CEM 没有找到维持接触的策略
3. **Connect 约束是 hack**: 绕过接触发现问题但引入穿模/变形
4. **CORE4D 数据是双人协作**: 单机器人物理上无法完成原始任务的搬运部分

---

## 碰撞盒 "修复" 回退分析

### 发现: fix_collision_boxes.py 的 1.05x margin 使结果退化

对比 E041c box025 修复前后视频:

**修复前** (碰撞盒 0.85x mesh, 来自 template):
- t=2s: 机器人站着弯腰, 手在箱子侧面, 姿态合理
- t=3s: 箱子倾斜但机器人还站着
- t=4s: 虽然最终趴在箱子上, 但前几秒有合理的站立+接触

**修复后** (碰撞盒 1.05x mesh, AABB×1.05):
- t=2s: 机器人已完全趴在箱子上, 比修复前更早崩溃
- t=3s-4s: 姿态更差, 手完全没碰到箱子

### 数值对比

| | 旧碰撞盒 (template) | 新碰撞盒 (AABB×1.05) | Mesh 实际 |
|---|---|---|---|
| box025 XY | 0.305 (0.85x mesh) | 0.377 (1.05x mesh) | 0.358 |
| box025 Z | 0.446 (1.00x mesh) | 0.469 (1.05x mesh) | 0.447 |
| **体积** | 0.332 m³ | **0.534 m³ (+61%)** | 0.455 m³ |

### 分析

1.05x margin 使碰撞盒比视觉 mesh **还大 5%**:
- 手在距离视觉表面 ~2cm 处就被 "隐形墙" 弹开
- 机器人更容易被碰撞力推倒 (碰撞体积大 → 碰撞力矩大)
- 接触面积虽然更大, 但机器人无法靠近物体中心

**0.85x (旧值) 虽然偏小但对 CEM 有利**:
- 手可以穿过视觉表面少许 → 更深的 contact → 更大的摩擦力
- 物体有效尺寸更小 → 单人更容易操作

### 建议修复

将 `fix_collision_boxes.py` 的 margin 从 1.05 改为 **0.95 或 1.00**:
- 1.00: 碰撞盒 = mesh AABB (精确匹配)
- 0.95: 碰撞盒 < mesh (允许轻微穿透, 有利于接触)
- 然后重新跑 E041c baseline 对比

**50+ 实验, 跨 5 种方法, 没有任何实验产生视觉上可接受的 loco-manipulation 结果**:

| 方法 | Body Tracking | 物体搬运 | 视觉质量 | 代表实验 |
|------|-------------|---------|---------|---------|
| E041c 单机器人 | 站着(desk)或摔倒(box) | ❌ 没搬 | ❌ | E048 |
| HDMI 单机器人 | ✅ 站着走 | ❌ 物体推歪 | 勉强(body OK) | E048a |
| 双机器人 CEM | ❌ 穿模变形 | ❌ connect假搬运 | ❌ | E016-E018 |
| 双机器人泛化 | ❌ 全部崩溃 | ❌ | ❌ | E031 |
| 阻尼弹簧/xfrc | — | ❌ 物体翻转 | ❌ | E028-E029 |

### 唯一真正有效的能力

**Body tracking (关节角度追踪)**: HDMI 7.3°, E035/E041c 15-20°
- 机器人能复现人体的站立、弯腰、行走等全身动作
- 但无法通过物理接触搬运物体

### 数值指标 vs 视觉真实的系统性偏差

| 指标 | 数值看起来 | 视觉实际 | 原因 |
|------|----------|---------|------|
| ObjPos=14cm (E041c box023) | "物体追踪好" | 机器人摔倒, 物体没动 | 物体静止=低误差 |
| Contact=93% (HDMI box023) | "手触碰物体" | 物体方向错 178° | euler 错配下的假接触 |
| obj_z=0.597m (E017d dual) | "箱子被抬起 112%" | connect 约束悬浮 | 非物理接触力 |
| Stability=100% (E041c desk) | "机器人稳定搬运" | 丢下桌子自己走了 | 只看 pelvis_z |
| MPKPE=0.7cm (HDMI) | "完美 body tracking" | sim vs drifted ref | E048评估bug已修正 |
