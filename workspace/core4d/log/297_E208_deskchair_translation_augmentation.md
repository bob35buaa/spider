...
**日期**: 2026-09-05 起（**P0–P8 完成，C4 未通过；待 P9 全量人审定论**）
**实验域**: `core4d`
**Run**: **R294**
...
**最终：105/105 `cem_ok`，10.33 h，零失败、零超时。**

| 指标 | 值 |
|---|---|
| wall_min | p50 **43.6** / p90 55.6 / max 71.1 |
| 总算力 | **80.1 GPU-h** |
| 各变体 wall 中位 | rot0 43.6 · rot1 43.1 · trans0 43.9 · trans1 43.4 · trans2 43.9 |

最后一行是 **C3b 的正式结果**：**旋转增强的求解开销与平移无法区分**（全部落在 43–44 min），失败率同样为零。考虑到 F6 —— 旋转此前被 E199 记为「系统性不可达」而实际是从未执行过 —— 这条推翻了那个记载。

**C7(d) 冻结集校验通过**：`(case_id, variant)` 集合 sha 仍为 `41a31921807e9a6f…`，105 行未增未减，runner 全程只写 status/gpu_id/wall_min/failure_mode/updated_at。

### P7 渲染 — **105/105 完成**

`--watch` 模式自动跟随队列，最终 `rendered=59 reused=105 failed=0`，105 条 MP4 齐全（评审 feed 的 126 行含 21 条复用 E206 的 orig 视频，也全部到位）。
...
### F14 · **【全量定论】C4 未通过 —— 差 0.002，且代价出在手-物穿透而非跟踪精度**

> n=105 全量结果。中途（n=56）我曾观察到 +0.179 并把它标为「非结论」，全量回落到 **+0.152**；也曾偏向「代价随幅度增大」，**全量数据不支持**，见下文更正。

#### 两条预注册判据的最终结果

| 判据 | 实测 | 门 | 结论 |
|---|---|---|---|
| 配对通过率下降（McNemar） | **+0.1524** | ≤0.15 | **✗ FAIL** |
| `obj_pos` HL 中位差 | **+0.498 cm** | ≤+5.0 | ✓ PASS（宽裕 10 倍） |

**幅度判据过得很轻松，失败只在二值通过率这一条，而且只差 0.002 —— 即恰好一行。** orig 75/105 → aug 59/105，净差 16 行；若 aug 多过 1 行即为 15/105 = 0.1429 → PASS。

**但这个下降不是偶然**：McNemar 精确检验 **p = 0.0113**。26 条 orig 过→aug 挂、10 条 orig 挂→aug 过（不是单向崩塌）。

#### 门级归因

| 门 | 丢失条数 | 按变体分布 |
|---|---:|---|
| `hand_penetration` | **19** | rot1 4 · rot0 4 · trans0 4 · trans1 4 · trans2 3 —— **各变体均匀，不是 rot 特有** |
| `object_ori` | **8** | rot1 4 · rot0 2 · trans0 1 · trans2 1 —— **6/8 是 rot** |

`object_ori` 这条机制上说得通且是**旋转增强的固有代价**：Δobj_ori 中位按变体为 trans0 −0.140 / trans1 −0.152 / trans2 +0.063 / **rot0 +1.095 / rot1 +2.703** 度。物体被要求转 45° 并被跟踪，朝向误差上升是预期内的。

#### 更正：代价**不**随增强幅度增大

中途我从「分档通过率」读出「满幅才付代价」。全量数据不支持这个读法：

| 分档 | n | 通过率下降 | p | Δhand_pen 中位 |
|---|---:|---|---|---|
| full | 80 | +0.213 | **0.002** | +0.040 |
| partial | 15 | +0.133 | 0.688 | **+0.134** |
| weak | 10 | **−0.300** | 0.250 | −0.042 |

- **连续量与幅度无关**：Δhand_pen 对有效位移的 Pearson **r = +0.010**；分档中位里 partial（+0.134）反而**高于** full（+0.040）。
- **通过率的分档差异被样本量混淆**：full 独占 105 中的 80，是唯一有检验效力的档；partial（n=15）与 weak（n=10）的 p 值只说明「无法与无效应区分」，**不等于无效应**。

所以正确的说法是：**没有证据表明代价随增强幅度增大**；分档表里 full 显著、其余不显著，主要反映的是各档的样本量差异。
...
| Δhand_pen > 0 的比例 | 67% |
| 中位 | +0.040 |
| 重尾 | p90 **+0.182**、max **+0.355** |

即：**增强给手-物穿透加了一个偏正但很小的量，是它的重尾越过了 0.30 这道二值门。** 跟踪精度几乎没动。

#### 机制假说（未验证）与下一步

lowgeom 代理是按物体**原始位姿**手编的粗盒近似；物体挪动/旋转后手落在代理上的位置改变，粗盒与真实表面的偏差以穿透形式显现。与 E206 R8 的口径一致 ——「**先怀疑代理，不怀疑控制器**」。但 105 对的数据里，逐物体的丢失分布（desk021 12 / chair006 7 / desk023 4 / desk007 3）与盒数（5 / 10 / 9 / 12）方向一致但不构成单因解释，**不下结论**。

**处置**：
1. **不改预注册判据**。看到结果再调门槛正是计划明令禁止的；C4 记为 FAIL。
2. **判定权交给 P9 全量人审**。C4 只差一行，而 19 条 `hand_penetration` 翻门里有多少是目视可见的真穿模、多少是粗代理造成的度量假象，只有看视频能分。C5（人审 USE 率 ≥0.85）与 C6（数值过但人否 ≤0.20）是这次真正的裁决者。
3. 若人审显示多数是代理假象，下一步应是**提高代理覆盖后重跑**，而不是放宽门。

### F16 · **配对是聚类的 —— 校正后通过率的下降与噪声无法区分**

105 对不是 105 个独立样本：它们来自 **21 个 case × 5 变体**，每个 case 的 5 条**共享同一条 orig**，而且（实测 21/21）每个 case 的 5 个变体**全部落在同一 offset 档**。Wilcoxon/McNemar 假设配对独立，于是行级 p 值是**反保守**的。

| 检验 | 单元 | p |
|---|---|---|
| 行级 McNemar | 105 对（相关） | **0.0113** |
| **case 级符号检验** | **21 例（独立）** | **0.4545** |

case 级把每例先塌缩成一个数：**10 例变差 / 6 例变好 / 5 例持平，中位下降 0.000**。

**结论修正**：C4 的通过率下降点估计 +0.152 确实越了 0.15 的门，但**它的不确定度远大于越门的那 0.002** —— 校正聚类后该下降与噪声无法区分。相反，跟踪指标经得起校正：case 级 `obj_pos` HL **+0.540 cm，Wilcoxon p=0.001**（n=21 独立），统计上真实但幅度只有门的 1/10。

**对报表的两处修复**：
1. `indicative` 规则原先按**行数** n<3 判，永远不会触发（最小的格也有 10 行）。改为按 **case 数** —— `weak` 档 10 行但只有 **2 个 case**，现在正确标为 indicative。
2. 每个分层格加 `cases` 列。`by_offset_band` 的独立单元数是 full 16 / partial 3 / weak 2，而不是 80 / 15 / 10。

> 顺带作废我在 F14 里对分档的解读：`weak` 档「aug 反而更好（−0.300）」完全是**两个 orig 本来就全挂的 case**（orig 通过 0/2），与增强幅度无关。

### F17 · **14 门口径下，最大退化项是 `eef_ori` 而不是 `hand_pen`；后者高度依赖阈值**

C4 只看 6 门，评审 feed 看 12 门，而 funnel 的完整口径是 **14 门**（4 硬 + 10 带，阈值取自 `funnel_config`，与 E204/E205/E206 同尺）。补齐 14 门后：

| 门 | orig过→aug挂 | 挂→过 | 净 | 过率差 | 在 C4 的 6 门内 |
|---|---:|---:|---:|---|---|
| **eef_ori** | **16** | 0 | **−16** | **+0.152** | **否** |
| obj_ori | 8 | 0 | −8 | +0.076 | 是 |
| root_pos | 8 | 2 | −6 | +0.057 | 否 |
| **hand_pen（阈 0.32）** | 17 | **11** | **−6** | +0.057 | 是（但 C4 用 **0.30**） |
| root_ori | 6 | 0 | −6 | +0.057 | 否 |
| release | 6 | 2 | −4 | +0.038 | 否（6 条 n/a） |
| ankle_jerk | 5 | 0 | −5 | +0.048 | 否（Δp90 **209**） |
| body_z / leg_pen | 2 | 0 | −2 | +0.019 | leg_pen 是 |
| fall / obj_pos / contact / obj_speed | 0 | 0 | 0 | 0 | — |

两条实质结论：

1. **`eef_ori`（末端执行器朝向）是最大单一退化项，而 C4 的 6 门里根本没有它 —— C4 对此完全盲。** 16 例只挂不过，是所有门里唯一的单向退化。
2. **`hand_pen` 的严重程度高度依赖阈值。** C4 用 0.30 → 净损 16、主导 C4 失败；14 门用 0.32 → 净损仅 6（17 挂但 **11 反而变过**）。**F14 里「hand_penetration 主导」的归因只在 0.30 这个阈值下成立**，换成 funnel 的 0.32 就不再是主因。两个阈值都有出处（0.30 是 E199/E202 增强线，0.32 是 funnel/E206），但结论对这 0.02 敏感这件事本身必须写明。

另记：`release` 门有 **6 条 n/a**（该例的 3cm mask 没有松手窗口）。`FC.passes` 对非有限值返回 False，会让「不适用」冒充「失败」；Gate14 页把 n/a 单独标灰，但**计数口径保持与 E206 一致**以维持可比。

### F15 · rescore 的容差比它比对的数据还精确，掩盖了一个真 bug

正式跑 `--rescore-orig` 时初报 **231/252 不符**，看似 C7(c) 崩了。实际是两件事叠加：

1. **容差设错**。E206 发布表存 4 位小数，逐字节相同的重打分最大也会差 5e-5，而 plan238 写的容差是 **1e-6 —— 比它所比对的数据还精确**。于是每个被舍入的值都报成不符（实测那 231 处几乎全在 4.98e-05 量级），**真正的漂移反而淹没在噪声里**。改为 1e-4，并把「E206 表里根本没有这一列」从「不符」拆成 `not_published` 单独统计。
2. **被噪声掩盖的那一条是真的**：`body_z_err_p95_m` 在 **115/115 行全是 NaN**。根因是它既不由 `evaluate_sequence` 产出也不由 `run_health` 产出 —— 它是独立的「冻结四体绝对 Z 误差 p95」，由 `eval_E187_e178_compat.body_z_p95` 计算，E206 显式调用（`eval_E206_arm_ablation.py:130`），我漏了。

**影响范围**：C4 的 6 门不含 body_z，**C4 结论不受影响**；受影响的是 KEY_METRICS 配对分析里这一项静默失效，以及评审 feed 的 12 门视图把每一行都判挂 body_z（修复前 12 门 13/77，修复后 **58/126**）。

修复后 **C7(c) `rescore_orig` = PASS，0 处不符** —— 打分可复现性在 21 例 × 12 指标上干净确立。

> 教训：一个「过紧」的容差不是保守，而是**把检查变成噪声发生器** —— 它同时制造假警报并掩盖真问题。容差必须匹配被比对数据的精度。
>
> 遗留（规则 13）：`body_z_p95` 住在 `runners/eval_E187_e178_compat.py` 而非 `eval/core`。E187/E206/E208 三个实验都靠它，本该上提到公共模块。本次只标记不重构（动它会牵连另两个实验）。

### F13 · 增强位移在渲染上不可视觉验证 —— 人审要判的是质量，不是「增强有没有发生」

我做 orig-vs-aug 的 A/B 抽帧时，两帧看起来几乎一样，差点把它读成结论。实际是两层原因叠加，**这个 A/B 无效**：

1. **机位跟着物体走**。`spider/viewers/__init__.py:262-291` 的 `_auto_video_camera` 每帧按「sim 与 ref 的 body 位置并集」的包围盒算 lookat 中心与半径。物体挪了，相机跟着挪 —— 跨视频比绝对姿态、比物体在画面里的位置与大小**都无效**。这条陷阱 E209 已在 [log298 §4-2](298_E209_desk_chair_prg_g1_gravcomp.md) 登记过（它当时也是先误读、后撤回）。
2. **aug 视频的 ref 面板本身就是增强后的参考**，两个面板一起偏移，所以单视频内部也看不出与 orig 的差异。

**结论**：增强位移只能由数值确立 —— V5c 的 P3（场景物体位姿 == 它所声称那条 trimmed npz 的第 0 帧）与 P5（从 XML 独立重导出位移，与 manifest 相符到 1e-4 m）已经做到，且这两条比任何目视都强。

**对 P9 的直接影响**：人审的职责是判**质量**（穿模、漂浮、抖动、姿态是否合理、接触是否成立），**不是**判「这条到底增强了没有」。判后者要看 `offset_band` 与 `approach_trans_offset_m_max` 两列，它们已在 feed 里。有效的目视比较只有一种：**单个视频内部的 sim vs ref**（同帧共享相机）。
...
| C3 aug 构建正确性 | 位移/yaw/pair 数/V5c/V5e | ✅ **通过**：110/110 建成、**V5c 105/105**、**V5e 105/105**（实测差异集恰为 `{task}`） |
| C3b rot 可行率 | 报可行率 | ✅ **通过并推翻既有记载**：rot0 20/22、rot1 21/22（与 trans 持平），CEM wall 中位 43.1–43.6 min 与 trans **无法区分** |
| C4 aug-vs-orig 数值不劣 | McNemar ≤0.15、obj_pos HL ≤+5cm | ❌ **未通过**：通过率下降 **+0.1524**（超门 0.002，即一行；McNemar p=0.0113）；HL **+0.498 cm** 宽裕通过。判定权交 P9 人审，见 F14 |
| C5 人审 USE 率 | ≥0.85 | ⬜ **未开始** —— feed 126 行全部可播且全部有 MP4，门线由 orig 21/21=1.00 定为 0.85 |
| C6 视觉无度量欺骗 | ≤0.20 | ⬜ 未开始（脚本就位；抽样视觉已做，无度量欺骗迹象） |
| C7 复现性 / 快照 | (a) 快照 (b) V6d (c) rescore-orig (d) 冻结行集 | ✅ **四项全通过**：(a) 126 目录快照；(b) V6d **105/105**，`geom_ids` **105/105 相同**；(c) rescore **0 处不符**；(d) 冻结集 sha 未变 |
...
**总判定：C0–C3b、C7 全部通过；C4 以 0.002 之差未通过；C5/C6 未开始。**
按计划口径，C4 是数值门而**人审才是最终裁决**（E206 R9：数值门只做排序不做否决）。因此本实验现在的状态是 **待 P9 人审定论**，而不是失败或成功。
...
一切前置都已就绪 —— feed 126 行（21 orig + 105 aug）**全部可播、全部有 MP4**，播放器索引已验证，覆盖率脚本的 media 门 PASS。

```bash
bash workspace/core4d/scripts/eval/wrappers/review_player.sh E208AUG --port 8208
# 审完后回填 .../eval/aug/user_manual_review_filled.tsv，再跑：
.venv/bin/python workspace/core4d/scripts/experiments/E208/build_review_coverage.py
```

**人审要回答的正是 C4 悬而未决的那件事**（见 F14）：19 条 `hand_penetration` 翻门里，多少是目视可见的真穿模、多少是粗代理造成的度量假象。C5（USE 率 ≥0.85）与 C6（数值过但人否 ≤0.20）是本实验的最终裁决者。

审片时**必读 F13**：跨视频比物体位置/姿态/大小无效（机位跟着物体走），只能在单个视频内部比 sim 与 ref。

### P10 收口（人审后）

- 本日志补人审结果与总判定、`EXPERIMENT_TRACKER.md` 加行、`build_log_index.py`
- **plan238 落盘**，并订正 7 处已被执行推翻的内容：5 变体（非 trans-only）、排除 chair005（105 非 66）、offset 分层、`[0.18,0.22]` 硬门作废（F7）、V5c 需容纳 euler 约定重选（F8）、C4 粒度由 G4 判定为逐 case（F10）、rescore 容差 1e-6 → 1e-4（F15）
- **规则 13 遗留**：`body_z_p95` 应从 `runners/eval_E187_e178_compat.py` 上提到 `eval/core`（E187/E206/E208 三方共用），本次只标记（F15）
