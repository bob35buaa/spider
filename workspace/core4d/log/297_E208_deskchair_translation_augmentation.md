...
### F14 · **【中途，重要】C4 的通过率判据正在失败，代价出在手-物穿透而非跟踪精度**

> 状态：队列 56/105 时的中途观察，**不是结论**。但方向明确且需要现在记录。

两条预注册判据分道扬镳：

| n | 通过率下降（门 0.15） | `obj_pos` HL（门 +5.0 cm） |
|---:|---|---|
| 9 | +0.111 | +0.061 |
| 46 | +0.109 | +0.627 |
| 49 | +0.143 | +0.584 |
...
**跟踪精度这条判据过得很轻松且稳定**；失败完全来自配对通过率。

#### 门级归因

56 对里 16 条 orig 过→aug 挂，其中 **`hand_penetration` 占 13 条**，`object_ori` 4 条。反向（orig 挂→aug 过）有 6 条。

**不是阈值假象**：13 条里只有 2 条落在 0.30–0.32（即仅因增强线比 E206 的 12 门口径严 0.02 而挂）。其余 aug 值中位 0.374、最大 0.523，而 orig 只有 0.077–0.284。

#### 两个直觉解释都被测过，**都不成立**

1. **「幅度越大代价越大」——否**。`Δhand_pen` 与有效位移的 Pearson r = **−0.029**；按分档看 partial 的 Δ 中位（+0.088）反而**高于** full（+0.042）。（注意：通过率下降确实是 full +0.239 / partial −0.100，但那是二值门的翻转率，不是底层连续量的行为 —— 两者不可互相推断。）
2. **「orig 本来就贴着门」——否**。orig 距门 ≤0.12 的翻门率 25%，>0.12 的 20%，几乎无差。desk007 的 orig 中位 0.218 是四个物体里最贴门的，却 **0/14 翻门**。

#### 目前站得住的刻画

| 事实 | 数值 |
|---|---|
| 穿透增加是**系统性的** | 84% 的配对 `Δ > 0` |
| 但**中位很小** | +0.042 |
| **右尾很重**，翻门的是尾部 | p75 +0.114 / p90 +0.190 / max +0.310 |
| 尾部集中在 chair006 | 其 p90 +0.238、max +0.310，翻门 7/14 |
| desk007 尾最薄 | p90 +0.105，翻门 **0/14** |

即：**增强给手-物穿透加了一个系统性但很小的量，是它的重尾越过了 0.30 这道二值门；而跟踪误差几乎没动。**

#### 机制假说（尚未验证，不作结论）

lowgeom 代理是按物体**原始位姿**手编的粗盒近似。物体被挪动/旋转后，手落在代理上的位置随之改变，粗盒与真实表面的偏差就以穿透的形式显现。这与 E206 R8 的口径一致 ——「**先怀疑代理，不怀疑控制器**」。chair006（`blind3cm=0.429`，代理覆盖最差）尾最重、desk007（12 盒，代理最细）尾最薄，与该假说方向一致，但 desk021（5 盒，最粗）尾却很薄（max +0.138）却翻门 6/14，所以**盒数不是单一解释变量**，不能只凭 56 对下定论。

#### 处置

- **不改预注册判据**。看到结果再调门槛正是计划明令禁止的。
- 等 105/105 跑完后再判 C4，并把上表按全量重算。
- 若届时仍失败，这是一条**有价值的实验结论**而非事故：它说明 desk/chair 的物体增强代价不在跟踪，而在手-物接触保真度，且很可能是**代理精度**问题 —— 对应的下一步是提高代理覆盖后重跑，而不是放宽门。
- 人审（C5/C6）在此尤其关键：13 条 `hand_penetration` 翻门里有多少是**目视可见的真穿模**、多少是粗代理造成的度量假象，只有看视频能分。
