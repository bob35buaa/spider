...
逐门配对 delta（G1 − PRG，正 = 变差）。**全 14 门**（4 硬门 + 10 带门）：

| 门 | PRG | G1 | Δ | 说明 |
|---|---:|---:|---:|---|
| fall*（硬） | 0.0 | 0.0 | 0.0 | 两臂皆无跌倒 |
| body_z*（硬） | 0.0753 | 0.0757 | +0.0004 | 门 ≤0.20，无变化 |
| ankle_jerk*（硬） | 605.8 | 620.7 | **+14.9** | 门 <1000，仍过；方向与「代价在机器人侧」一致 |
| obj_speed*（硬） | 1.1834 | 1.1981 | +0.0147 | 门 <3.0，无实质变化 |
...
| F8 | **逐门表只有 10+1 门，漏了 3 个硬门**（`fall` / `ankle_jerk` / `obj_speed`）。根因在 E206 runner 的 `per_gate` 循环写的是 `BANDED_GATES + body_z`，E209 照抄。漏报的危害是：读者会默认未列出的门没变，而实际上它们只是没被算 | eval runner 改为 `HARD_GATES + BANDED_GATES` 共 14 门并加 `kind` 列；`fall_flag` 是布尔，加 `num()` 强制转 0/1（原先 `_finite` 把它变 NaN，会让 fall 门从所有聚合里静默消失）。xlsx 的 `paired_delta` 从 7 个手挑指标扩到全 14 门。重算后 `e209_two_arm_rollout.tsv` 逐字节一致，确认打分确定、结论不变 |
