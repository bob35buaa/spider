...
**方法学结论（对后续 arm 有用）**：
1. **`contact_in_mask` 不是虚扶的检测器**（−0.016 完全没反应），**`eef_ori` 才是**。E194 C4 当年担心的风险是对的，但盯错了指标；E207 因为 bucket 上 eef_ori 没被触发而没暴露。
...
1. **P8 人审（唯一待办）**：`bash workspace/core4d/scripts/eval/wrappers/review_player.sh E209ARM`，22 例 A/B 对看 PRG vs G1 视频。**重点确认「虚扶」是否肉眼可见**——数值上 eef_ori +2.78° / desk007_034_p1 +33.7° 已经指向它，人眼是最后一道验证。降级必须填 `object_floating` taxonomy。
