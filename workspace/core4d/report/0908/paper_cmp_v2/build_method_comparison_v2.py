...
# OmniRetarget: recomputed in-repo by recompute_omnirt_v2.py (kinematic eval of its
# converted trajectory, gen_paper_results protocol) -- SPIDER-CEM's upstream input.
OMNIRT_JSONL = OUT / "omnirt_metrics.jsonl"

METHODS = ["SPIDER-CEM", "OmniRetarget", "SBTO", "GMR"]  # ours first, then baselines
...
def _jsonl_by_case(path: Path, cases: set[str]) -> dict[str, dict[str, float]]:
    """Read recompute_omnirt_v2.py output {case_id: {metric_key: value}}."""
    out: dict[str, dict[str, float]] = {}
    for line in path.read_text(encoding="utf-8").splitlines():
        if not line.strip():
            continue
        r = json.loads(line)
        if r.get("status") == "ok" and r["case_id"] in cases:
            out[r["case_id"]] = {k: _num(v) for k, v in r.get("metrics", {}).items()}
    return out
...
def best_method(summ: dict[str, dict[str, dict[str, float]]], key: str, direction: str) -> str | None:
    ...
    ranked = ranked_methods(summ, key, direction)
    return ranked[0] if ranked else None


def ranked_methods(summ: dict[str, dict[str, dict[str, float]]], key: str, direction: str) -> list[str]:
    """Methods ordered best -> worst by mean (finite only)."""
    cand = [(m, summ[m][key]["mean"]) for m in METHODS if _finite(summ[m][key]["mean"])]
    cand.sort(key=lambda t: t[1], reverse=(direction == "higher"))
    return [m for m, _ in cand]


# --------------------------------------------------------------------------- md
def write_markdown(methods, cases, objects) -> None:
    L = ["# 50-case method comparison — SPIDER-CEM vs OmniRetarget vs SBTO vs GMR", "",
         f"_{len(cases)} cases (E214 paper set). All methods aggregate over "
    ...
    for c in cases:
        ...
        L.append("| " + " | ".join(row) + " |")
    L += ["", "## Caveats", "",
          "- 全部 4 方法在**同一 50 case**上聚合（覆盖 50/50）。contact@5mm / pen@5mm 为帧占比（%），"
          "max pen 为 mm，obj speed m/s，foot slip m。",
          "- **OmniRetarget（运动学上游，无碰撞）穿透很重**：pen@5mm 44.6%、max pen 21.6mm、接触仅 22%；"
          "SPIDER-CEM 的物理优化把接触提到 72% 同时把穿透降到 7.6%（这正是本方法的价值）。",
          "- **GMR 的低 pen@5mm 才是假象**：GMR 几乎不接触物体（contact@5mm≈0.2% ≪ SPIDER 72%），"
          "不接触自然不穿透；其 max pen 23.5mm 仍比 SPIDER 12.0mm 更深。评价接触质量应看 contact@5mm + max pen，"
          "不能单看 pen@5mm。",
          "- SPIDER-CEM 取自 E214 ablation 的 `full` 列（论文缓存 + 5mm/max-pen 现算），与 gen_paper_results "
          "的 SPIDER-CEM 一致；**OmniRetarget 用 recompute_omnirt_v2.py 复算**（gen_paper_results 的 OmniRetarget "
          "回放/评估协议，对其转换后的运动学轨迹做同口径 kinematic 评估）；GMR/SBTO 取各自 paper xlsx 的 per-case 表。",
          "- obj speed / foot slip 属健康类指标，各方法各自 pipeline 计算，绝对值口径可能略有差异；"
...
def write_latex(methods, cases) -> None:
    summ = {m: summarize(methods[m], cases) for m in METHODS}
    L = [r"% Auto-generated by build_method_comparison_v2.py (50-case, 4 methods).",
         r"% Requires: \usepackage{booktabs}.", r"",
         r"\begin{table}[t]", r"\centering",
         r"\caption{Method comparison on CORE4D-G1 (all 50 paper sequences), "
         r"mean over cases. \textbf{SPIDER-CEM} (ours) vs \textbf{OmniRetarget} "
         r"(our upstream kinematic input), \textbf{SBTO}, and \textbf{GMR}. "
         r"Physical contact/penetration are frame fractions from the same MuJoCo "
         r"contact detector; max penetration is the deepest hand-object "
         r"interpenetration. $\uparrow$/$\downarrow$: higher/lower is better; best "
         r"per row in bold, second best \underline{underlined}. OmniRetarget "
         r"(kinematic, no collision) penetrates "
         r"heavily (44.6\% pen@5mm, 21.6\,mm max) at modest contact; SPIDER-CEM's "
         r"physics optimisation raises contact to 72\% while cutting pen to 7.6\%. "
         r"GMR's low pen@5mm is instead a `barely-touches' artifact (contact@5mm "
         r"$\approx0.24\%$); its max penetration (23.5\,mm) still exceeds ours.}",
         r"\label{tab:method_cmp50}",
         r"\begin{tabular}{l " + "c" * len(METHODS) + r"}", r"\toprule",
         "Metric & " + " & ".join(METHODS) + r" \\", r"\midrule"]
    for key, label, direction, kind in METRICS:
        ranked = ranked_methods(summ, key, direction)
