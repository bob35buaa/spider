...
- [x] 2026-06-09 E151 full 已启动：本地 tmux `E151_local_full_232329` 跑 `local-gpu0` 3 条 box021，远端 tmux `E151_full_232329` 跑 `remote-gpu0` 3 条 box004 与 `remote-gpu1` 3 条 box023。启动检查确认 split/task 正确，首条 full 均进入 CEM；初始 full plan time 约 local 24s/tick、remote 9-11s/tick。期间 SSH 监控曾出现一次 `kex_exchange_identification` 断开，重试成功，未干预 tmux/GPU/其他进程。
- [x] 2026-06-09 E151 full 首轮监控：首条 full 仍健康推进，local `E151_box021_029_p2_b2_sup` 到 `sim_steps=40/150`（约 24s/tick），remote GPU0 `E151_box004_083_p2_b2_sup` 到 `74/210`（约 10.6s/tick），remote GPU1 `E151_box023_person2_b2_sup` 到 `82/272`（约 9.4s/tick）；当前 full root NPZ 仍为 0，符合进度预期。
- [x] 2026-06-09 E151 full 远端首批产物：远端 GPU0 `E151_box004_083_p2_b2_sup` 于 23:41:39 完成，GPU1 `E151_box023_person2_b2_sup` 于 23:43:57 完成，远端 root NPZ 2/6；两卡均已进入 B2-tip。local `E151_box021_029_p2_b2_sup` 到 `sim_steps=118/150`，预计先完成本地首条。
- [x] 2026-06-09 E151 full 本地首条产物：local `E151_box021_029_p2_b2_sup` 于 23:51:40 完成，root NPZ 已生成，本地进入 `b2_tip`；remote GPU0 `box004_b2_tip` 到 `144/210`，remote GPU1 `box023_b2_tip` 到 `132/272`。当前 full root NPZ：本地 1/3，远端 2/6。
- [x] 2026-06-10 E151 full 远端第二批进度：remote GPU0 `E151_box004_083_p2_b2_tip` 于 23:59:42 完成并进入 `b1_mesh`（约 `38/210`）；remote root NPZ 3/6。remote GPU1 `E151_box023_person2_b2_tip` 到 `246/272`，预计马上完成；local `box021_b2_tip` 到 `64/150`。SSH 监控第二次遇到 transient kex 断开，重试成功，未影响 tmux。
- [x] 2026-06-10 E151 full 远端进入最终 B1：remote GPU1 `E151_box023_person2_b2_tip` 于 00:04:16 完成并进入 `b1_mesh`（约 `42/272`）；remote root NPZ 4/6。remote GPU0 `box004_b1_mesh` 已在跑，local `box021_b2_tip` 到 `86/150`，本地 root NPZ 1/3。
- [x] 2026-06-10 E151 full 远端完成、本地最终 B1 运行中：local `E151_box021_029_p2_b2_tip` 于 00:19:36 完成并进入 `b1_mesh`，当前约 `62/150`，本地 root NPZ 2/3。remote tmux `E151_full_232329` 已自然退出，远端 root NPZ 6/6（box004 与 box023 的 B2-sup/B2-tip/B1-mesh 均生成）；下一步 pull full 远端结果，本地继续等待最终 B1。
- [x] 2026-06-10 E151 full 回收状态：已执行 `pull_E151_remote_results.sh full`，本地 full 目录已有 root NPZ/MP4 8/9，remote 6/6 已回收。本地最终 `E151_box021_029_p2_b1_mesh` 仍运行，当前约 `116/150`（约 25s/tick）；完成后即可运行 full eval。
- [x] 2026-06-10 E151 full 全部完成：本地最终 `E151_box021_029_p2_b1_mesh` 于 00:48:47 完成，tmux 自然退出；本地 full 目录 root NPZ 9/9、MP4 9/9，远端结果已回收。EGL destructor warning 仍只出现在进程退出清理阶段，NPZ/MP4/outdir 已生成；下一步执行 full eval（无 `--allow-missing`）。
...
- [x] 2026-06-09 E150 git sync + full 启动：已提交并推送 `a6d5564 exp(core4d): add E150 eef offset sweep runner`（未包含用户本地 `.vscode/settings.json`）。`run_E150_remote.sh full` 远端 gate 通过，tmux `E150_full_181630` 已启动；GPU0 跑 off08、GPU1 跑 off11，各 8 条。启动检查确认两张 RTX 6000 Ada 均进入 `E150_box021_035_p2_off08/off11` full CEM，CLI override 中含 `contact_hdmi_eef_offset=[0.08/0.11,0,0]`。
- [x] 2026-06-09 E150 remote full 第一对完成：远端 full root NPZ/MP4/outdir trajectory 为 2/2/2，完成 `E150_box021_035_p2_off08` 与 `E150_box021_035_p2_off11`。随后第二对 `box023_person2` 已启动，tmux 显示 task 为修复后的 `box023_person2_legobj`，证明 smoke 中的 scene sidecar mismatch 已解除；当前继续监控健康运行。
- [x] 2026-06-09 E150 remote full 进度 4/16：第二对 `E150_box023_person2_off08/off11` 完成，full root NPZ/MP4/outdir trajectory 为 4/4/4；第三对 `E150_box021_029_p2_off08/off11` 已启动。两张 GPU 仍健康运行，未干预任何进程。
- [x] 2026-06-09 E150 remote full 进度 6/16：第三对 `E150_box021_029_p2_off08/off11` 完成，full root NPZ/MP4/outdir trajectory 为 6/6/6；第四对 `E150_box004_082_p1_off08/off11` 已启动。两张 GPU 仍健康运行。
- [x] 2026-06-09 E150 remote full 进度 8/16：第四对 `E150_box004_082_p1_off08/off11` 完成，full root NPZ/MP4/outdir trajectory 为 8/8/8；第五对 `E150_box004_083_p2_off08/off11` 已启动。
- [x] 2026-06-09 E150 remote full 进度 10/16：第五对 `E150_box004_083_p2_off08/off11` 完成，full root NPZ/MP4/outdir trajectory 为 10/10/10；第六对 `E150_box021_035_p1_off08/off11` 已启动并正常推进。
- [x] 2026-06-09 E150 remote full 进度 12/16：第六对 `E150_box021_035_p1_off08/off11` 完成，full root NPZ/MP4/outdir trajectory 为 12/12/12；第七对 `E150_box026_139_p1_off08/off11` 已启动。
- [x] 2026-06-09 E150 remote full 进度 14/16：第七对 `E150_box026_139_p1_off08/off11` 完成，full root NPZ/MP4/outdir trajectory 为 14/14/14；最后一对 `E150_box004_083_p1_off08/off11` 已启动。SSH 状态读取间歇出现 kex 断开，但 tmux/GPU/CEM 日志均显示运行健康。
- [x] 2026-06-09 E150 remote full 完成：远端 full 自然结束，tmux `E150_full_181630` 已消失；远端 artifact audit 为 16/16 root NPZ、16/16 MP4、16/16 outdir trajectory、16 logs，GPU0/GPU1 回到空闲。下一步 pull 本地并跑 full eval/xlsx。
...
- [x] 2026-06-05 E145 后续推进计划创建并改为中文两阶段：基于 E144 当前状态（5cm pass: bucket 33 / desk 33 / chair 16；board/stick pass=0；21 source templates，其中 17 个 draft not_released），新增/改写 `workspace/core4d/plan/154_E145_full_nonbox_template_release_to_rl_ready_plan.md`。计划分 Phase 1“CEM-ready + 可视化”（显式 template release、Stage2b、target gate、visual QC、S5/CEM-ready preflight，不跑 CEM）和 Phase 2“本地 1 卡 + 远程 2 卡 full CEM”（评估、S6 evidence、`rl_export_input.tsv`），硬边界是不启动 RL。
- [x] 2026-06-05 18:17 CST E145 Phase 1 中间状态：已生成显式 template release TSV（21/21 `approve_clean`，bucket 9 / desk 8 / chair 4）并刷新 registry（82 rows `clean_reviewed`）；Stage2b 全量执行在 `desk023_20231030_019_p2` 处遇到 CVXPY infeasible 后中断，已有 28/82 verify summary。新增 `workspace/core4d/scripts/E145/run_stage2b_continue.py` 用于按 verify summary 断点续跑、记录 per-case failure 并刷新 E145 Stage2b manifest terminal 状态；`py_compile` 与 refresh-only 检查通过。该中间状态已被后续 69 pass / 13 fail 终态取代，未启动 CEM/RL。
- [x] 2026-06-05 18:45 CST E145 Stage2b 断点续跑中间状态：`run_stage2b_continue.py` 已持续跳过 solver infeasible rows 并继续剩余 case；当时约 44/82 verify summary，failure TSV 5 条。该中间状态已被后续 Stage2b 终态取代，未启动 CEM/RL。
- [x] 2026-06-05 18:59 CST E145 Phase 1 完成：Stage2b 82 rows 终态为 69 pass / 13 fail；target gate 69 pass / 13 not_run；visual QC render package 69 pass / 13 skipped，并补做 12 页 contact-sheet montage 视觉审查，visual QC manifest 69 pass / 13 not_run；S5 `HANDOFF_READY=69`；raw_mask_ref_fk CEM-ready 69 variants，preflight 69/69 pass，类别 bucket 33 / chair 11 / desk 25，三卡 split 为 local-gpu0 23 / remote-gpu0 23 / remote-gpu1 23。log: `workspace/core4d/log/184_E145_phase1_cem_ready_visual_qc_results.md`。本轮未启动 CEM/RL。
- [x] 2026-06-05 19:53 CST E145 Phase 2 人工标注清单解析完成：读取 UTF-16 标注表 `e145_phase2_case_selection_69-anno.txt`，并检查 E144 历史 CEM，确认 6 个 `bucket004_*` 已有 `raw_mask_ref_fk` full CEM 产物，标为不重跑。本轮按“只跑高、bucket007 最多 sample4、desk 只跑高、低/丢弃淘汰、中/空白下一轮”生成 resolved selection：本轮高优 9、下一轮 26、淘汰 34。输出 `e145_phase2_run_this_round_high.tsv`、`e145_phase2_next_round.tsv`、`e145_phase2_eliminated.tsv` 和 summary markdown；未启动 CEM/RL。
- [x] 2026-06-05 20:58 CST E145 Phase 2 高优 9 case full CEM 完成：新增 E145 Phase2 high runner/remote/pull/eval 固定入口，按 local 1 卡 + remote 2 卡完成 9/9 full CEM artifacts（root npz、full mp4、outdir trajectory 均齐全），pull 后 eval 9/9、missing 0。S6 evidence 9 rows：`cem_status=pass` 1、fail 8，`rl_status=not_run` 9；`s6_downstream/rl_export/rl_export_input.tsv` 为 `RL_EXPORT_READY=1`、`SKIP_CEM_FAIL=8`。本轮没有启动 RL，没有 checkpoint。事后补做 E144 full-CEM overlap audit，发现并记录 `bucket003_20231018_001_p2` 属于误重复计算；已给 E145 runner 加默认 E144-overlap guard，未来除非显式 `E145_ALLOW_E144_OVERLAP=1` 不允许重跑。log: `workspace/core4d/log/185_E145_phase2_high_priority_full_cem_results.md`。
- [x] 2026-06-05 21:24 CST E145 partner OmniRetarget 两条补齐：按 E108 `s6_downstream/rl_export/partner_omnirt` 格式刷新 9 行 `cases_rl_partner_omnirt.tsv`、manifest、summary、run script，当前 `pass=2`、`ready_to_execute=7`。已补齐 `bucket010_20231003_2_055_p2 -> bucket010_20231003_2_055_p1`（复用 E145 S3 已完成 OmniRetarget/trim artifacts 同步到 S6 partner root，trim `0/130`）和 `desk023_20231030_019_p1 -> desk023_20231030_019_p2`（标准 no-fingertip retarget 首次 CVXPY infeasible；保持 `REPLACE_WRIST_WITH_FINGERTIP=0`，仅对该 case 启用 Holosoma 自带 `--retargeter.enable-constraint-relaxation --retargeter.max-fallback-retries 3` 后通过，trim `57/139`）。本次只做/整理 OmniRetarget/trim，未启动 contact/SPIDER/CEM/RL。
### 2026-06-10 E152 续跑恢复
- [x] 恢复 E152 计划：当前目标是 `workspace/core4d/plan/160_E152_axis1_hand_object_physics_gate_plan.md`，不是继续改 E151。用户指出 `box004` 三个方法接触箱子前手碰地，需要在 E152 evaluator 显式报告手-地接触/穿透。
